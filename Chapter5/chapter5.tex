...
\end{align}
for some theory parameters $\lambda_{DBI}$, $V_0$, and $\beta_{IR}$. The sound speed can be obtained from \eqref{eqn:general_single_field_sound_speed}, which evaluates to $c^{DBI}_s \approx 3/\beta_{IR} N_e$ under slow-roll approximations \cite{Chen2005runningdbi}. Here $N_e$ denotes the number of e-folds until the end of inflation.
...
\begin{align}
	B_\Phi^{DBI}(k_1,k_2,k_3) = \frac{6A_\Phi^2}{(k_1 k_2 k_3)^3} \frac{-3}{7(k_1 + k_2 + k_3)^2} \left[ \sum_i k_i^5 + \sum_{i\neq j} (2k_i^4 k_j - 3k_i^3 k_j^2) + \sum_{i\neq j\neq l} (k_i^3 k_j k_l - 4k_i^2 k_j^2 k_l) \right]. 
\end{align}
The primordial potential $\Phi$ 



- DBI sound speed constraints approx relation $f_{NL}^{equil} = -(35/108)c_s^{-2}$ for $c_s \ll 1$. 
- DBI resonance scan
- tetrapyd decomp? 
