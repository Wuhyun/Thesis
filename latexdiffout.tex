...
%DIF DEL older.tex   Wed May 18 22:41:56 2022
%DIF ADD newer.tex   Wed May 18 22:40:36 2022

%DIF 3c3
%DIF < \documentclass[a4paper,12pt,times,numbered,print,index]{Classes/PhDThesisPSnPDF}
%DIF -------
\documentclass[a4paper,12pt,times,custombib,print,index]{Classes/PhDThesisPSnPDF} %DIF > 
%DIF -------
...
\ifuseCustomBib
%DIF 85c85
%DIF <    \RequirePackage[square, sort, numbers, authoryear]{natbib} 
%DIF -------
\RequirePackage[numbers,sort&compress]{natbib} %DIF > 
%DIF -------
...
\DIFdelbegin \DIFdel{And }\DIFdelend \DIFaddbegin \DIFadd{If this thesis was to be shared between everyone who made it happen, my share would be about a page. }\DIFaddend I would like to \DIFdelbegin \DIFdel{acknowledge . .}\DIFdelend \DIFaddbegin \DIFadd{use this page to thank them and apologise to those who were not mentioned.
}

\DIFadd{I would first like to thank my supervisors, James Fergusson and Paul Shellard. James has been the most patient and considerate supervisor one can ever hope for and was always there to give me valuable advice whenever I needed it. I am lucky to have been introduced to cosmology by him and benefit from his scientific intuition on many subjects. I am grateful to  Paul for his insightful comments throughout my studies and for running our department. I would like to thank Kacper Kornet for his technical support; he is the reason why my code could run at all. I also express my gratitude to Trinity College for creating such an inspiring environment to study and for providing financial support during my postgraduate studies.
}

\DIFadd{I am immensely grateful to Philip Clarke for helping me throughout the ups and downs of my PhD as a close collaborator, office mate, badminton buddy and an awesome friend. DAMTP is the place I will miss forever thanks to Áron Kovács, Fred Alford, Owain Salter Fitz-Gibbon, Miren Radia, Roland Bittleston, Daniel Zhang, João Melo, Charles Board, Roxana and Mycroft Rosca-Mead, and many more amazing people who I have had the privilege to meet.
}

\DIFadd{I am forever indebted to the members of the Donut who accompanied me throughout the pandemic. Chuyi Yang, Kaimyn Chapman, Paul Tao, and Zen Ng\textemdash thank you for making me feel at home during the wildest of times.
}

\DIFadd{Last but not least, I am incredibly privileged to have a caring family. Mom, dad, and Hyojie, your bottomless love and boxes full of Korean food sent from five thousand miles away gave me the energy to keep moving forward. And grandpa, sorry I am a little late, but I hope you get to celebrate your grandson's PhD degree all the same from where you are}\DIFaddend .
...
The Cosmic Microwave Background (CMB) is one of the most valuable probes of the universe we have today. Anisotropies present in the ancient light contain rich statistical information about the perturbations in the early universe and their subsequent evolution until now. The CMB bispectrum, the Fourier equivalent of \DIFaddbegin \DIFadd{the }\DIFaddend three-point correlation function, allows us to study weak non-Gaussian signatures of the primordial fluctuations. Primordial non-Gaussianity is a key prediction of many physically well-motivated inflation models, and measuring its shape and amplitude allows us to constrain them.

This thesis comprises two sections. In the first section, we present forecasts on primordial non-Gaussianity constraints from upcoming CMB \DIFdelbegin \DIFdel{Stage-4 }\DIFdelend surveys. We focus our attention \DIFdelbegin \DIFdel{to }\DIFdelend \DIFaddbegin \DIFadd{on }\DIFaddend models favoured by the Planck analysis, where a sharp feature in either the inflationary potential or sound speed causes oscillations in the bispectrum. Using preliminary specifications, we find that the Simons Observatory will have up to a factor of 1.6 improvements over Planck, increased to 1.7-2.2 for \DIFaddbegin \DIFadd{the }\DIFaddend CMB Stage-4 \DIFdelbegin \DIFdel{experiments}\DIFdelend \DIFaddbegin \DIFadd{experiment}\DIFaddend . Motivated by bright prospects, we developed a novel CMB bispectrum estimator suited for the resolution and sensitivity of future surveys.

We discuss our high-resolution bispectrum estimator in the second section. Our code, named CMB-BEst, utilises a set of general basis functions to accurately constrain a wide variety of models. Implementing such a flexible and precise estimator was a computationally challenging task. We detail our algorithm design, code optimisation and parallelisation for \DIFdelbegin \DIFdel{high performance }\DIFdelend \DIFaddbegin \DIFadd{high-performance }\DIFaddend computing clusters, which made this \DIFdelbegin \DIFdel{very }\DIFdelend challenging computation tractable. Validation tests, both for internal consistency and comparisons against conventional estimators\DIFdelbegin \DIFdel{are provided, }\DIFdelend \DIFaddbegin \DIFadd{, are provided }\DIFaddend together with a proof-of-concept application. We highlight how CMB-BEst can be used for both general and targeted analyses of previously unconstrained models.
...
The galaxies we observe today are in fact their images from \DIFdelbegin \DIFdel{more than }\DIFdelend \DIFaddbegin \DIFadd{tens of }\DIFaddend thousands of years ago. \DIFdelbegin \DIFdel{Since }\DIFdelend \DIFaddbegin \DIFadd{This is due to }\DIFaddend the speed of light \DIFdelbegin \DIFdel{is limited, we see the universe further back in time the farther away we look. To what extent can we do this? Can we have a glimpse of the universe from, say, 13.8 billion years ago?}\DIFdelend \DIFaddbegin \DIFadd{being finite, so the more distant an object is from us, the older the image we observe. This raises the question ``how far back can we see?''
}\DIFaddend 

The radio astronomers Arno Penzias and Robert Wilson were calibrating their 50-foot-long horn antenna in 1964 when they found a mysterious background noise \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Penzias1965}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . The measurements were independent of time and location in the sky and persisted after the removal of various potential contaminants. After the theoretical work of Robert Dicke, Jim Peebles, and David Wilkinson was brought forward\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Dicke1965}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend , Penzias and Wilson identified the noise as Cosmic Microwave Background (CMB) radiation: ancient light from the early universe reaching us after billions of years \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Penzias1965}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Penzias1965,Dicke1965}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . The discovery provided us with one of the most valuable probes of the physical universe. 

Our modern mathematical formulation of cosmology is built on Einstein's work on general relativity in \DIFdelbegin \DIFdel{1915. }\DIFdelend \DIFaddbegin \DIFadd{1915 \mbox{%DIFAUXCMD
\cite{Einstein1915fieldequations}}\hspace{0pt}%DIFAUXCMD
. }\DIFaddend Using his framework, Friedmann, Lemaître, Robertson, and Walker contributed to writing down the unique metric for a spatially homogeneous and isotropic universe \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Friedman1922flrw1,Friedmann1924flrw2,Lemaitre1927flrw,Robertson1935flrw,Walker1937flrw}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .  It was then realised that this FLRW metric has no stable static solution, and the universe likely started from a Big Bang and has been continuously expanding since. This was supported by Edwin Hubble's measurements of Cepheid variables in 1929 \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Hubble1929} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend and confirmed by the aforementioned discovery of the CMB, which was one of the key predictions of the Big Bang cosmology.

The Big Bang theory by itself, however, could not explain the apparent uniformity of the CMB in the early universe\DIFaddbegin \DIFadd{, as well as the flatness of the universe and the lack of magnetic monopoles today}\DIFaddend . The theory of cosmic inflation emerged to address \DIFdelbegin \DIFdel{this problem}\DIFdelend \DIFaddbegin \DIFadd{these problems}\DIFaddend ; the universe undergoes a phase of accelerated expansion shortly after the Big Bang \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Guth1981inflation}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . Inflation successfully resolved the problem and further provided a mechanism for generating the initial density fluctuations which grow into the structure in the universe we observe today. These primordial fluctuations originate from quantum \DIFdelbegin \DIFdel{fluctuation }\DIFdelend \DIFaddbegin \DIFadd{fluctuations }\DIFaddend of the inflaton which has \DIFdelbegin \DIFdel{near }\DIFdelend \DIFaddbegin \DIFadd{a nearly }\DIFaddend scale-invariant spectrum. This \DIFdelbegin \DIFdel{has been }\DIFdelend \DIFaddbegin \DIFadd{was }\DIFaddend confirmed when the Cosmic Background Explorer (COBE) discovered small anisotropies within the CMB in 1992 consistent with the inflationary predictions \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Smoot1992cobe}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .

What is widely accepted to be the standard model of modern cosmology, the $\Lambda$CDM model, appeared only in the late 1990s. In 1998, measurements of distant type Ia supernovae showed that the universe's expansion is accelerating \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Riess1998acceleration} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend which implied the presence of dark energy ($\Lambda$). Various astrophysical observations including galaxy rotation curves suggested \DIFaddbegin \DIFadd{the existence of dark matter \mbox{%DIFAUXCMD
\cite{Rubin1980rotationcurves}}\hspace{0pt}%DIFAUXCMD
, and }\DIFaddend that most of the matter in the universe is \DIFdelbegin \DIFdel{in fact }\DIFdelend dark matter. The cold dark matter (CDM) scenario is currently the most favoured. The $\Lambda$CDM model considers the primary constituents of the universe to be dark energy, cold dark matter, baryons and radiation. The $\Lambda$CDM model has been extremely successful in explaining modern cosmological observations. CMB measurements from the \textit{Planck} satellite, in particular, show exceptional agreement with the model \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{PlanckCollaboration2018Parameters}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . Planck has mapped the CMB anisotropy to remarkable precision and resolution. The Planck CMB data by itself has placed one of the most stringent bounds on the theoretical parameters of $\Lambda$CDM to date \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{PlanckCollaboration2018Parameters,PlanckCollaboration2018inflation}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .
...
\cite{Hinshaw2003wmapPower,Dunkley2011act,PlanckCollaboration2018power,BicepKeck2021}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . Meanwhile, higher-order statistics such as three-point correlation functions (bispectra in Fourier space) allow us to test the validity of the Gaussian assumption. They probe the non-Gaussian statistics of the CMB which can arise from either primordial dynamics or late-time effects \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Lewis2011lensing}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .

Primordial non-Gaussianity is a key statistic for studying \DIFaddbegin \DIFadd{the }\DIFaddend physics of the early universe \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Chen2010review,Komatsu2010}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . The theory of inflation has been successful in describing the observed data, but its exact mechanism is yet undetermined. Currently\DIFaddbegin \DIFadd{, }\DIFaddend there are numerous viable inflationary models with well-founded physical motivations. Non-Gaussian signatures of primordial fluctuations are robust predictions of various models, and measuring their shape and amplitude allows us to constrain particular inflationary scenarios. CMB bispectrum analysis from Planck yielded the most precise measurements of primordial non-Gaussianity to date \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{PlanckCollaboration2018}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . So far, no statistically significant amount of non-Gaussianity has been detected.

In the near future, we expect several new major CMB experiments. The Simons Observatory (SO) \cite{TheSimonsObservatoryCollaboration2018} is a ground-based experiment currently under construction in the Atacama Desert of Chile. SO is expected to measure both CMB temperature and polarisation to unprecedented precision, largely improved compared to Planck. The first light from SO is planned to be observed in early 2022. Many more \DIFaddbegin \DIFadd{next-generation experiments such as }\DIFaddend CMB Stage-4 (CMB-S4) \DIFdelbegin \DIFdel{experiments are proposed, brightening future }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Abazajian2016,Abazajian2019} }\hspace{0pt}%DIFAUXCMD
and LiteBIRD \mbox{%DIFAUXCMD
\cite{Hazumi2019litebird} }\hspace{0pt}%DIFAUXCMD
are due course, brightening }\DIFaddend prospects for CMB science. In particular, the upcoming measurements will allow us to \DIFaddbegin \DIFadd{further }\DIFaddend constrain primordial non-Gaussianity\DIFdelbegin \DIFdel{further}\DIFdelend , providing discovery potential \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Meerburg2019}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .
...
Chapters 4 and 5 contain my original work based on research conducted myself under \DIFaddbegin \DIFadd{the }\DIFaddend supervision of James Fergusson. Chapter \DIFdelbegin \DIFdel{4 }\DIFdelend \DIFaddbegin \textbf{\DIFadd{4}} \DIFaddend contains the forecast for future CMB-S4 surveys on the primordial non-Gaussianity parameter \DIFdelbegin \DIFdel{$f_{NL}$ }\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}$ }\DIFaddend which was published in \cite{Sohn2019}. SO experiment specifications and expected CMB-S4 setup were used to predict their improved constraints via Fisher information analysis. We focussed on models with oscillatory features, where the significant enhancement in polarisation sensitivity \DIFdelbegin \DIFdel{greatly benefits }\DIFdelend \DIFaddbegin \DIFadd{significantly enhances }\DIFaddend the constraining power \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{PlanckCollaboration2015}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .

Motivated by the positive prospects from forecasts detailed in Chapter \DIFdelbegin \textbf{\DIFdel{4}}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{4}\DIFaddend , we worked on developing a high-resolution bispectrum estimation pipeline suitable for future surveys. Chapter \textbf{5} contains the formulation and development details of the developed program, as well as many consistency checks performed during the extensive verification process. We outline the advantages of the new pipeline compared to conventional methods. Some working examples, studied in collaboration with Philip Clarke and Paul Shellard, are presented \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Sohn2021inprep}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . Lastly, Chapter \textbf{6} concludes the thesis by summarising and laying out plans for future research.

\DIFdelbegin \DIFdel{The chapters }\DIFdelend \DIFaddbegin \DIFadd{Chapters }\DIFaddend 1 and 2 are purely introductory and \DIFdelbegin \DIFdel{contains }\DIFdelend \DIFaddbegin \DIFadd{contain }\DIFaddend textbook materials for cosmology\DIFdelbegin \DIFdel{based on \mbox{%DIFAUXCMD
\cite{Dodelson2003textbook,Fergusson2020cosmology,Challinor2009lecture,Baumann2018primordialNotes,Lim2012advancedCosmo}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{. The formalism and interpretations presented are largely based on \mbox{%DIFAUXCMD
\cite{Dodelson2003textbook} }\hspace{0pt}%DIFAUXCMD
and publicly available lecture notes \mbox{%DIFAUXCMD
\cite{Fergusson2020cosmology,Challinor2009lecture,Baumann2018primordialNotes,Lim2012advancedCosmo}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . Chapter 3 consists of literature reviews on the topic with references in the text. All written \DIFaddbegin \DIFadd{texts, }\DIFaddend codes and plots in \DIFdelbegin \DIFdel{Chapter }\DIFdelend \DIFaddbegin \DIFadd{Chapters }\DIFaddend 4 and 5 are results of my own work unless explicitly specified otherwise.
...
In reality, the energy density and pressure appearing in the Friedmann equations are sums of contributions from different fluid components. \DIFdelbegin \DIFdel{Fractional }\DIFdelend \DIFaddbegin \DIFadd{The fractional }\DIFaddend density of a given fluid X is defined as
\begin{align}
	\Omega_X := \frac{\rho_X}{\rho_{\text{crit},0}}.
\end{align}
In \DIFaddbegin \DIFadd{the }\DIFaddend previous section we derived how each fluid's energy density depends on the scale factor. Quoting results summarised in Table \ref{table:cosmic_inventory}, the Friedmann equations can be rewritten as follows;
\begin{align}
...
\end{align}
These equations dictate the growth (or shrinking) of the universe given its curvature and energy density composition today. For \DIFdelbegin \DIFdel{$k <= 0$, the right hand }\DIFdelend \DIFaddbegin \DIFadd{$k \le 0$, the right-hand }\DIFaddend side of (\ref{eqn:Friedmann_3}) is always positive regardless of fractional density value. In this case, the fact that the universe is currently expanding suffices to show that scale factor $a$ has been increasing monotonically. The universe began with the Big Bang at $a=0$.

When $k > 0$, $\dot{a}$ vanishes at one or two values of $a$. There are multiple scenarios in this case, including \DIFdelbegin \DIFdel{the }\DIFdelend Einstein's Static Universe (ESU) where $\dot{a} = \ddot{a} = 0$. The ESU is however unstable; perturbing around the static solution as $a(t)=a_{\text{ESU}}(1+\xi(t))$ in (\ref{eqn:Friedmann_4}) gives $\ddot{\xi}>0$ to leading order, which implies that there exists a growing solution for $\xi$. In fact, there are no stable static solutions to the Friedmann equations, rendering such models implausible. Another possibility is a closed universe \DIFaddbegin \DIFadd{without dark energy ($\Omega_\Lambda = 0$)}\DIFaddend , where the scale factor grows until it hits the maximum and then decreases. This scenario requires $\ddot{a}<0$ at all times. Measurements of Type Ia supernovae strongly suggest that the universe is \DIFaddbegin \DIFadd{currently }\DIFaddend in an accelerating phase, ruling out this option as well. Lastly, there is the bouncing universe model. The scale factor starts large, drops to a minimum value, and bounces back to \DIFdelbegin \DIFdel{an }\DIFdelend accelerating growth. This model has been disregarded due to the need for \DIFaddbegin \DIFadd{the }\DIFaddend introduction of various new physics in the early universe \DIFdelbegin \DIFdel{, }\DIFdelend but has recently regained popularity as an alternative to inflation \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Brandenberger2017bouncingcosmo}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .
...
Inflation explains the homogeneity of the observed CMB by requiring $\mathcal{H}^{-1}$ to have shrunk rapidly in early times; $d\mathcal{H}^{-1}/d\ln a < 0$. We find that $\ddot{a} > 0$, so the universe undergoes accelerated expansion. \DIFdelbegin \DIFdel{Steep }\DIFdelend \DIFaddbegin \DIFadd{The steep }\DIFaddend decrease in $\mathcal{H}^{-1}$ is \DIFdelbegin \DIFdel{expressed }\DIFdelend \DIFaddbegin \DIFadd{parametrised }\DIFaddend using a `slow-roll' parameter
\begin{align}
...
\begin{align}
	v_\vv{k}(\tau) = \frac{1}{\sqrt{2k}} \left( 1 - \frac{i}{k\tau} \right) e^{-ik\tau}. \DIFdelbegin %DIFDELCMD < \label{eqn:bunch_davis_mode_function}
%DIFDELCMD < %%%
...
This chapter provides mathematical background and some physical intuitions for studying the CMB anisotropy\DIFaddbegin \DIFadd{, based on some standard materials on the topic including \mbox{%DIFAUXCMD
\cite{Dodelson2003textbook,Fergusson2020cosmology,Challinor2009lecture}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . We start by \DIFdelbegin \DIFdel{making }\DIFdelend \DIFaddbegin \DIFadd{taking }\DIFaddend a step forward from the homogeneous and isotropic universe discussed in the previous chapter and formulate the cosmological perturbation theory in Section \ref{section:the_inhomogeneous_universe}. Focussing on scalar perturbations we derive the linearised equations governing the time evolution of the perturbative variables in Section \ref{section:linearised_equations}. In Section \ref{section:CMB_anisotropy} we introduce the Boltzmann equation for photons and outline how \DIFdelbegin \DIFdel{they are }\DIFdelend \DIFaddbegin \DIFadd{it is }\DIFaddend solved numerically. We connect the primordial power spectrum to the angular power spectrum of the CMB anisotropy measured today.
...
\begin{align}
	ds^2 = \DIFdelbegin %DIFDELCMD < \bar{a}%%%
\DIFdelend \DIFaddbegin \DIFadd{a}\DIFaddend (t)^2 (-d\tau^2 + d\vv{x}^2)\DIFdelbegin \DIFdel{,
}\DIFdelend \DIFaddbegin \DIFadd{.
}\DIFaddend \end{align}
\DIFdelbegin \DIFdel{where we used a bar to distinguish the `background' quantities computed in the homogeneous universe. We now perturbed }\DIFdelend \DIFaddbegin \DIFadd{We now perturb }\DIFaddend the metric as follows.
\begin{align}
	ds^2 = \DIFdelbegin %DIFDELCMD < \bar{a}%%%
\DIFdelend \DIFaddbegin \DIFadd{a}\DIFaddend (t)^2 \left[ -(1+2A) dt^2 + 2B_i \; d\tau dx^i + (\delta_{ij} + h_{ij}) dx^i dx^j \right].
\end{align}
The spatial indices $i,j,\cdots$ here are lowered and raised using $\delta_{ij}$. Note that the scale factor has not been perturbed, since any variation of it can be absorbed into other perturbative variables. There are 1, 3 and 6 degrees of freedom coming from $A$, $B_i$ and $h_{ij}$, respectively, adding up to 10 as expected from \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{a }\DIFaddend metric tensor of \DIFaddbegin \DIFadd{the }\DIFaddend 3+1 dimensional spacetime.
...
The Scalar-Vector-Tensor (SVT) theorem states that to linear order in the perturbations, these modes decouple and evolve in three independent groups: scalars ($A,B,C,E$), vectors ($B_i^V,E_i^V$), and tensors ($E_{ij}^T$). These groups each contain 4, 4, and 2 degrees of freedom, \DIFdelbegin \DIFdel{respectively, }\DIFdelend again adding up to 10 as required. In this section, we are only interested in the scalar perturbations \DIFdelbegin \DIFdel{which generate }\DIFdelend \DIFaddbegin \DIFadd{responsible for generating }\DIFaddend the temperature anisotropy and E-mode polarisation \DIFaddbegin \DIFadd{observed }\DIFaddend in the CMB. \DIFaddbegin \footnote{\DIFadd{Tensor modes also generate the temperature anisotropy and E-mode polarisation as well as B-mode polarisation, but their contributions are negligible compared to those of the scalar modes.}} \DIFaddend We will therefore set the vector and tensor modes of the perturbations to zero.

Keeping only the scalar modes, the perturbed metric becomes
\begin{align}
	ds^2 = \DIFdelbegin %DIFDELCMD < \bar{a}%%%
\DIFdelend \DIFaddbegin \DIFadd{a}\DIFaddend (\tau)^2 \left[ -(1+2A) dt^2 + 2 \partial_i B \; d\tau dx^i + \left( (1+2C)\delta_{ij} + 2\partial_{\langle i}\partial_{j \rangle} E \right) dx^i dx^j \right]. \label{eqn:perturbed_metric_scalar}
\end{align}

Coordinate transformations are \textit{gauge} symmetries in General Relativity; they correspond to redundancies in our mathematical representation of the system. Redefining the coordinate variables may change how the theory looks\DIFdelbegin \DIFdel{like}\DIFdelend , but all physical results derived in the new set are equivalent to the original theory. These redundant degrees of freedom may still appear to evolve in a non-trivial manner and therefore need to be treated properly. Here, we outline how to fix a gauge in the perturbed metric and use gauge-invariant quantities to connect results from different gauges.
...
In order to complete the Einstein field equation, we need the energy-momentum tensor as well as the metric written to first order in perturbations. Recall that a perfect fluid in \DIFaddbegin \DIFadd{a }\DIFaddend homogeneous universe has $\bar{T}_{\mu\nu} = (\bar{\rho} + \bar{P}) \bar{U}_\mu \bar{U}_\nu + \bar{P} \bar{g}_{\mu\nu}$. The perturbations in $T_{\mu\nu}$ come from two sources: a) directly from perturbations in the energy density and pressure, and b) indirectly through variations in the metric and comoving observer's four-velocity. We consider the local inertial frame to separate these two effects. Let
\begin{align}
	(E_0)^\mu := a^{-1} (1 - \Psi) \delta^\mu_0, \quad (E_i)^\mu := a^{-1} (1 + \Phi) \delta^\mu_i. \label{eqn:newtonian_gauge_tetrad}
\end{align}
The four vectors above form an orthonormal frame where $g_{\mu\nu}(E_\alpha)^\mu (E_\beta)^\nu = \eta_{\alpha\beta}$. In this \DIFdelbegin \DIFdel{locally Minkowski }\DIFdelend \DIFaddbegin \DIFadd{locally-Minkowski }\DIFaddend frame, each component of the tensor $\tilde{T}^{\alpha\beta}$ \DIFdelbegin \DIFdel{corresponds to physical properties }\DIFdelend \DIFaddbegin \DIFadd{represent a physical property }\DIFaddend of the fluid. We write
\begin{align}
...
\end{align}
The momentum density $\vv{q}$ and the mean velocity $\vv{v}$ are first-order quantities which vanish in the absence of perturbations. The symmetric and traceless matrix $\Pi^{ij}$ represents anisotropic stress. This quantity vanishes for non-relativistic \DIFdelbegin \DIFdel{fluid }\DIFdelend \DIFaddbegin \DIFadd{fluids }\DIFaddend such as dark matter, while it remains small but non-zero for relativistic particles like photons.

Transforming back to the original coordinates through $T^{\mu\nu}=(E_\alpha)^\mu (E_\beta)^\nu \tilde{T}^{\alpha\beta}$, we get
\begin{alignat}{2}
	T^{00} &= a^{-2} \bar{\rho} &&\;\;+ a^{-2}\left( \delta\rho - 2\Psi\bar{\rho} \right)\DIFaddbegin \DIFadd{, }\DIFaddend \label{eqn:perturbed_energy_momentum_tensor_1}\\
	T^{0i} &= 0 &&\;\;+ a^{-2} q^i\DIFaddbegin \DIFadd{, }\DIFaddend \label{eqn:perturbed_energy_momentum_tensor_2}\\
	T^{ij} &= a^{-2} \bar{P} \delta^{ij} &&\;\;+ a^{-2} \left[ (\delta P + 2\Phi\bar{P})\delta^{ij} - \Pi^{ij} \right]\DIFaddbegin \DIFadd{,}\DIFaddend \label{eqn:perturbed_energy_momentum_tensor_3}
\end{alignat}
\DIFdelbegin \DIFdel{to }\DIFdelend \DIFaddbegin \DIFadd{at }\DIFaddend first order in perturbations. Note that when only the scalar modes are considered, we may write $v^i = \partial_i v$ and $\Pi^{ij} = \partial_{\langle i} \partial_{j \rangle} \Pi$ for some $v$ and $\Pi$. If multiple fluids contribute, then the total energy-momentum tensor can be obtained by summing over the individual values for each constituent $I$; $\delta\rho = \sum_I \delta\rho_I$, $\delta P = \sum_I \delta P_I$, and $\delta \vv{q} = \sum_I \vv{q}_I$.

\DIFaddbegin \DIFadd{The energy-momentum tensor is also subject to the gauge transformations }\eqref{eqn:gauge_transformation_matrix}\DIFadd{. Each perturbation variable appearing in $\delta T^{\mu\nu}$ transforms as
}\begin{align}
	\DIFadd{\tilde{\delta\rho} }&\DIFadd{= \delta\rho - T\bar{\rho}',  }&&\DIFadd{\tilde{\delta P} = \delta P -T\bar{P}', \label{eqn:gauge_transform_perturbations_3}}\\
	\DIFadd{\tilde{q}^i }&\DIFadd{= q^i + (\bar{\rho} + \bar{P})L',  }&&\DIFadd{\tilde{\Pi}^{ij} = \Pi^{ij}. \label{eqn:gauge_transform_perturbations_4}
}\end{align}
\DIFadd{We may therefore choose to work in a gauge where some of the above vanish. For example, $\delta \rho = B = 0$ in }\textit{\DIFadd{uniform density}} \DIFadd{gauge, and spatial slices of the fluid have constant energy density.
}


\DIFaddend \subsection{Initial perturbations}

With the metric and matter perturbations written down, we only need one more ingredient to \DIFdelbegin \DIFdel{be able to }\DIFdelend solve the linearised Einstein's equations: initial conditions. In inflationary \DIFdelbegin \DIFdel{$\Lambda CDM$, modulations about }\DIFdelend \DIFaddbegin \DIFadd{$\Lambda$CDM, deviations from }\DIFaddend the background solution originate from \DIFaddbegin \DIFadd{the }\DIFaddend quantum fluctuations of the inflation field as we discussed in Section \ref{section:quantum_fluctuations}. Statistical properties of the perturbations' distribution depend on the details of the inflationary scenario.
...
\end{alignat}
The background values \DIFdelbegin \DIFdel{at the zeroth order in perturbations }\DIFdelend appear as the first term in each equation. Note that these look different to \eqref{eqn:homogenous_christoffel_1}-\eqref{eqn:homogenous_christoffel_3} because the conformal time $\eta$ is used here instead of the comoving time $t$. Spatial indices are lowered and raised using \DIFdelbegin \DIFdel{delta function in }\DIFdelend \DIFaddbegin \DIFadd{the delta function }\DIFaddend above.
...
\end{align}
This is the continuity equation. Only the first two terms would be present if the spacetime \DIFdelbegin \DIFdel{was }\DIFdelend \DIFaddbegin \DIFadd{were }\DIFaddend flat; any changes in the energy density \DIFdelbegin \DIFdel{are caused by }\DIFdelend \DIFaddbegin \DIFadd{perturbation $\delta\rho$ are then due to }\DIFaddend the fluid flow \DIFaddbegin \DIFadd{parametrised by the momentum density $\vv{q} = (\bar{\rho}+\bar{P})\vv{v}$}\DIFaddend . The third term accounts for \DIFdelbegin \DIFdel{dilution of fluid density due to }\DIFdelend \DIFaddbegin \DIFadd{the dilution of the fluid density from }\DIFaddend the expansion of the universe. The last term corrects for the perturbations in the expansion, where $\Phi'$ comes from the time derivative of the effective spatial scale factor $a(\eta)(1-\Phi(\eta,\vv{x}))$. All factors of \DIFdelbegin \DIFdel{three }\DIFdelend \DIFaddbegin \DIFadd{3 }\DIFaddend here originate from having three spatial dimensions.
...
\begin{align}
	P^0 = a^{-1} (1 - \Psi)p, \qquad P^i = a^{-1}(1 + \Phi) p \hat{p}^i\DIFaddbegin \DIFadd{. }\DIFaddend \label{eqn:perturbed_photon_four_momentum}
\end{align}
...
\end{align}
to first order in perturbations. \DIFdelbegin \DIFdel{Photons travel more slowly in overdense regions where the gravitational potentials }\DIFdelend \DIFaddbegin \DIFadd{A comoving observer finds photons to be travelling more slowly while passing through overdense regions with }\DIFaddend $\Psi,\Phi<0$.
...
\end{align}
Only the first term on the \DIFdelbegin \DIFdel{right hand }\DIFdelend \DIFaddbegin \DIFadd{right-hand }\DIFaddend side is present at the zeroth order in perturbations, where we get $p\propto a^{-1}$. We confirm that photons are redshifted as the universe expands, justifying the definition of the redshift $z$ in \eqref{def:redshift}. The other two terms quantify the effect of gravitational perturbations on \DIFdelbegin \DIFdel{the }\DIFdelend photon energy. Photons \DIFdelbegin \DIFdel{in a deepening potential well have to spend some of their energy to climb out of it, hence the term $\Phi'$}\DIFdelend \DIFaddbegin \DIFadd{at a place where the potential is increasing over time }\textit{\DIFadd{gain}} \DIFadd{energy $\propto\Phi'$}\DIFaddend . On the other hand, \DIFdelbegin \DIFdel{moving into a region with deeper potential provides energy equal to $-({\vv{p}}\cdot\nabla)\Psi$}\DIFdelend \DIFaddbegin \DIFadd{photons moving out of a potential well }\textit{\DIFadd{lose}} \DIFadd{energy $\propto({\vv{p}}\cdot\nabla)\Psi$}\DIFaddend . Dividing by $p$ gives the last term in \eqref{eqn:perturbed_p_total_derivative}.
...
\begin{align}
	\left. \frac{df}{d\eta_e} \right|_\text{scattering} = \bar{n}_e \int d^2 \hat{\vv{p}}\DIFdelbegin \DIFdel{_{in} }\DIFdelend \DIFaddbegin \DIFadd{_\textrm{in} }\DIFaddend \; \frac{d\sigma}{d\Omega} (\hat{\vv{p}}, \hat{\vv{p}}\DIFdelbegin \DIFdel{_{in}}\DIFdelend \DIFaddbegin \DIFadd{_\textrm{in}}\DIFaddend ) \; \left[ f(p,\hat{\vv{p}}\DIFdelbegin \DIFdel{_{in}}\DIFdelend \DIFaddbegin \DIFadd{_\textrm{in}}\DIFaddend ) - f(p,\hat{\vv{p}})\right], \label{eqn:thomson_scattering_electron_rest_frame}
\end{align}
where $\eta_e$ and $\bar{n}_e$ denote the proper time and (background) mean number density of the electrons, respectively. We have suppressed the $\eta$ and $\vv{x}$ dependencies from the photon distribution function $f(\eta,\vv{x},p,\hat{\vv{p}})$ for now. The integrand of \eqref{eqn:thomson_scattering_electron_rest_frame} comprises two parts. The former corresponds to the photons originally travelling \DIFdelbegin \DIFdel{along the direction $\hat{\vv{p}}_{in}$ }\DIFdelend \DIFaddbegin \DIFadd{in the direction $\hat{\vv{p}}_\textrm{in}$ }\DIFaddend but then scattered \textit{into} $\hat{\vv{p}}$. The latter is the opposite; it accounts for the photons scattering \textit{out of} $\hat{\vv{p}}$ to some other direction, say \DIFdelbegin \DIFdel{$\hat{\vv{p}}_{in}$}\DIFdelend \DIFaddbegin \DIFadd{$\hat{\vv{p}}_\textrm{in}$}\DIFaddend . Both \textit{in} and \textit{out} scatters are equally effective since \DIFdelbegin \DIFdel{the }\DIFdelend Thomson scattering is \DIFdelbegin \DIFdel{a reversibleprocess}\DIFdelend \DIFaddbegin \DIFadd{reversible}\DIFaddend . The rate of interaction depends on the number densities $\bar{n}_e$ and $f$, as well as the \DIFdelbegin \DIFdel{angle dependent }\DIFdelend \DIFaddbegin \DIFadd{angle-dependent }\DIFaddend function given by
\begin{align}
	\frac{d\sigma}{d\Omega} (\hat{\vv{p}}, \hat{\vv{p}}\DIFdelbegin \DIFdel{_{in}}\DIFdelend \DIFaddbegin \DIFadd{_\textrm{in}}\DIFaddend ) = \frac{3\sigma_T}{16\pi} \left[ 1 + (\hat{\vv{p}} \cdot \hat{\vv{p}}\DIFdelbegin \DIFdel{_{in}}\DIFdelend \DIFaddbegin \DIFadd{_\textrm{in}}\DIFaddend )^2 \right] \label{eqn:thomson_scattering_cross_section}
\end{align}
...
\end{align}
where we relabelled \DIFdelbegin \DIFdel{$\hat{\vv{p}}_{in}$ }\DIFdelend \DIFaddbegin \DIFadd{$\hat{\vv{p}}_\textrm{in}$ }\DIFaddend as $\hat{\vv{n}}$ and performed the $\int d^2\hat{\vv{n}}$ integral for the \textit{out} term cancelling out the factor $3/16\pi$. We now have a complete form of the scattering term appearing \DIFdelbegin \DIFdel{Boltzmann's }\DIFdelend \DIFaddbegin \DIFadd{in the Boltzmann }\DIFaddend equations.
...
\end{align}
where $\tau$ is not to be confused with conformal time, also denoted $\tau$ elsewhere in other chapters. \DIFdelbegin \footnote{\DIFdel{Unfortunately, we could not have avoided the clash by writing $\eta$ for conformal time to begin with, since $\eta$ is also commonly used as one of the slow-roll parameters in inflation.}} %DIFAUXCMD
...
The Gaussianity of primordial perturbations from slow-roll inflation means that the $a_{lm}$s, which evolved linearly from the initial fluctuations, are also Gaussian. CMB measurements show that the $a_{lm}$s are indeed consistent with a multivariate Gaussian distribution. The form of \eqref{eqn:alm_from_phi} shows that the multipoles $a_{lm}$ are uncorrelated with each other \DIFdelbegin \DIFdel{, and}\DIFdelend \DIFaddbegin \DIFadd{and, }\DIFaddend since they are Gaussian distributed, vanishing correlation implies independence. In particular, the $a_{lm}$s with \DIFaddbegin \DIFadd{the }\DIFaddend same $l$ but distinct $m$s are independent and identically distributed. We can therefore combine them to get a more precise estimate for $C_l$;
\begin{align}
	\hat{C}_l = \frac{1}{2l+1} \sum_{m=-l}\DIFdelbegin \DIFdel{^{m=l} }\DIFdelend \DIFaddbegin \DIFadd{^{l} }\DIFaddend a^*_{lm} a_{lm}.
\end{align}
\DIFdelbegin \DIFdel{This allows us to do }\DIFdelend \DIFaddbegin 

\DIFadd{In cosmology, we are often interested in the theoretical distribution from which an observable is sampled from. Unfortunately, there is only one realisation of the universe we may observe: the one we live in. The number of samples used to estimate the underlying distribution is therefore limited, and it imposes an inherent lower bound on the estimation error called }\textit{\DIFadd{cosmic variance}}\DIFadd{.
}

\DIFadd{This also applies to estimating the angular power spectrum $C_l$ from a single set of $a_{lm}$s observed from CMB experiments. Even in a perfect experiment with zero measurement noise, we expect the variance of the estimator to be
}\begin{align}
	\DIFadd{\text{Var}\left[ \hat{C}_l \right] = \frac{2}{2l+1} C_l^2.
}\end{align}
\DIFadd{Having access to $2l+1$ independent samples $a_{lm}$ for each $l$ is therefore crucial for doing }\DIFaddend precision cosmology with the CMB data\DIFaddbegin \DIFadd{; the cosmic variance for $C_l$ is suppressed by a factor of $2l+1$}\DIFaddend .
...
The primordial perturbations are consistent with being Gaussian distributed according to observations to date \cite{PlanckCollaboration2018}. The statistical properties of a Gaussian random field are completely characterised by its mean and \DIFdelbegin \DIFdel{covariance }\DIFdelend \DIFaddbegin \DIFadd{two-point }\DIFaddend functions. The latter is directly related to the CMB angular power spectrum since the CMB is a near-linear probe of the early universe. In fact, the power spectrum is a sufficient statistic \DIFdelbegin \DIFdel{in the perfect Gaussianlimit}\DIFdelend \DIFaddbegin \DIFadd{if the perturbations are Gaussian}\DIFaddend , which means that it contains all the information about the distribution of the primordial perturbations we may ever extract from the CMB. The inverse is also true however; if the initial perturbations were non-Gaussian, it is essential to go beyond the power spectra and study higher-order statistics. Such non-Gaussian contributions are \DIFdelbegin \DIFdel{best }\DIFdelend \DIFaddbegin \DIFadd{well }\DIFaddend captured in the three-point correlation functions, or their Fourier counterpart, the bispectrum.

In \DIFdelbegin \DIFdel{single field }\DIFdelend \DIFaddbegin \DIFadd{single-field }\DIFaddend slow-roll inflation with standard kinetic term and vacuum, the primordial bispectrum is suppressed by slow-roll parameters \cite{Maldacena2013}. However, numerous other inflationary scenarios that are physically well-motivated violate these simple assumptions and hence predict non-Gaussian signatures. They are expected to leave imprints on the CMB bispectrum with \DIFaddbegin \DIFadd{a }\DIFaddend characteristic shape and amplitude, where the latter is parametrised by \DIFdelbegin \DIFdel{$f_{NL}$}\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}$}\DIFaddend .

We discuss the bispectrum in relation to primordial non-Gaussianity in this section, from both theoretical and observational \DIFdelbegin \DIFdel{side}\DIFdelend \DIFaddbegin \DIFadd{sides}\DIFaddend . Section \ref{section:bispectrum} covers the basic formalism of bispectrum analysis. Section \ref{section:primordial_non_Gaussianity} then introduces \DIFaddbegin \DIFadd{the }\DIFaddend theoretical tools for calculating the primordial bispectrum from a given inflation model Lagrangian. On the CMB side, we formulate the optimal bispectrum estimator and \DIFdelbegin \DIFdel{existing implementations of it }\DIFdelend \DIFaddbegin \DIFadd{its existing implementations }\DIFaddend in Section \ref{section:CMB_bispectrum_estimation}.
...
The squeezed, equilateral and flattened limits of bispectra all have distinct physical \DIFdelbegin \DIFdel{meaning}\DIFdelend \DIFaddbegin \DIFadd{meanings}\DIFaddend . For example, the squeezed limit corresponds to a configuration with two small-scale (large $k$) modes and one large-scale (small $k$) mode. Heuristically, this relates to how the small scale covariances are affected by an encompassing \DIFdelbegin \DIFdel{large scale }\DIFdelend \DIFaddbegin \DIFadd{large-scale }\DIFaddend mode.

One of the most studied bispectrum shapes comes from the \textit{local} model. In this model, the perturbative field $\zeta(\vv{x})$ is expanded as a local function of some Gaussian field \DIFdelbegin \DIFdel{$\zeta_G(\vv{x})$ as
}\DIFdelend \DIFaddbegin \DIFadd{$\zeta_\text{G}(\vv{x})$ as
}\DIFaddend \begin{align}
	\zeta(\vv{x}) = \zeta\DIFdelbegin \DIFdel{_G}\DIFdelend \DIFaddbegin \DIFadd{_\text{G}}\DIFaddend (\vv{x}) + \frac{3}{5} f\DIFdelbegin \DIFdel{_{NL} }\DIFdelend \DIFaddbegin \DIFadd{_\text{NL} }\DIFaddend \left(\zeta\DIFdelbegin \DIFdel{_G}\DIFdelend \DIFaddbegin \DIFadd{_\text{G}}\DIFaddend ^2(\vv{x}) - \left< \zeta\DIFdelbegin \DIFdel{_G}\DIFdelend \DIFaddbegin \DIFadd{_\text{G}}\DIFaddend ^2 \right> \right) + \cdots. \label{eqn:local_model_fNL_defitintion}
\end{align}
The non-linearity parameter \DIFdelbegin \DIFdel{$f_{NL}$ }\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}$ }\DIFaddend measures the amplitude of quadratic contributions to the field. A factor of $(3/5)$ here is conventional; the original definition of \DIFdelbegin \DIFdel{$f_{NL}$ }\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}$ }\DIFaddend was written in terms of the potential $\Phi$, which is equal to $(3/5)\zeta$ on super-horizon scales.

Substituting this form into \eqref{eqn:bispectrum_from_fourier_derivation_1} gives an expression involving correlation functions of four Gaussian fields, to leading order in \DIFdelbegin \DIFdel{$f_{NL}$}\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}$}\DIFaddend . Isserlis' theorem\DIFaddbegin \DIFadd{, also known as Wick's theorem to physicists, }\DIFaddend allows us to write such four-point correlators in terms of the sum over all possible contractions:
\begin{align}
	\left< \zeta\DIFdelbegin \DIFdel{_G}\DIFdelend \DIFaddbegin \DIFadd{_\text{G}}\DIFaddend (\vv{x}_1)^2 \zeta\DIFdelbegin \DIFdel{_G}\DIFdelend \DIFaddbegin \DIFadd{_\text{G}}\DIFaddend (\vv{x}_2) \zeta\DIFdelbegin \DIFdel{_G}\DIFdelend \DIFaddbegin \DIFadd{_\text{G}}\DIFaddend (\vv{x}_3) \right> = &\left< \zeta\DIFdelbegin \DIFdel{_G}\DIFdelend \DIFaddbegin \DIFadd{_\text{G}}\DIFaddend (\vv{x}_1)^2 \right> \left< \zeta\DIFdelbegin \DIFdel{_G}\DIFdelend \DIFaddbegin \DIFadd{_\text{G}}\DIFaddend (\vv{x}_2) \zeta\DIFdelbegin \DIFdel{_G}\DIFdelend \DIFaddbegin \DIFadd{_\text{G}}\DIFaddend (\vv{x}_3) \right> + \left< \zeta\DIFdelbegin \DIFdel{_G}\DIFdelend \DIFaddbegin \DIFadd{_\text{G}}\DIFaddend (\vv{x}_1) \zeta\DIFdelbegin \DIFdel{_G}\DIFdelend \DIFaddbegin \DIFadd{_\text{G}}\DIFaddend (\vv{x}_2) \right> \left< \zeta\DIFdelbegin \DIFdel{_G}\DIFdelend \DIFaddbegin \DIFadd{_\text{G}}\DIFaddend (\vv{x}_1) \zeta\DIFdelbegin \DIFdel{_G}\DIFdelend \DIFaddbegin \DIFadd{_\text{G}}\DIFaddend (\vv{x}_3) \right> \nonumber \\ &\hspace{0.15\textwidth} + \left< \zeta\DIFdelbegin \DIFdel{_G}\DIFdelend \DIFaddbegin \DIFadd{_\text{G}}\DIFaddend (\vv{x}_1) \zeta\DIFdelbegin \DIFdel{_G}\DIFdelend \DIFaddbegin \DIFadd{_\text{G}}\DIFaddend (\vv{x}_3) \right> \left< \zeta\DIFdelbegin \DIFdel{_G}\DIFdelend \DIFaddbegin \DIFadd{_\text{G}}\DIFaddend (\vv{x}_1) \zeta\DIFdelbegin \DIFdel{_G}\DIFdelend \DIFaddbegin \DIFadd{_\text{G}}\DIFaddend (\vv{x}_2) \right>. \label{eqn:local_model_derivation_1}
\end{align}
Contributions from the first term in \eqref{eqn:local_model_derivation_1} cancel out with the ones coming from \DIFdelbegin \DIFdel{$\left< \zeta_G^2 \right>$ }\DIFdelend \DIFaddbegin \DIFadd{$\left< \zeta_\text{G}^2 \right>$ }\DIFaddend in the definition \eqref{eqn:local_model_fNL_defitintion}. After expressing the two-point correlations in terms of the power spectrum in Fourier space \DIFaddbegin \DIFadd{as $\left< \zeta_\text{G}(\vv{k}_1) \zeta_\text{G}(\vv{k}_2) \right> = (2\pi)^3 \delta^{(3)}(\vv{k}_1 + \vv{k}_2) P_\zeta (k_1, k_2)$}\DIFaddend , we obtain the \textit{local} bispectrum;
\begin{align}
	B(k_1, k_2, k_3) = \frac{3}{5} f\DIFdelbegin \DIFdel{_{NL} }\DIFdelend \DIFaddbegin \DIFadd{_\text{NL} }\DIFaddend \left[ 2P_\zeta(k_2) P_\zeta(k_3) + 2P_\zeta(k_3) P_\zeta(k_1) + 2P_\zeta(k_1) P_\zeta(k_2) \right]. \label{eqn:local_bispectrum_using_power_spectrum}
\end{align}

Due to this fact, it is common in literature (e.g. \cite{Burrage2011large}) to define `the reduced bispectrum' \DIFdelbegin \DIFdel{$f_{NL}$ to be $f_{NL} := 5B(k_1,k_2,k_3) / 6(P(k_1)P(k_2) + P(k_2)P(k_3) + P(k_3)P(k_1))$. $f_{NL}$ }\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}$ to be $f_\text{NL}(k_1,k_2,k_3) := 5B(k_1,k_2,k_3) / 6(P(k_1)P(k_2) + P(k_2)P(k_3) + P(k_3)P(k_1))$. $f_\text{NL}$ }\DIFaddend is then a three-dimensional function in general. This \DIFdelbegin \DIFdel{can potentially be confusing due to }\DIFdelend \DIFaddbegin \DIFadd{is potentially confusing because of }\DIFaddend a conflicting convention in the observational community where \DIFdelbegin \DIFdel{$f_{NL}$ represents a constant }\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}$ represents a }\DIFaddend parameter measuring the amplitude of a specific bispectrum shape.

In this thesis\DIFaddbegin \DIFadd{, }\DIFaddend we follow the latter convention and associate one \DIFdelbegin \DIFdel{$f_{NL}$ }\DIFdelend \DIFaddbegin \DIFadd{scalar $f_\text{NL}$ }\DIFaddend with each of the bispectrum shapes. For example, \DIFdelbegin \DIFdel{$f_{NL}$ }\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}$ }\DIFaddend in \eqref{eqn:local_bispectrum_using_power_spectrum} is denoted \DIFdelbegin \DIFdel{$f_{NL}^{local}$}\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}^\textrm{local}$}\DIFaddend , tied to the local shape. \DIFdelbegin \DIFdel{Connection }\DIFdelend \DIFaddbegin \DIFadd{The connection }\DIFaddend between the theoretical predictions and observations is often made using \DIFdelbegin \DIFdel{analytic but approximate }\DIFdelend \DIFaddbegin \DIFadd{approximate analytic }\DIFaddend templates for the bispectrum.

We parametrise the power spectrum as $P_\zeta(k) = A_\zeta k^{n_s-4}$ using the power spectrum amplitude $A_\zeta$ and scalar spectral index $n_s$. The local \DIFaddbegin \DIFadd{bispectrum }\DIFaddend template is then \DIFdelbegin \DIFdel{given by
}\DIFdelend \DIFaddbegin \DIFadd{defined as 
}\DIFaddend \begin{align}
	B_\zeta\DIFdelbegin \DIFdel{^{local} }\DIFdelend \DIFaddbegin \DIFadd{^\textrm{local} }\DIFaddend (k_1, k_2, k_3) := 2A\DIFdelbegin \DIFdel{_\zeta}\DIFdelend ^2 \left[ \frac{1}{k_1^{4-n_s} k_2^{4-n_s}} + \text{\;2 cyc.} \right]\DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend \label{def:local_template} 
\end{align}
\DIFaddbegin \DIFadd{where $A^2 := (3/5) A_\zeta^2$. }\DIFaddend Note that we do not include \DIFdelbegin \DIFdel{$f_{NL}^{local}$ }\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}^\textrm{local}$ }\DIFaddend in our definition of the template either; it is set to \DIFdelbegin \DIFdel{$1$}\DIFdelend \DIFaddbegin \DIFadd{one}\DIFaddend .
...
Meanwhile, all the information about the interactions is captured in the time evolution operator \DIFdelbegin \DIFdel{$\hat{U_I}(t,t_0) = T\exp \left(-i\int_{t_0}^t H_I(t')dt'\right)$, where $H_I$ denotes $H_{int}$ }\DIFdelend \DIFaddbegin \DIFadd{$\hat{U_I}(t,t_0) = T\exp \left(-i\int_{t_0}^t H_\text{I}(t')dt'\right)$, where $H_\text{I}$ denotes $H_\text{int}$ }\DIFaddend in the interaction picture. The time-ordering operator $T$ arranges the operators so that the ones evaluated at earlier times appear on the right. The states at time $t$ are given by $|\psi_I(t)\rangle = U_I(t,t_0) |\psi_I(t_0)\rangle$.
...
\begin{align}
	\left< Q(t) \right> &:= \langle \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \textrm{\DIFadd{in}} \DIFaddend |\;Q(t)\;| \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \textrm{\DIFadd{in}} \DIFaddend \rangle \nonumber\\ &=  \langle\vv{0}| \left[ \bar{T} \exp \left( i \int_{t_0}^{t} H\DIFdelbegin \DIFdel{_I}\DIFdelend \DIFaddbegin \DIFadd{_\text{I}}\DIFaddend (t')dt \right) \right] Q_I(t) \left[ T \exp \left( -i \int_{t_0}^{t} H\DIFdelbegin \DIFdel{_I}\DIFdelend \DIFaddbegin \DIFadd{_\text{I}}\DIFaddend (t')dt \right) \right]  |\vv{0}\rangle. \label{eqn:in_in_formalism_expectation}
\end{align}
...
\end{align}
The normal ordering $::$ keeps $\hat{a}$s to the right and $\hat{a}^\dagger$ to the left \DIFdelbegin \DIFdel{, so that its }\DIFdelend \DIFaddbegin \DIFadd{so that the }\DIFaddend vacuum expectation value vanishes. The fields $\hat{O}$ are either $\hat{\zeta}$ or $\hat{\pi}$. 

\subsection{Bispectrum from \DIFdelbegin \DIFdel{single field }\DIFdelend \DIFaddbegin \DIFadd{single-field }\DIFaddend inflation} \label{section:bispectrum_from_single_field_inflation}
...
\end{align}
where $\tau$ is conformal time. The choice of $u_k(\tau)$ being the positive frequency solution as shown here is analogous to \DIFdelbegin %DIFDELCMD < \eqref{eqn:bunch_davis_mode_function} %%%
...
When working perturbatively, the leading contributions to the three-point correlation function comes from the terms with one factor of the interaction Hamiltonian. Noting that the two terms coming from the two \DIFdelbegin \DIFdel{$H_I$}\DIFdelend \DIFaddbegin \DIFadd{$H_\text{I}$}\DIFaddend s in \eqref{eqn:in_in_formalism_expectation} are complex conjugates to each other, we have
\begin{align}
	\left< \zeta_{\vv{k}_1}(t) \zeta_{\vv{k}_2}(t) \zeta_{\vv{k}_3}(t) \right> = 2 \text{Re}\left[ \langle\vv{0}| -i \zeta_{\vv{k}_1}(t) \zeta_{\vv{k}_2}(t) \zeta_{\vv{k}_3}(t) \int_{t_0}^{t} H\DIFdelbegin \DIFdel{_I}\DIFdelend \DIFaddbegin \DIFadd{_\text{I}}\DIFaddend (t') dt' |\vv{0}\rangle \right], \label{eqn:in_in_formalism_bispectrum}
\end{align}
in tree-level. This is the key formula for computing the primordial bispectrum from a given \DIFdelbegin \DIFdel{single field }\DIFdelend \DIFaddbegin \DIFadd{single-field }\DIFaddend inflation Lagrangian.

We will now demonstrate how to compute \eqref{eqn:in_in_formalism_bispectrum} using a simple example. We take the term proportional to $\dot\zeta^3$ within the cubic action \eqref{eqn:cubic_action_example_term}. To simplify further, we write it as \DIFdelbegin \DIFdel{$\lambda a^3 \dot{\zeta}^3$, where $\lambda=\lambda(t)$ }\DIFdelend \DIFaddbegin \DIFadd{$\mu a^3 \dot{\zeta}^3$, where $\mu=\mu(t)$ }\DIFaddend is given in terms of the variation parameters \DIFdelbegin \DIFdel{like }\DIFdelend \DIFaddbegin \DIFadd{such as $\lambda$ and }\DIFaddend $\epsilon$. The corresponding contribution to the interaction Hamiltonian is given by
\begin{align}
	H\DIFdelbegin \DIFdel{_{I,\dot{\zeta}^3}}\DIFdelend \DIFaddbegin \DIFadd{_{\text{I},\dot{\zeta}^3}}\DIFaddend (t) &:= -\DIFdelbegin \DIFdel{\lambda}\DIFdelend \DIFaddbegin \DIFadd{\mu}\DIFaddend (t) a(t)^3 \int d^3\vv{x} \; \left[  \dot{\zeta}(\vv{x},t)^3 \right]  \\
	&= -\DIFdelbegin \DIFdel{\lambda }\DIFdelend \DIFaddbegin \DIFadd{\mu }\DIFaddend a^3 \int d^3\vv{x} \; \frac{d^3\vv{p}_1}{(2\pi)^3} \frac{d^3\vv{p}_2}{(2\pi)^3} \frac{d^3\vv{p}_3}{(2\pi)^3} \; e^{i(\vv{p}_1 + \vv{p}_2 + \vv{p}_3)\cdot\vv{x}} \left[ \dot{\zeta}_{\vv{p}_1} \dot{\zeta}_{\vv{p}_2} \dot{\zeta}_{\vv{p}_3} \right] \\
	&= - (2\pi)^3 \delta^{(3)}(\vv{p}_1 + \vv{p}_2 + \vv{p}_3) \cdot \DIFdelbegin \DIFdel{\lambda }\DIFdelend \DIFaddbegin \DIFadd{\mu }\DIFaddend a^3 \int \frac{d^3\vv{p}_1}{(2\pi)^3} \frac{d^3\vv{p}_2}{(2\pi)^3} \frac{d^3\vv{p}_3}{(2\pi)^3} \; \left[ \dot{\zeta}_{\vv{p}_1} \dot{\zeta}_{\vv{p}_2} \dot{\zeta}_{\vv{p}_3} \right], \label{def:interaction_hamiltonian_zeta_dot_cube}
\end{align}
...
\begin{align}
	...
	&= \int d^3 \vv{r} \; e^{-i\vv{r} \cdot (\vv{k}_1 + \vv{k}_2 + \vv{k}_3)}  B(k_1, k_2, k_3)\DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend \label{eqn:bispectrum_derivation_delta_function}
\end{align}
...
\end{align}
\DIFaddbegin \DIFadd{There are no implied summations over indices throughout this section. }\DIFaddend Incorporating \eqref{eqn:bispectrum_derivation_base_form}, \eqref{eqn:bispectrum_derivation_delta_function}, and \eqref{eqn:bispectrum_derivation_rayleigh}, we obtain
\begin{align}
...
\end{align}
The form of \eqref{eqn:bispectrum_estimation_core} makes it clear that the bispectrum estimation is \DIFdelbegin \DIFdel{a }\DIFdelend linear regression in essence. Borrowing words from statistics, we specify the components of our analysis below.
\begin{itemize}
	\item \DIFdelbegin \DIFdel{$\vv{B}^{obs}$ }\DIFdelend \DIFaddbegin \DIFadd{$\vv{B}^\text{obs}$ }\DIFaddend is the \textit{regressand}, in our case the noisy observed bispectrum samples \DIFdelbegin \DIFdel{$B^{obs}_\vv{L}$ }\DIFdelend \DIFaddbegin \DIFadd{$B^\text{obs}_\vv{L}$ }\DIFaddend obtained for each \textit{observation} $\vv{L}$.
	\item $\vv{B}^{(i)}$s are the \textit{regressors}, theoretical bispectra $B^{(i)}_\vv{L}$ motivated from inflationary models.
	\item \DIFdelbegin \DIFdel{$f^{(i)}_{NL}$}\DIFdelend \DIFaddbegin \DIFadd{$f^{(i)}_\text{NL}$}\DIFaddend s are the \textit{regression coefficients} which parametrise how much each regressor contributes to the regressand. Our main objective is to estimate them.
	\item $\vv{\epsilon}$ is the \textit{error term} which represents the noise in \DIFaddbegin \DIFadd{the }\DIFaddend bispectrum sample compared to the true underlying value. $\epsilon_\vv{L}$ can be sourced by the sampling error (from having limited number of samples) and/or inaccuracies in the measurements. 
\end{itemize}

We adopt the least squares method to estimate \DIFdelbegin \DIFdel{$f_{NL}$}\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}$}\DIFaddend s from given \DIFdelbegin \DIFdel{$\vv{B}^{obs}$ }\DIFdelend \DIFaddbegin \DIFadd{$\vv{B}^\text{obs}$ }\DIFaddend and $B^{(i)}_\vv{L}$. Before doing so, each element of the regressand and regressors needs to be normalised so that the expected variances in the error are constant across different observations $\vv{L}$. We compute the theoretical variance in \DIFdelbegin \DIFdel{$B^{obs}_\vv{L}$ }\DIFdelend \DIFaddbegin \DIFadd{$B^\text{obs}_\vv{L}$ }\DIFaddend under two assumptions. First, we work in a weak non-Gaussian limit where the dominant contributions to correlation functions come from the Gaussian part of $a_\vv{l}$s. Wick's theorem then reduces the problem down to summing over all possible contractions. Second, we neglect inaccuracies in the $a_{lm}$ measurements \DIFdelbegin \DIFdel{, since their contributions to the }\DIFdelend \DIFaddbegin \DIFadd{as we are cosmic variance limited; the sampling error dominates the }\DIFaddend total error\DIFdelbegin \DIFdel{is much smaller compared to the sampling errors}\DIFdelend .

Under these assumptions, the expected covariance of the observed bispectra is given by
\begin{align}
	\left< B\DIFdelbegin \DIFdel{^{obs}}\DIFdelend \DIFaddbegin \DIFadd{^\text{obs}}\DIFaddend _\vv{L} B\DIFdelbegin \DIFdel{^{obs}}\DIFdelend \DIFaddbegin \DIFadd{^\text{obs}}\DIFaddend _{\vv{L}'} \right> &= \left< a_{\vv{l}_1} a_{\vv{l}_2} a_{\vv{l}_3} a_{\vv{l}'_1} a_{\vv{l}'_2} a_{\vv{l}'_3} \right> 
...
\end{align}
\DIFaddbegin \DIFadd{Here, }\DIFaddend \eqref{eqn:bispectrum_variance_contractions2} contains all possible contractions of the 6 $a_\vv{l}$s, 3 from each of \DIFdelbegin \DIFdel{$B^{obs}_\vv{L}$ and $B^{obs}_{\vv{L}'}$}\DIFdelend \DIFaddbegin \DIFadd{$B^\text{obs}_\vv{L}$ and $B^\text{obs}_{\vv{L}'}$}\DIFaddend . There are 15 such contractions: 6 consisting only of terms between the two bispectra (in $\ddagger$) and 9 which also have internal contractions ($\ddagger\ddagger$). Our aim is to modify the regression variables so that the resulting covariance matrix reduces to the identity. The error in each $\vv{L}$ is required to be independent and of unit variance.

The terms in ($\ddagger$) originate from \DIFdelbegin \DIFdel{a symmetry }\DIFdelend \DIFaddbegin \DIFadd{symmetries }\DIFaddend present in the bispectrum; \DIFdelbegin \DIFdel{$B^{obs}_\vv{L} = a_{\vv{l}_1}  a_{\vv{l}_2}  a_{\vv{l}_3}$ }\DIFdelend \DIFaddbegin \DIFadd{$B^\text{obs}_\vv{L} = a_{\vv{l}_1}  a_{\vv{l}_2}  a_{\vv{l}_3}$ }\DIFaddend is invariant under permutations of ${\vv{l}_1, \vv{l}_2, \vv{l}_3}$. In order to remove duplicate elements, we restrict our $\vv{L}$s to ones satisfying $\vv{l}_1 \le \vv{l}_2 \le \vv{l}_3$. \footnote{$(l_1,m_1) < (l_2,m_2)$ if and only if ($l_1 < l_2$) \textbf{or} ($l_1 = l_2$ and $m_1 < m_2$).}

On the other hand, terms in ($\ddagger\ddagger$) reveal a more complex issue about our formulation. The observed anisotropies $a_\vv{l}$ are not necessarily independent since various factors such as partial sky coverage and correlated noise can induce correlations between them. Even if they are independent, our construction of \DIFdelbegin \DIFdel{$B^{obs}_\vv{L}$ }\DIFdelend \DIFaddbegin \DIFadd{$B^\text{obs}_\vv{L}$ }\DIFaddend can yield non-zero terms in ($\ddagger\ddagger$) when at least two out of $\vv{l}_1, \vv{l}_2, \vv{l}_3$ are identical. This is similar, in essence, to incorrectly estimating the variance of a random variable $X$ from samples $X_i$ by computing $\sum_i X_i^2$ instead of the correct $\sum_i (X_i- \left< X \right>)^2$.

We redefine the observed bispectra in terms of the $a_\vv{l}$s by subtracting off these extra contributions.
\begin{align}
	{B'}\DIFdelbegin \DIFdel{^{obs}}\DIFdelend \DIFaddbegin \DIFadd{^\text{obs}}\DIFaddend _\vv{L} := a_{\vv{l}_1} a_{\vv{l}_2} a_{\vv{l}_3} - \left< a_{\vv{l}_1} a_{\vv{l}_2} \right> a_{\vv{l}_3} - \left< a_{\vv{l}_2} a_{\vv{l}_3} \right> a_{\vv{l}_1} - \left< a_{\vv{l}_3} a_{\vv{l}_1} \right> a_{\vv{l}_2}. \label{def:bispectrum_estimate_including_linear}
\end{align}
The newly introduced terms linear in $a_\vv{l}$s have zero mean \DIFdelbegin \DIFdel{, }\DIFdelend since $\left< a_\vv{l} \right> = 0$ except for the monopole $l=0$ which is excluded from the CMB bispectrum analysis. Thus \DIFdelbegin \DIFdel{${B'}^{obs}_\vv{L}$ }\DIFdelend \DIFaddbegin \DIFadd{${B'}^\text{obs}_\vv{L}$ }\DIFaddend is still an unbiased estimate of the underlying bispectra. Substituting into \eqref{eqn:bispectrum_variance_contractions1}, all terms that were previously in ($\ddagger\ddagger$) now vanish.

In theory, we can compute \eqref{eqn:bispectrum_variance_contractions1} using the full covariance matrix $C_{\vv{l}_1 \vv{l}_2} = \left< a_{\vv{l}_1} a_{\vv{l}_2} \right>$ and invert it to get the least squares estimate for \DIFdelbegin \DIFdel{$f_{NL}$}\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}$}\DIFaddend s. In practice, however, this is a costly and numerically demanding operation. \DIFaddbegin \footnote{\DIFadd{For Planck-like experiments, the matrix is about $2500^2 \times 2500^2$ in size and hence costly and numerically unstable to invert.}} \DIFaddend We instead approximate the non-diagonal covariances in \eqref{def:bispectrum_estimate_including_linear} using the Monte Carlo method: $\langle a_{\vv{l}_1}^{G} a_{\vv{l}_2}^{G} \rangle_{MC}$ from an ensemble of realistic Gaussian simulations. In other places\DIFaddbegin \DIFadd{, }\DIFaddend we assume $C_{\vv{l}_1 \vv{l}_2} \approx C_{l_1} \delta_{\vv{l}_1 \vv{l}_2}$ to simplify our calculations.
...
\begin{align}
	\hat{f}\DIFdelbegin \DIFdel{_{NL} }\DIFdelend \DIFaddbegin \DIFadd{_\text{NL} }\DIFaddend &= \frac{1}{N} \sum_{l_j,m_j} \frac{\mathcal{G}^{l_1 l_2 l_3}_{m_1 m_2 m_3} b_{l_1 l_2 l_3}}{C_{l_1} C_{l_2} C_{l_3}} \left[ a_{l_1 m_1} a_{l_2 m_2} a_{l_3 m_3} - 3\left< a_{l_1 m_1} a_{l_2 m_2} \right> a_{l_3 m_3} \right], \label{eqn:bispectrum_estimator_single_shape} \\
...
\end{align}
where the symmetry in indices was used to combine the linear terms into one. We also defined the normalisation factor $N$ to follow conventions in the literature. \DIFaddbegin \DIFadd{The theoretical error on $\hat{f}_\text{NL}$ is then $\sigma(\hat{f}_\text{NL}) = \sqrt{F^{-1}} = \sqrt{6/N}$.
}\DIFaddend 

\subsection{CMB bispectrum estimators} \DIFaddbegin \label{section:CMB_bispectrum_estimators}
\DIFaddend The estimator \eqref{eqn:bispectrum_estimator_single_shape} can be intimidating. The multipole moments $l$ can go up to $2500$ for the Planck survey \cite{PlanckCollaboration2013paramters}, meaning that the sum over all possible $l_j, m_j$ contains $\approx 2.7\times 10^{15}$ terms.\footnote{This number was calculated considering the symmetry ($l_1 \ge l_2 \ge l_3$), triangle inequality ($l_2+l_3 \ge l_1$), and restrictions on $m_j$s from angular momentum conservation ($m_1+m_2+m_3=0$). } Direct evaluation of the Gaunt integral $\mathcal{G}^{l_1 l_2 l_3}_{m_1 m_2 m_3}$ involves calculating Wigner 3-j symbols, which are expensive to compute and store. Not to mention that the number of terms scales as \DIFdelbegin \DIFdel{$\propto l_{max}^5$}\DIFdelend \DIFaddbegin \DIFadd{$\propto l_\text{max}^5$}\DIFaddend .
...
So far, we have been working on the late-time harmonic space directly related to the CMB observations. The theoretical bispectrum $B(k_1,k_2,k_3)$ predicted by inflation models, however, lies within the primordial Fourier space. The remaining work is about how to bridge this gap. In particular, we would like to obtain $\alpha^R$ from a given shape function $S(k_1,k_2,k_3) = (k_1 k_2 k_3)^2 B(k_1, k_2, k_3)$ so that we can get an \DIFdelbegin \DIFdel{$f_{NL}$ }\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}$ }\DIFaddend estimate for $S(k_1,k_2,k_3)$.

The primordial and late-time spaces are \DIFdelbegin \DIFdel{treated symmetrically }\DIFdelend \DIFaddbegin \DIFadd{formulated similarly }\DIFaddend in the Modal \DIFdelbegin \DIFdel{formalism}\DIFdelend \DIFaddbegin \DIFadd{approach}\DIFaddend . We expand the shape function using an independent set of \textit{primordial} basis functions;
\begin{align}
...
\begin{align}
	\bar{\gamma}_{n_1 n_2} &:=  \left< \bar{\vv{Q}}_{n_1} , \bar{\vv{Q}}_{n_2} \right>\DIFaddbegin \DIFadd{, }\DIFaddend \\
	\bar{\gamma}^{-1} &= \bar{\lambda} \bar{\lambda}^T\DIFaddbegin \DIFadd{, }\DIFaddend \\
	\bar{\vv{R}}_t &:= \sum_n \bar{\lambda}_{nt} \; \bar{\vv{Q}}_{n}\DIFaddbegin \DIFadd{, }\DIFaddend \\
	\bar{\alpha}^{\bar{R}}_t &= \left< S(k_1,k_2,k_3) \;,\; \bar{\vv{R}}_t \right>\DIFaddbegin \DIFadd{, }\DIFaddend \\
...
The binned bispectrum estimator, introduced in \cite{Bucher2010} and used for Planck analyses \cite{PlanckCollaboration2013,PlanckCollaboration2015,PlanckCollaboration2018,Bucher2016}, uses carefully chosen bins to reduce the computational complexity. In this section\DIFaddbegin \DIFadd{, }\DIFaddend we highlight the main concepts of \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{its }\DIFaddend formalism while relabelling some notations in the original literature \cite{Bucher2010} in order to be consistent with our previous discussions.
...
\end{align}
They correspond to the $l$-multipole component of the original map. We define the observed bispectrum \DIFdelbegin \DIFdel{$B^{obs}_{l_1 l_2 l_3}$ }\DIFdelend \DIFaddbegin \DIFadd{$B^\text{obs}_{l_1 l_2 l_3}$ }\DIFaddend to be the integrand of \eqref{eqn:binned_estimator_formulation_1}.\footnote{This is the convention from the original literature \cite{Bucher2010}, which is different to some conventions elsewhere by a factor of $h_{l_1 l_2 l_3}$. In terms of the theoretical reduced bispectrum $b_{l_1 l_2 l_3}$, we have \DIFdelbegin \DIFdel{$\langle B^{obs}_{l_1 l_2 l_3} \rangle = h^2_{l_1 l_2 l_3} b^{th}_{l_1 l_2 l_3}$}\DIFdelend \DIFaddbegin \DIFadd{$\langle B^\text{obs}_{l_1 l_2 l_3} \rangle = h^2_{l_1 l_2 l_3} b^\text{th}_{l_1 l_2 l_3}$}\DIFaddend .}
...
\begin{align}
	\Theta\DIFdelbegin \DIFdel{_{ISW}}\DIFdelend \DIFaddbegin \DIFadd{_\text{ISW}}\DIFaddend (\eta_0,\vv{x}_0, \hat{\vv{n}}) = 2\int_0^{\chi_*} d\chi \; \Psi'_W (\chi\hat{\vv{n}}; \eta_0-\chi). \label{eqn:ISW_effect}
\end{align}
The integrand is small except during the accelerated expansion phase at low redshift, again $z\lessapprox2$. The two integrals \eqref{def:lensing_potential} and \eqref{eqn:ISW_effect} are highly correlated through \DIFdelbegin \DIFdel{$\Psi_W$ }\DIFdelend \DIFaddbegin \DIFadd{$\Psi_\text{W}$ }\DIFaddend and thus generate a non-trivial bispectrum, mainly in the squeezed limit. This lensing-ISW bispectrum is well approximated by the template given by \cite{Lewis2011lensing};
\begin{align}
	b\DIFdelbegin \DIFdel{^{lens}}\DIFdelend \DIFaddbegin \DIFadd{^\text{lens}}\DIFaddend _{l_1 l_2 l_3} = \frac{1}{2}\left[ l_1 (l_1 + 1) - l_2 (l_2 + 1) + l_3 (l_3 + 1) \right] \tilde{C}_{l_1}^{TT} C_{l_3}^{T\psi} + \text{5 perms.}, \label{eqn:lensing_bispectrum_template}
\end{align}
where $\tilde{C}^{TT}_l$ is the \textit{lensed} angular power spectrum and $C^{T\psi}_l$ is the cross power spectrum between temperature ($T$) and $\psi$. Assuming the standard $\Lambda$CDM cosmology we expect to find the bispectrum above in the CMB without any additional factors. We can measure its amplitude in the CMB data through the usual bispectrum estimator \eqref{eqn:bispectrum_estimator_single_shape}-\eqref{eqn:bispectrum_estimator_normalisation_single_shape} by setting \DIFdelbegin \DIFdel{$\vv{b} = \vv{b}^{lens}$ so that $\hat{f}^{lens}_{NL} = \langle \vv{b}^{lens}, \; \vv{b}^{obs} \rangle / \langle \vv{b}^{lens}, \; \vv{b}^{lens} \rangle$}\DIFdelend \DIFaddbegin \DIFadd{$\vv{b} = \vv{b}^\text{lens}$ so that $\hat{f}^\text{lens}_\text{NL} = \langle \vv{b}^\text{lens}, \; \vv{b}^\text{obs} \rangle / \langle \vv{b}^\text{lens}, \; \vv{b}^\text{lens} \rangle$}\DIFaddend , where we used the inner product $\langle \cdot,\cdot \rangle$ from \eqref{def:bispectrum_estimation_fisher_inner_product}. This value has been \DIFdelbegin \DIFdel{first detected }\DIFdelend \DIFaddbegin \DIFadd{estimated }\DIFaddend in the Planck 2013 analysis to \DIFaddbegin \DIFadd{a }\DIFaddend $2.5\sigma$ level \cite{PlanckCollaboration2013ISW}. The most recent Planck analysis estimates \DIFdelbegin \DIFdel{$f^{lens}_{NL} = 0.73 \pm 0.27$ }\DIFdelend \DIFaddbegin \DIFadd{$f^\text{lens}_\text{NL} = 0.73 \pm 0.27$ }\DIFaddend (68\% CL) using the SMICA foreground-cleaned CMB temperature maps \cite{PlanckCollaboration2018}.

The lensing-ISW bispectrum is an observable \DIFdelbegin \DIFdel{caused by }\DIFdelend \DIFaddbegin \DIFadd{originating from }\DIFaddend the non-linear evolution of the CMB. When studying primordial contributions to the observed bispectrum, however, such late-time effects need to be subtracted. The lensing-ISW \textit{bias} is given by
\begin{align}
	\Delta f\DIFdelbegin \DIFdel{_{NL}}\DIFdelend \DIFaddbegin \DIFadd{_\text{NL}}\DIFaddend ^{(t)} = \DIFdelbegin \DIFdel{\frac{ \langle \vv{b}^{(t)}, \; \vv{b}^{lens} \rangle }{ \langle \vv{b}^{(t)}, \; \vv{b}^{(t)} \rangle} }\DIFdelend \DIFaddbegin \DIFadd{\frac{ \langle \vv{b}^{(t)}, \; \vv{b}^\text{lens} \rangle }{ \langle \vv{b}^{(t)}, \; \vv{b}^{(t)} \rangle} }\DIFaddend \label{eqn:lensing_ISW_bias}
\end{align}
for the bispectrum template $\vv{b}^{(t)}$ of interest. This bias can be significant for templates with \DIFaddbegin \DIFadd{a }\DIFaddend large squeezed limit, such as the local template, since they are highly correlated with the lensing-ISW bispectrum \eqref{eqn:lensing_bispectrum_template}.

\DIFaddbegin \DIFadd{\hspace{10pt}
}

\DIFadd{Note that CMB lensing also affects the bispectrum estimation by increasing the estimator's covariance \mbox{%DIFAUXCMD
\cite{Coulton2020bispectrumcovariance}}\hspace{0pt}%DIFAUXCMD
...
\DIFaddend \subsubsection*{Sky mask and inpainting}
...
\iffalse
We present forecasts on the primordial non-Gaussianity parameter $f_\text{NL}$ of feature models for \DIFdelbegin \DIFdel{the }\DIFdelend future Cosmic Microwave Background \DIFdelbegin \DIFdel{Stage-4 (CMB-S4) }\DIFdelend experiments. The Fisher matrix of the bispectrum estimator was computed using noise covariances \DIFdelbegin \DIFdel{expected for preliminary CMB-S4 specifications including ones }\DIFdelend \DIFaddbegin \DIFadd{from preliminary specifications }\DIFaddend for the Simons Observatory \DIFaddbegin \DIFadd{and the CMB Stage-4 (CMB-S4) experiment}\DIFaddend . We introduce a novel method that improves the computation by orthonormalising the covariance matrix. The most sensitive CMB-S4 \DIFdelbegin \DIFdel{experiment }\DIFdelend \DIFaddbegin \DIFadd{specification }\DIFaddend with 1' beam and 1$\mu K$-arcmin noise would yield a factor of 1.7-2.2 times more stringent constraints compared to Planck. Under the Simons Observatory baseline conditions\DIFaddbegin \DIFadd{, }\DIFaddend the improvement would be about 1.3-1.6 times to Planck. We also thoroughly studied the effects of various model and experimental parameters on the forecast. Detailed analysis \DIFdelbegin \DIFdel{on }\DIFdelend \DIFaddbegin \DIFadd{of }\DIFaddend the constraints coming from temperature and E-mode polarisation, in particular, provided some insight into detecting oscillatory features in the CMB bispectrum.
\fi
...
\DIFdelbegin \DIFdel{The next generation of CMB experiments, }\DIFdelend CMB Stage-4 \DIFdelbegin \DIFdel{, consists of many exciting proposed experiments }\DIFdelend \DIFaddbegin \DIFadd{(CMB-S4) is the next-generation CMB experiment }\DIFaddend located at the South Pole \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{and in }\DIFaddend the Atacama Desert in Chile \DIFdelbegin \DIFdel{, and perhaps space \mbox{%DIFAUXCMD
\cite{Abazajian2016,TheSimonsObservatoryCollaboration2018,TheCOrEcollaboration2015}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Abazajian2016}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . One of the main goals of \DIFdelbegin \DIFdel{these experiments }\DIFdelend \DIFaddbegin \DIFadd{CMB-S4 }\DIFaddend is to measure the polarisation signal in the CMB to the cosmic variance limit. In the Planck analysis, constraints on oscillatory models benefited much more from the inclusion of E-mode polarisation data compared to others, and therefore are anticipated to improve significantly with the future CMB experiments. Preliminary specifications have been released for these experiments \cite{Abazajian2016,TheSimonsObservatoryCollaboration2018} and have been used to produce some forecasts for the standard $f_\text{NL}$ templates, but not yet for feature type models. Here we address this by presenting the Fisher forecasts on \DIFdelbegin \DIFdel{$f_{NL}$ }\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}$ }\DIFaddend for feature models based on these specifications and observe that the feature type models do indeed receive larger improvements from the extra polarisation information than the standard templates.

This chapter is organised as follows. First\DIFaddbegin \DIFadd{, }\DIFaddend we briefly review the theory of the CMB bispectrum in Section \ref{section: feature model bispectrum}, building on our previous formulation to include polarisation. A bispectrum template for the feature model is defined and computed here. In Section \ref{section: Efficient computation of KSW estimator with polarisation} we formulate the bispectrum estimator and introduce methods to further optimise its computation. The technique is applied to the case of feature models to obtain equations for the Fisher forecast of \DIFdelbegin \DIFdel{$f_{NL}$}\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}$}\DIFaddend . The implementation details are briefly discussed. In Section \ref{section: CMB-S4 forecast results} we present the results of our forecast and how they depend on the model and experimental parameters. In particular, forecasts for the Simons Observatory are compared with the Planck results. We provide a summary at the end. The materials presented in this chapter are from my original research supervised by James Fergusson and published in \cite{Sohn2019}.


\section{Feature model bispectrum} \label{section: feature model bispectrum}
\subsection{CMB bispectrum}

Recall that the \DIFdelbegin \DIFdel{3-point }\DIFdelend \DIFaddbegin \DIFadd{three-point }\DIFaddend correlation function of the primordial perturbations is defined as
\begin{equation}
...
\end{equation}
under assumptions of the statistical homogeneity and isotropy. The primordial bispectrum $B_\Phi$ vanishes for Gaussian perturbations, but more general inflation models predict non-zero bispectra with various shapes.  In order to constrain these models we re-parameterise the bispectrum into \DIFdelbegin \DIFdel{a amplitude parameter $f_{NL}$ }\DIFdelend \DIFaddbegin \DIFadd{an amplitude parameter $f_\text{NL}$ }\DIFaddend and a normalised shape part. Constraining \DIFdelbegin \DIFdel{$f_{NL}$ }\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}$ }\DIFaddend from the CMB measurements allows to determine how well a particular shape under consideration aligns with the data, which we translate into constraints on the model itself.

In order to compare the theory with measurements\DIFaddbegin \DIFadd{, }\DIFaddend we first need to relate the primordial perturbations to spherical multipole modes of the late-time CMB anisotropies. The form of \eqref{eqn:alm_from_phi} derived previously generalises to include the CMB polarisation as well;
\begin{equation}
...
Note that \DIFdelbegin \DIFdel{most }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend CMB-S4 \DIFdelbegin \DIFdel{experiments are }\DIFdelend \DIFaddbegin \DIFadd{experiment is }\DIFaddend ground-based and inevitably probe smaller fractions of the sky compared to satellite experiments like Planck. Having a smaller fraction of the sky available leads to increased uncertainties for the estimator. \DIFdelbegin \DIFdel{Current }\DIFdelend \DIFaddbegin \DIFadd{The current }\DIFaddend estimate is that many of the new experiments will cover approximately 40\% of the sky, which is much less than the 74\% of Planck. The error bars will thus increase by a factor of 1.38 from the decrease in $f_\text{sky}$ alone.  This loss in constraining power can be ameliorated by adding the Planck data from the pixels unobserved by these experiments.

\subsection{Orthonormalising the covariance matrix}

In \cite{Fergusson2014} it was noted that orthogonalising the multipoles of temperature and polarisation maps dramatically reduces the number of terms \DIFdelbegin \DIFdel{in computation of the Modal estimators}\DIFdelend \DIFaddbegin \DIFadd{required for computing the Modal estimator}\DIFaddend . This technique can also be applied to KSW estimators, or indeed any optimal bispectrum estimator, which is yet to be done to the authors' knowledge.

In both (\ref{Estimator S definition}) and (\ref{Estimator F definition}) there are summations over indices $X$ and $X'$ to account for correlations between the CMB temperature and E-mode polarisation. This can be simplified by essentially making a change of basis in $X$ space for each $l$ so that every $C_l$ becomes orthonormal. Perform a Cholesky decomposition on $C_l$ and invert the matrix. Then \DIFdelbegin \DIFdel{$C_l^{-1} = L_l^T L_l$}\DIFdelend \DIFaddbegin \DIFadd{${C_l^{-1} = L_l^T L_l}$}\DIFaddend , where $L_l$ is a lower triangular matrix given by
\begin{equation}
	L_l = \DIFdelbegin %DIFDELCMD < \begin{pmatrix} \frac{1}{\sqrt{C_l^{TT}}} & 0  \\ \frac{- C_l^{TE}}{\sqrt{C_l^{TT}} \sqrt{C_l^{TT} C_l^{EE} - C_l^{TE^2}}}  &  \frac{ C_l^{TT}}{\sqrt{C_l^TT} \sqrt{C_l^{TT} C_l^{EE} - C_l^{TE^2}}} \end{pmatrix}%%%
\DIFdelend \DIFaddbegin \begin{pmatrix} \frac{1}{\sqrt{C_l^{TT}}} & 0  \\ \frac{- C_l^{TE}}{\sqrt{C_l^{TT}} \sqrt{C_l^{TT} C_l^{EE} - C_l^{TE^2}}}  &  \frac{ C_l^{TT}}{\sqrt{C_l^{TT}} \sqrt{C_l^{TT} C_l^{EE} - C_l^{TE^2}}} \end{pmatrix}\DIFaddend 
\end{equation}
\DIFaddbegin 
...
For the Planck component-separated maps, the beam FWHM equals 5 arcmin and the noise level of ~47 $\mu K \cdot $arcmin gives a good approximation to the noise covariances. For CMB-S4 \DIFdelbegin \DIFdel{experiments }\DIFdelend the details are not confirmed, but the beam FWHM is expected to lie between 1-5 arcmin, while the noise level will range from 1 to 9 $\mu K \cdot $arcmin. \cite{Abazajian2016}

In reality\DIFdelbegin \DIFdel{there exist extra contaminations }\DIFdelend \DIFaddbegin \DIFadd{, there exists extra contamination }\DIFaddend in large angular scales due to the $1/f$ noise. Though most of our analysis assumes \DIFaddbegin \DIFadd{a }\DIFaddend simpler form of \DIFaddbegin \DIFadd{the }\DIFaddend noise covariances elaborated above, for the Simons Observatory forecasts we follow \cite{TheSimonsObservatoryCollaboration2018} and model the $1/f$ noise as \DIFdelbegin %DIFDELCMD < {%%%
\DIFdel{$N_l = N_{red} (l/l_{knee})^{\alpha_{knee}} + N_{white}$}%DIFDELCMD < }%%%
\DIFdelend \DIFaddbegin \newline \DIFadd{${N_l = N_\text{red} (l/l_\text{knee})^{\alpha_\text{knee}} + N_\text{white}}$}\DIFaddend . The noise curves from each channel were then put together using the inverse variance method. This is a good approximation for the E-mode polarisation but not for temperature, since extra degradations occur during the component separation process. Still, this would be a reasonable approximation for our forecast since the dominant contributions to the feature model signal come from the polarisation data. For Planck, the full post-component-separation noise curves are available and were used for our analysis. 



\subsection{Implementation and validation}

We implemented the pipeline outlined above using the C programming language and parallelised using hybrid MPI + openMP \DIFaddbegin \DIFadd{to fully benefit from modern computing architecture}\DIFaddend . The code was then run in the COSMOS super-computing system.

The transfer functions are generated from the CAMB code \cite{Lewis2000}. The spherical \DIFdelbegin \DIFdel{bessel }\DIFdelend \DIFaddbegin \DIFadd{Bessel }\DIFaddend function values were pre-computed using recursion relations and stored, while the Legendre function values were computed on the fly using the GNU scientific library. We computed the angular power spectrum data from the $\Lambda$CDM parameters estimated in the Planck 2015 results. 

Numerical \DIFdelbegin \DIFdel{integration for }\DIFdelend \DIFaddbegin \DIFadd{integrations for the }\DIFaddend variables $k$, $r$ and $r'$ were performed using \DIFdelbegin \DIFdel{simple trapezoidal methods}\DIFdelend \DIFaddbegin \DIFadd{a simple trapezoidal method}\DIFaddend , which were found \DIFdelbegin \DIFdel{do }\DIFdelend \DIFaddbegin \DIFadd{to }\DIFaddend be accurate and easier to vectorise for optimisation purposes. On the other hand, the integration of $\mu$ required more care because the Legendre polynomials are highly oscillatory. We adopted the Gauss-Legendre quadrature rule with $1.5 \,l_\text{max}+1$ points which can integrate polynomials up to order $3\,l_\text{max}$ exactly. The weights and nodes for the quadrature were computed using the QUADPTS code \cite{Hale2013}.

We performed various checks to ensure that the code runs correctly. First we used the code to reproduce the Planck results \DIFaddbegin \DIFadd{on the error bars for feature models}\DIFaddend , which agreed within 3\% \DIFdelbegin \DIFdel{error}\DIFdelend \DIFaddbegin \DIFadd{level. The error mainly comes from the difference in Modal and KSW approaches for representing primordial bispectra. We discuss this further in Chapter \ref{chapter:high_resolution_cmb_bispectrum_estimator}}\DIFaddend . The code was then used to compute the bispectrum of the constant model which corresponds to the feature model template with $\omega=\phi=0$. There exists an approximate analytic form in this case \cite{Fergusson2012} which we were able to reproduce accurately. We also performed convergence tests on $r$ and $r'$ integration by doubling the number of points for each of them. The grid was chosen to be very dense around recombination and quite dense near reionisation. We confirmed that changes in the integral are kept less than 0.5\% for each value of $\omega$.


\section{CMB-S4 forecast results} \label{section: CMB-S4 forecast results}
\subsection{Phase dependence}

We now present the CMB-S4 forecast on the \DIFdelbegin \DIFdel{$f_{NL}$ }\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}$ }\DIFaddend estimation power for feature models. For notational convenience, we denote the error bars for \DIFdelbegin \DIFdel{$f_{NL}$ }\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}$ }\DIFaddend of sine and cosine feature models by $\sigma_{\sin}$ and $\sigma_{\cos}$. The superscripts $T$ and $T+E$ are also put to distinguish temperature-only analyses from the full polarisation ones.
...
\subsection{\DIFdelbegin \DIFdel{$l_{max}$ }\DIFdelend \DIFaddbegin \DIFadd{$l_\text{max}$ }\DIFaddend dependence}

Figure \ref{forecast lmax dependence} shows the graph of \DIFaddbegin \DIFadd{the }\DIFaddend forecast error bar $\sigma_{\sin}^{T+E}$ as we increase \DIFdelbegin \DIFdel{$l_{max}$}\DIFdelend \DIFaddbegin \DIFadd{$l_\text{max}$}\DIFaddend . The forecasts were done within angular scale range \DIFdelbegin \DIFdel{$2\le l \le l_{max}$}\DIFdelend \DIFaddbegin \DIFadd{$2\le l \le l_\text{max}$}\DIFaddend , the oscillation frequency $\omega$ set to 100, and assuming 1' beam and 1$\mu K\cdot$arcmin noise. The Planck noise curves were approximated by ones for 5' beam and 47 $\mu K\cdot$arcmin noise for this plot only \DIFdelbegin \DIFdel{, }\DIFdelend since we extend \DIFdelbegin \DIFdel{$l_{max}$ }\DIFdelend \DIFaddbegin \DIFadd{$l_\text{max}$ }\DIFaddend to 4000 here. 

The Planck error bar essentially stalls out when \DIFdelbegin \DIFdel{$l_{max}$ }\DIFdelend \DIFaddbegin \DIFadd{$l_\text{max}$ }\DIFaddend reaches 2000. The forecast error bar, on the other hand, keeps decreasing until \DIFdelbegin \DIFdel{$l_{max}=4000$ }\DIFdelend \DIFaddbegin \DIFadd{$l_\text{max}=4000$ }\DIFaddend thanks to the improved sensitivity in measuring small scales (and large $l$'s). Despite the information loss due to smaller sky coverage \DIFdelbegin \DIFdel{$f_{sky}$}\DIFdelend \DIFaddbegin \DIFadd{$f_\text{sky}$}\DIFaddend , the forecast error bar reduces to about 42\% of Planck by \DIFdelbegin \DIFdel{$l_{max}=4000$}\DIFdelend \DIFaddbegin \DIFadd{$l_\text{max}=4000$}\DIFaddend . This corresponds to a factor of 2.4 times improvement to measurement precision on $f_\text{NL}$.

\begin{figure}[ht]
	\centering
	\includegraphics[width=0.5\textwidth]{lmax_dependence.pdf}
	\caption{Forecast error bars $\sigma_{\sin}^{T+E}$ when multipoles \DIFdelbeginFL \DIFdelFL{$2\le l \le l_{max}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$2\le l \le l_\text{max}$ }\DIFaddendFL are included, in comparison with Planck. The oscillation frequency $\omega$ is set to 100 Mpc in all cases. Planck did not have access to the information from modes $l\ge 2000$, but \DIFdelbeginFL \DIFdelFL{the }\DIFdelendFL CMB-S4 \DIFdelbeginFL \DIFdelFL{experiments are }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{is }\DIFaddendFL expected to be able to explore modes up to $l=4000$.}
	\label{forecast lmax dependence}
\end{figure}

\subsection{Beam and noise dependence}

Now we explore the effects of different beam \DIFdelbegin \DIFdel{width }\DIFdelend \DIFaddbegin \DIFadd{widths }\DIFaddend and noise levels on the forecast error bars. Figure \ref{forecast beam and noise dependence} shows forecast $\sigma_{\sin}^{T+E}$ for ranges of beam and noise levels. Their oscillation frequencies are also varied, but only two representatives $\omega=20$ and $2000$ are chosen here. Forecasts for the other values of $\omega$ also display similar dependences on beam \DIFdelbegin \DIFdel{width }\DIFdelend \DIFaddbegin \DIFadd{widths }\DIFaddend and noise levels.

First of all, note that all estimated error bars in the plot are smaller than Planck, for which $\sigma_{\sin}^{T+E}=34$ when $\omega=20$ and $\sigma_{\sin}^{T+E}=610$ when $\omega=2000$. In fact\DIFaddbegin \DIFadd{, }\DIFaddend even the least sensitive CMB-S4 specification of 5' beam and 9$\mu K\cdot$arcmin noise is expected to put better bounds on feature models. \DIFaddbegin \DIFadd{Note that $l_\text{max}=4000$ for both $T$ and $E$ here.
}\DIFaddend 
...
We present the main results of the forecast. Figure \ref{forecast omega dependence pol} summarises the $\sigma_{\sin}$ forecasts for several different CMB-S4 preliminary specifications, including the Simons Observatory (SO) baseline and goal. Note that the \DIFdelbegin \DIFdel{1/f noise effects are }\DIFdelend \DIFaddbegin \DIFadd{$1/f$ noise effect is }\DIFaddend incorporated in SO forecasts but not in other ones. We also provide 1$\sigma$ errors for joint estimators, for which Planck signals from the fraction of the sky not covered by CMB-S4 are combined via \DIFaddbegin \DIFadd{a simple inverse variance method; }\DIFaddend $\sigma_\text{joint}^{-2} = \sigma_\text{CMB-S4}^{-2} +  \sigma_\text{Planck}^{-2}$. This \DIFdelbegin \DIFdel{method }\DIFdelend is not statistically optimal but \DIFdelbegin \DIFdel{sufficient to give an idea }\DIFdelend \DIFaddbegin \DIFadd{serves as a conservative estimate }\DIFaddend of the joint estimation power.

The most sensitive setup with 1' beam and 1$\mu K \cdot$arcmin noise would yield error bars that are 47-62\% of Planck. These correspond to a factor of 1.6-2.1 improvements. Here relatively smaller improvements are made for high oscillation frequencies. They correspond to smaller momentum scales $k_\ast = 2\pi/3\omega$, or larger angular scales, which benefit less from the increased sensitivity of CMB-S4\DIFdelbegin \DIFdel{experiments}\DIFdelend . When the results are combined with Planck, the error bar reduces to 45-57\% of Planck, or a factor of 1.7-2.2 improvement.

Forecast error bars from the SO baseline specification and the more ambitious one \DIFaddbegin \DIFadd{(`goal') }\DIFaddend do not differ very much. Quoting in terms of the baseline values, $\sigma_{\sin}$ lies about 68-86\% of that of Planck or equivalently, 1.2-1.5 times smaller than Planck. Numbers change to 62-74\% when combined with Planck, so \DIFdelbegin \DIFdel{that }\DIFdelend the overall improvement ratio is about 1.3-1.6.
...
\subsection{Comparison to \DIFdelbegin \DIFdel{scale invariant }\DIFdelend \DIFaddbegin \DIFadd{scale-invariant }\DIFaddend models}

Our pipeline for forecasting \DIFdelbegin \DIFdel{$f_{NL}^\text{feat}$ }\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}^\text{feat}$ }\DIFaddend also yields forecasts for \DIFdelbegin \DIFdel{$f_{NL}$ }\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}$ }\DIFaddend of the constant model\DIFdelbegin \DIFdel{. Constant models are scale invariant and have trivial shape, so that }\DIFdelend \DIFaddbegin \DIFadd{, which is scale-invariant and have a trivial shape; }\DIFaddend $B(k_1,k_2,k_3) \propto (k_1 k_2 k_3)^{-2}$.
Forecasts on \DIFdelbegin \DIFdel{$f_{NL}^\text{const}$ }\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}^\text{const}$ }\DIFaddend follow from our pipeline by simply setting the oscillation frequency $\omega = 0$ and phase $\phi = \pi/2$. Table \ref{forecast constant model} summarises the forecast results for several different CMB-S4 specifications mentioned before, using both T and E data and in combination with Planck data from the regions of the sky not covered by CMB-S4. For the 1' beam and 1$\mu K \cdot$arcmin noise setup, the error bar is expected to be reduced by a factor of 2.3 compared to Planck.

\begin{table}[ht]
	\caption{Forecasts on the estimation errors of \DIFdelbeginFL \DIFdelFL{$f_{NL}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$f_\text{NL}$ }\DIFaddendFL for the constant model}
	\centering
...
The latest Planck constraints on $f_\text{NL}$ of some popular bispectrum templates are given by $f_\text{NL}^\text{local} = 2.5 \pm 5.7$, $f_\text{NL}^\text{equil} = -16 \pm 70$, and $f_\text{NL}^\text{ortho} = -34 \pm 33$ \cite{PlanckCollaboration2015}. CMB-S4 \DIFdelbegin \DIFdel{experiments are }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend expected to yield better estimates on these as well. Table \ref{forecast various models} summarises the forecast improvement ratio given in \cite{Abazajian2016} together with the constant and feature model ratios computed in this work.
...
Surprisingly, the estimation error for feature models does not improve as much as other templates. Feature models benefit much more from \DIFdelbegin \DIFdel{inclusion of the }\DIFdelend \DIFaddbegin \DIFadd{the inclusion of }\DIFaddend polarisation data than any other \DIFdelbegin \DIFdel{scale independent }\DIFdelend \DIFaddbegin \DIFadd{scale-independent }\DIFaddend shapes; for example, $\sigma^{T}/\sigma^{T+E} = 4.6$ for the feature models with $\omega=200$ in CMB-S4 while the value equals 2.8 for the constant models. Because \DIFdelbegin \DIFdel{the }\DIFdelend CMB-S4 \DIFdelbegin \DIFdel{experiments }\DIFdelend would have a significantly enhanced polarisation measurement sensitivity, we originally expected the feature models to be constrained significantly better than Planck.
...
\begin{table}[ht]
	\caption{Expected improvements on the estimation errors of $f_\text{NL}$ for each combination of Planck / CMB-S4 temperature (T) and polarisation (E) data. Here the CMB-S4 assumes 1' beam and 1$\mu K$arcmin noise. \DIFdelbeginFL \DIFdelFL{For }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{The }\DIFaddendFL feature model \DIFdelbeginFL \DIFdelFL{the }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{has }\DIFaddendFL oscillation frequency $\omega=200$ and phase $\phi=0$. The sky fraction \DIFdelbeginFL \DIFdelFL{$f_{sky}=0.4$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$f_\text{sky}=0.4$ }\DIFaddendFL for all cases except for Planck T + Planck E, for which \DIFdelbeginFL \DIFdelFL{$f_{sky}=0.76$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$f_\text{sky}=0.76$}\DIFaddendFL .}
	\centering
	...
	\begin{tabular}{c c c c c c c c}
		...
		& CMB-S4 & 1.1 & 2.2 & & CMB-S4 & 0.9 & \DIFdelbeginFL \DIFdelFL{1.9 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{2.0 }\DIFaddendFL \\
		\bottomrule
	\end{tabular}
\end{table}
...
On the other hand, adapted KSW-type estimators \cite{Munchmeyer2014,Munchmeyer2015resonance,Meerburg2016jointResonance} may probe models with much faster oscillations in their bispectra. They exploit \DIFaddbegin \DIFadd{the }\DIFaddend separability of specific oscillatory templates and/or Fourier-type bases to obtain accurate constraints \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{on }\DIFaddend a model-by-model basis. However, these methods are not easily applicable for models with more complicated bispectra, especially the ones that do not depend solely on $K=k_1+k_2+k_3$. The formalism is also \DIFdelbegin \DIFdel{specialised }\DIFdelend \DIFaddbegin \DIFadd{restricted }\DIFaddend to certain types of bispectra and requires a separate implementation for each case. This was one of the main motivations behind the research project; can we have a bispectrum estimator which has the efficiency and versatility of Modal, whilst also benefiting from the precision of the KSW formalism for a more specialised analysis?

We developed a novel CMB bispectrum estimation pipeline \DIFdelbegin \DIFdel{CMB-BEst. }\DIFdelend \DIFaddbegin \textsc{\DIFadd{CMB-BEst}}\DIFadd{. }\DIFaddend \footnote{Short for \underline{CMB} \underline{B}ispectrum \underline{Est}imator. The goal of \DIFdelbegin \DIFdel{CMB-BEst }\DIFdelend \DIFaddbegin \textsc{\DIFadd{CMB-BEst}} \DIFaddend is to be the best bispectrum estimator with wide applications and great precision.} \DIFdelbegin \DIFdel{CMB-BEst }\DIFdelend \DIFaddbegin \textsc{\DIFadd{CMB-BEst}} \DIFaddend has two main strengths over former methods. First of all, it is implemented for completely general basis sets, which allows broad analyses \DIFdelbegin \DIFdel{on }\DIFdelend \DIFaddbegin \DIFadd{of }\DIFaddend inflationary models. In fact, the conventional KSW estimator is equivalent to a specific choice of basis in \DIFdelbegin \textit{\DIFdel{CMB-BEst}}%DIFAUXCMD
\DIFdel{. CMB-BEst }\DIFdelend \DIFaddbegin \textsc{\DIFadd{CMB-BEst}}\DIFadd{. }\textsc{\DIFadd{CMB-BEst}} \DIFaddend can cover any model KSW estimator can, and furthermore, it can simultaneously constrain general bispectrum shapes through primordial mode expansion, just like Modal.

Secondly, \DIFdelbegin \DIFdel{CMB-BEst }\DIFdelend \DIFaddbegin \textsc{\DIFadd{CMB-BEst}} \DIFaddend is designed to be able to handle complex and highly oscillatory signals. It does not require a separate set of basis functions for late-time $l$ space, \DIFdelbegin \DIFdel{in }\DIFdelend contrary to Modal. More diverse and specialised choices of basis can therefore be made. The code will be used on numerous inflation models with complex oscillations which are yet to be investigated due to lack of resolution from previous methods. Potential choices of \DIFaddbegin \DIFadd{the }\DIFaddend targeted basis for this purpose will be discussed in Section \ref{section:basis_functions}.

Every good thing comes at a price. For \DIFdelbegin \DIFdel{CMB-BEst}\DIFdelend \DIFaddbegin \textsc{\DIFadd{CMB-BEst}}\DIFaddend , the price is computational cost. Combining the best of Modal and KSW estimators, \DIFdelbegin \DIFdel{CMB-BEst}\DIFdelend \DIFaddbegin \textsc{\DIFadd{CMB-BEst}}\DIFaddend 's formalism is more numerically demanding than both of them. Na\"ively speaking, running \DIFdelbegin \DIFdel{CMB-BEst }\DIFdelend \DIFaddbegin \textsc{\DIFadd{CMB-BEst}} \DIFaddend for one set of basis functions is equivalent to computing thousands of KSW estimators put together. It is prohibitively expensive unless properly and thoroughly optimised.

We invested a considerable amount of time and effort \DIFdelbegin \DIFdel{on }\DIFdelend \DIFaddbegin \DIFadd{in }\DIFaddend optimising the code. Separable mode expansions and subsequent algorithm design were studied in detail for \DIFaddbegin \DIFadd{a }\DIFaddend maximal reduction in computational complexity. We also made full use of parallelisation techniques to exploit modern computing architecture. Improving data locality for efficient memory access yielded an order of magnitude speed-up. We illustrate our optimisation procedure in Section \ref{section:implementation}.

The code was then tested thoroughly both internally and against Planck analysis. We used CMB maps and simulations from the Planck satellite experiment and checked that \DIFdelbegin \textit{\DIFdel{CMB-BEst}} %DIFAUXCMD
\DIFdelend \DIFaddbegin \textsc{\DIFadd{CMB-BEst}} \DIFaddend agrees with previous routines map-by-map for various bispectrum templates. Different choices of basis functions within \DIFdelbegin \textit{\DIFdel{CMB-BEst}} %DIFAUXCMD
\DIFdelend \DIFaddbegin \textsc{\DIFadd{CMB-BEst}} \DIFaddend also yielded consistent results. We dedicate Section \ref{section:validation} to presenting the outcome of these consistency checks.

Lastly, we present some applications of the code which \DIFdelbegin \DIFdel{serves }\DIFdelend \DIFaddbegin \DIFadd{serve }\DIFaddend as a proof of concept. In particular, we connect \DIFdelbegin \DIFdel{CMB-BEst }\DIFdelend \DIFaddbegin \textsc{\DIFadd{CMB-BEst}} \DIFaddend to \textsc{Primodal}, an efficient numerical code for computing bispectra of primordial perturbations from a given single field model \cite{Clarke2021}. \textsc{Primodal} uses separable modes for its computation, which enables template-free, direct model-to-constraint analysis when combined with \DIFdelbegin \DIFdel{CMB-BEst }\DIFdelend \DIFaddbegin \textsc{\DIFadd{CMB-BEst}} \DIFaddend appropriately. Section \ref{section:proof_of_concept} highlights some results from the combined pipeline. We also identify areas where improvements could be made and discuss directions for future research.

Upcoming surveys will provide major improvements to constraints on primordial non-Gaussianity. Inflationary models with oscillations would also greatly benefit from the enhanced sensitivity, as we discussed in the previous chapter. It is therefore crucial to have a robust and flexible bispectrum estimation routine ready for \DIFdelbegin \DIFdel{the }\DIFdelend future surveys, especially one which can handle high-frequency oscillations. Having another code independent from the existing ones would also be greatly beneficial for \DIFdelbegin \DIFdel{cross validation}\DIFdelend \DIFaddbegin \DIFadd{cross-validation}\DIFaddend . We expect \DIFdelbegin \DIFdel{CMB-BEst }\DIFdelend \DIFaddbegin \textsc{\DIFadd{CMB-BEst}} \DIFaddend to fill this role in the near future.


\section{Formalism}

\subsection{\DIFdelbegin \DIFdel{CMB-BEst }\DIFdelend \DIFaddbegin \textsc{\DIFadd{CMB-BEst}} \DIFaddend formalism}
Recall that the optimal CMB bispectrum estimator for a given template can be written as
\begin{align}
	\hat{f}\DIFdelbegin \DIFdel{_{NL} }\DIFdelend \DIFaddbegin \DIFadd{_\text{NL} }\DIFaddend = \frac{1}{N} \sum_{l_j,m_j} \frac{\mathcal{G}^{l_1 l_2 l_3}_{m_1 m_2 m_3} b_{l_1 l_2 l_3}}{C_{l_1} C_{l_2} C_{l_3}} \left[ a_{l_1 m_1} a_{l_2 m_2} a_{l_3 m_3} - \left( \left< a^G_{l_1 m_1} a^G_{l_2 m_2} \right> a_{l_3 m_3} + \text{2\ cyc.} \right)  \right].		\label{eqn:bispectrum_estimator_standard}
\end{align}
...
\begin{align}
	\hat{f}\DIFdelbegin \DIFdel{_{NL}}\DIFdelend \DIFaddbegin \DIFadd{_\text{NL}}\DIFaddend ^{(i)} = \frac{1}{N} \sum_{p_j} \alpha_{p_1 p_2 p_3} (\beta\DIFdelbegin \DIFdel{^{cub,(i)}}\DIFdelend \DIFaddbegin \DIFadd{^{\textrm{cub},(i)}}\DIFaddend _{p_1 p_2 p_3} - 3 \beta\DIFdelbegin \DIFdel{^{lin,(i)}}\DIFdelend \DIFaddbegin \DIFadd{^{\textrm{lin},(i)}}\DIFaddend _{p_1 p_2 p_3}), \label{eqn:fNL_from_betas}
\end{align}
where most of the computation required is now contained in the `$\beta$'s, given by
\begin{align}
	\beta\DIFdelbegin \DIFdel{^{cub,(i)}}\DIFdelend \DIFaddbegin \DIFadd{^{\textrm{cub},(i)}}\DIFaddend _{p_1 p_2 p_3} &:= \int dr \int d^2\vv{n} \; M^{(i)}_{p_1} (\vv{n},r) M^{(i)}_{p_2} (\vv{n},r) M^{(i)}_{p_3} (\vv{n},r),	\label{def:beta_cubic} \\
	\beta\DIFdelbegin \DIFdel{^{lin,(i)}}\DIFdelend \DIFaddbegin \DIFadd{^{\textrm{lin},(i)}}\DIFaddend _{p_1 p_2 p_3} &:= \DIFdelbegin \DIFdel{\frac{1}{N_{sims}} }\DIFdelend \DIFaddbegin \DIFadd{\frac{1}{N_\text{sims}} }\DIFaddend \sum_{j \neq i} \int dr \int d^2\vv{n} \; M^{(j)}_{p_1} (\vv{n},r) M^{(j)}_{p_2} (\vv{n},r) M^{(i)}_{p_3} (\vv{n},r). \label{def:beta_linear}
\end{align}
Here we evaluate \DIFdelbegin \DIFdel{$f_{NL}$ }\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}$ }\DIFaddend estimates for each of the Gaussian simulations \DIFdelbegin \DIFdel{$i=1,\cdots,N_{sims}$}\DIFdelend \DIFaddbegin \DIFadd{$i=1,\cdots,N_\text{sims}$}\DIFaddend . Naturally, they are normally distributed with \DIFdelbegin \DIFdel{mean }\DIFdelend \DIFaddbegin \DIFadd{a mean of }\DIFaddend zero. Under the null hypothesis that the initial fluctuations are purely Gaussian, the value of \DIFdelbegin \DIFdel{$f_{NL}$ }\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}$ }\DIFaddend estimated from the observed CMB map is also drawn from the same normal distribution. Any statistically significant deviations from zero would therefore allow us to reject the null hypothesis.

It is important to note that the $\beta$ matrices depend only on the choice of mode functions and input map data, and are independent of the theoretical bispectrum considered. Once \DIFdelbegin \DIFdel{$\beta^{cub}$ and $\beta^{lin}$ }\DIFdelend \DIFaddbegin \DIFadd{$\beta^\text{cub}$ and $\beta^\text{lin}$ }\DIFaddend are computed and stored, we may constrain any model of interest by decomposing the template to get $\alpha$. The \DIFdelbegin \DIFdel{$f_{NL}$ }\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}$ }\DIFaddend estimate is then  a simple dot product: $\vv{\alpha} \cdot \vv{\beta} / N$.
...
\DIFdelbegin \DIFdel{First, we }\DIFdelend \DIFaddbegin \DIFadd{The main difference between }\textsc{\DIFadd{CMB-BEst}} \DIFadd{and the conventional Modal estimator \mbox{%DIFAUXCMD
\cite{Fergusson2010general} }\hspace{0pt}%DIFAUXCMD
is that }\textsc{\DIFadd{CMB-BEst}} \DIFadd{has a single set of mode functions while Modal requires two; one in each of the primordial and late-time space (see section \ref{section:CMB_bispectrum_estimators}). While the Modal approach can accurately express a wide range of bispectra \mbox{%DIFAUXCMD
\cite{Fergusson2012,Fergusson2014,PlanckCollaboration2013}}\hspace{0pt}%DIFAUXCMD
, the two-step modal expansion becomes less accurate for highly oscillatory shapes since the oscillations often get lost during the projection to late-time space \mbox{%DIFAUXCMD
\cite{Fergusson2015a,PlanckCollaboration2018}}\hspace{0pt}%DIFAUXCMD
. }\textsc{\DIFadd{CMB-BEst}} \DIFadd{avoids these issues in the second mode expansion by evaluating an exact analytical form analogous to the KSW estimator \mbox{%DIFAUXCMD
\cite{Komatsu2005}}\hspace{0pt}%DIFAUXCMD
. The results obtained from }\textsc{\DIFadd{CMB-BEst}} \DIFadd{is therefore as accurate as the primordial basis expansion.
}

\DIFadd{We }\DIFaddend observe that the KSW estimator \cite{Komatsu2005} is derived from a simple monomial basis in our notation;
\begin{align}
...
\begin{align}
	S\DIFdelbegin \DIFdel{^{local}}\DIFdelend \DIFaddbegin \DIFadd{^\textrm{local}}\DIFaddend (k_1 k_2 k_3) :=& 2A^2 \left[ \frac{k_1^2}{k_2 k_3} + \frac{k_2^2}{k_3 k_1} + \frac{k_3^2}{k_1 k_2}  \right] \\
...
The \DIFdelbegin \DIFdel{CMB-BEst }\DIFdelend \DIFaddbegin \textsc{\DIFadd{CMB-BEst}} \DIFaddend formalism significantly reduces the amount of computation needed for CMB bispectrum estimation. Obtaining the linear term \DIFdelbegin \DIFdel{$\beta^{lin}$ }\DIFdelend \DIFaddbegin \DIFadd{$\beta^\text{lin}$ }\DIFaddend in (\ref{def:beta_linear}), however, is still prohibitively expensive unless thoroughly optimised. Performance is especially crucial here since it directly affects the breadth of models we can cover; the faster the code, the higher number of modes we can have, and so the more shapes we can expand in our basis accurately.

In this section, we provide details for various aspects of our optimisation process: algorithm design, parallel computing, and data locality improvements. \DIFdelbegin \DIFdel{Final }\DIFdelend \DIFaddbegin \DIFadd{The final }\DIFaddend specifications and data files used are outlined at the end.

Throughout this section\DIFaddbegin \DIFadd{, }\DIFaddend we will treat the functions of interest as discrete arrays. Our notation for indices and their limits are summarised in Table \ref{table:index_conventions}. We adopt a simple trapezoidal rule for most numerical integrals. For the $\mu$ integral in (\ref{def:gamma}), however, we use \DIFaddbegin \DIFadd{the }\DIFaddend Gauss-Legendre quadrature computed from the public code \textsc{Quadpts} \cite{Hale2013} due to the highly oscillatory and unstable nature of the integral. Multi-dimensional arrays are stored in the \DIFdelbegin \DIFdel{row major }\DIFdelend \DIFaddbegin \DIFadd{row-major }\DIFaddend order following the \textsc{C} convention for efficiency. We use the \textsc{Healpix} library \cite{Gorski2005healpix} for pixelisation of the sky, which includes the \textsc{Libsharp} \cite{Reinecke2013libsharp} library for Spherical Harmonic Transforms (SHTs).
...
In most cases of interest, the bottleneck point of our pipeline is computing the linear term \DIFdelbegin \DIFdel{$\beta^{lin}$}\DIFdelend \DIFaddbegin \DIFadd{$\beta^\text{lin}$}\DIFaddend . Even though the $\Gamma$ matrix computation through (\ref{def:gamma}) grows more rapidly with the number of basis functions (\DIFdelbegin \DIFdel{$\propto p_{max}^6$}\DIFdelend \DIFaddbegin \DIFadd{$\propto p_\text{max}^6$}\DIFaddend ) than the $\beta$'s (\DIFdelbegin \DIFdel{$\propto p_{max}^3$}\DIFdelend \DIFaddbegin \DIFadd{$\propto p_\text{max}^3$}\DIFaddend ), it does not involve operations with high-definition maps and remains subdominant in terms of \DIFaddbegin \DIFadd{the }\DIFaddend total cost.

We dedicate this section to \DIFdelbegin \DIFdel{explain }\DIFdelend \DIFaddbegin \DIFadd{explaining }\DIFaddend our algorithm design for $\beta$ computation in detail. The discretised versions of (\ref{def:beta_cubic}-\ref{def:beta_linear}) are given by
\begin{align}
	\beta\DIFdelbegin \DIFdel{^{cub}}\DIFdelend \DIFaddbegin \DIFadd{^\text{cub}}\DIFaddend (i, p_1,p_2,p_3) &= \sum_r \sum_n \; M(r, i, p_1, n) \cdot M(r, i, p_2, n) \cdot M(r, i, p_3, n), \\
	\beta\DIFdelbegin \DIFdel{^{lin}}\DIFdelend \DIFaddbegin \DIFadd{^\text{lin}}\DIFaddend (i, p_1,p_2,p_3) &= \sum_r \sum_{j \neq i} \sum_n \; M(r, j, p_1, n) \cdot M(r, j, p_2, n) \cdot M(r, i, p_3, n).
\end{align}
The order of indices \DIFdelbegin \DIFdel{are }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend chosen such that later calculations have optimal memory layouts. Some integral weights and factors are absorbed into arrays for brevity.

Note that the data arrays for different values of $r$ are completely independent \DIFdelbegin \DIFdel{to }\DIFdelend \DIFaddbegin \DIFadd{of }\DIFaddend each other. This provides us \DIFaddbegin \DIFadd{with }\DIFaddend a natural way to distribute tasks. We compute and save contributions to $\beta$'s for each $r$ separately. The summation over $r$ is performed \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{at }\DIFaddend the end. Therefore, throughout the rest of this chapter, we assume that $r$ is fixed and drop the $r$ dependence in the descriptions of our algorithms.
...
As a rough guide to the size of each summation, we typically have \DIFdelbegin \DIFdel{$N_{sim} \approx 150$ simulations, $p_{max} = 30$ }\DIFdelend \DIFaddbegin \DIFadd{$N_\text{sim} \approx 150$ simulations, $p_\text{max} = 30$ }\DIFaddend modes, and \DIFdelbegin \DIFdel{$N_{pix} = 50,331,648$ pixels. }\DIFdelend \DIFaddbegin \DIFadd{$N_\text{pix} = 50,331,648$ pixels. }\DIFaddend \footnote{This value corresponds to $N_{side} = 2048$ in Healpix. \DIFdelbegin \DIFdel{$N_{pix} = 12 N_{side}^2$}\DIFdelend \DIFaddbegin \DIFadd{$N_\text{pix} = 12 N_{side}^2$}\DIFaddend } Considering the fact that one double-precision array of size $\sim 50$ million pixels takes about 400MB of memory space, this is indeed a task for supercomputers.

Our first and most straightforward method of computing $\beta$s is outlined in Algorithm \ref{alg:beta_first_attempt}. \DIFdelbegin \DIFdel{Computational }\DIFdelend \DIFaddbegin \DIFadd{The computational }\DIFaddend complexity of each innermost loop is annotated on the \DIFdelbegin \DIFdel{right hand }\DIFdelend \DIFaddbegin \DIFadd{right-hand }\DIFaddend side.

\begin{algorithm}[htbp]
	\caption{Computing $\beta$s: the na\"ive method}
	\label{alg:beta_first_attempt}
	\begin{algorithmic}[1] \State Allocate $M(i,p,n)$
		\DIFdelbegin %DIFDELCMD < \Comment{Memory $\sim N_{sims} \cdot p_{max} \cdot N_{pix}$}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \Comment{Memory $\sim N_\text{sims} \cdot p_\text{max} \cdot N_\text{pix}$}
\DIFaddend 

		\\
		\For{each map $i$}
			\For{each mode $p$}
			\DIFdelbegin %DIFDELCMD < \Comment{$O(N_{sims} \cdot p_{max} \cdot N_{pix}^{3/2})$}
%DIFDELCMD < 				%%%
\DIFdelend \DIFaddbegin \Comment{$O(N_\text{sims} \cdot p_\text{max} \cdot N_\text{pix}^{3/2})$}
				\DIFaddend \State \textbf{compute} $M(i,p,n)$ by SHT
			\EndFor
		...
		\For{each map $i$}
			\For{each set of modes $(p_1,p_2,p_3)$}
				\For{each pixel $n$}
				\DIFdelbegin %DIFDELCMD < \Comment{$O(N_{sims} \cdot p_{max}^3 \cdot N_{pix})$}
%DIFDELCMD < 					%%%
\DIFdelend \DIFaddbegin \Comment{$O(N_\text{sims} \cdot p_\text{max}^3 \cdot N_\text{pix})$}
					\DIFaddend \State \DIFdelbegin \DIFdel{$\beta^{cub}(i, p_1, p_2, p_3) \pluseq M(i,p_1,n) \cdot M(i,p_2,n) \cdot M(i,p_3,n)$
				}\DIFdelend \DIFaddbegin \DIFadd{$\beta^\text{cub}(i, p_1, p_2, p_3) \pluseq M(i,p_1,n) \cdot M(i,p_2,n) \cdot M(i,p_3,n)$
				}\DIFaddend \EndFor
			\EndFor
		\EndFor
		...
		\For{each map $i$}
			\For{each map $j \neq i$}
				\For{each set of modes $(p_1,p_2,p_3)$}
					\For{each pixel $n$}
					\DIFdelbegin %DIFDELCMD < \Comment{$O(N_{sims}^2 \cdot p_{max}^3 \cdot N_{pix})$}
%DIFDELCMD < 						%%%
\DIFdelend \DIFaddbegin \Comment{$O(N_\text{sims}^2 \cdot p_\text{max}^3 \cdot N_\text{pix})$}
						\DIFaddend \State \DIFdelbegin \DIFdel{$\beta^{lin}(i, p_1, p_2, p_3) \pluseq M(j,p_1,n) \cdot M(j,p_2,n) \cdot M(i,p_3,n)$
					}\DIFdelend \DIFaddbegin \DIFadd{$\beta^\text{lin}(i, p_1, p_2, p_3) \pluseq M(j,p_1,n) \cdot M(j,p_2,n) \cdot M(i,p_3,n)$
					}\DIFaddend \EndFor
				\EndFor
			\EndFor
		\EndFor
...
Here, we first obtain the filtered maps $M(i,p,n)$ via SHT for each map and \DIFdelbegin \DIFdel{modes}\DIFdelend \DIFaddbegin \DIFadd{mode}\DIFaddend . The full results are stored in memory. Next, we iterate through each map and set of modes $(p_1, p_2, p_3)$ and sum over each of the map pixels to obtain \DIFdelbegin \DIFdel{$\beta^{cub}$ and $\beta^{lin}$}\DIFdelend \DIFaddbegin \DIFadd{$\beta^\text{cub}$ and $\beta^\text{lin}$}\DIFaddend . Symmetries in the indices are respected; we only loop over $(p_1,p_2,p_3)$ satisfying $p_1\ge p_2$ for the linear terms, and $p_1\ge p_2\ge p_3$ for the cubic ones.

SHT typically scales as \DIFdelbegin \DIFdel{$\propto l_{max}^3$, where $l_{max}$ }\DIFdelend \DIFaddbegin \DIFadd{$\propto l_\text{max}^3$, where $l_\text{max}$ }\DIFaddend is the maximum degree of spherical harmonic functions used. The total number of pixels \DIFdelbegin \DIFdel{$N_{pix}$ grows $\propto l_{max}^2$ }\DIFdelend \DIFaddbegin \DIFadd{$N_\text{pix}$ grows $\propto l_\text{max}^2$ }\DIFaddend when chosen appropriately for resolution, hence the SHT costs \DIFdelbegin \DIFdel{$O(N_{sims}\cdot p_{max} \cdot N_{pix}^{3/2})$}\DIFdelend \DIFaddbegin \DIFadd{$O(N_\text{sims}\cdot p_\text{max} \cdot N_\text{pix}^{3/2})$}\DIFaddend . The most expensive part of this algorithm is still the loop where we calculate the linear term, which scales as \DIFdelbegin \DIFdel{$O(N_{sims}^2 \cdot p_{max}^3 \cdot N_{pix})$}\DIFdelend \DIFaddbegin \DIFadd{$O(N_\text{sims}^2 \cdot p_\text{max}^3 \cdot N_\text{pix})$}\DIFaddend .

Our first major optimisation comes from the observation that we may swap the order of summations to reduce the computation. By precomputing $C(p_1, p_2, n) = \sum_j M(j,p_1,n) \cdot M(j,p_2,n)$, we require one fewer loop over maps for \DIFdelbegin \DIFdel{$\beta^{lin}$}\DIFdelend \DIFaddbegin \DIFadd{$\beta^\text{lin}$}\DIFaddend , leading to a factor of \DIFdelbegin \DIFdel{$N_{sims}\sim150$ }\DIFdelend \DIFaddbegin \DIFadd{$N_\text{sims}\sim150$ }\DIFaddend improvement. Algorithm \ref{alg:beta_second_attempt} shows a pseudocode for this method.

\begin{algorithm}[htbp]
	\caption{Computing $\beta$s: optimised for computation}
	\label{alg:beta_second_attempt}
	\begin{algorithmic}[1] \State Allocate $M(i,p,n)$ \DIFdelbegin %DIFDELCMD < \Comment{Memory $\sim N_{sims} \cdot p_{max} \cdot N_{pix}$}
%DIFDELCMD < 		%%%
\DIFdelend \DIFaddbegin \Comment{Memory $\sim N_\text{sims} \cdot p_\text{max} \cdot N_\text{pix}$}
		\DIFaddend \State Allocate $C(p_1,p_2,n)$ \DIFdelbegin %DIFDELCMD < \Comment{Memory $\sim p_{max}^2 \cdot N_{pix}$}
%DIFDELCMD < 		%%%
\DIFdelend \DIFaddbegin \Comment{Memory $\sim p_\text{max}^2 \cdot N_\text{pix}$}
		\DIFaddend \\
		\For{each map $i$}
			\For{each mode $p$}
				\DIFdelbegin %DIFDELCMD < \Comment{$O(N_{sims} \cdot p_{max} \cdot N_{pix}^{3/2})$}
...
\DIFdelend \DIFaddbegin \Comment{$O(N_\text{sims} \cdot p_\text{max} \cdot N_\text{pix}^{3/2})$}
		...
		\For{each map $j$}
			\For{each pair of modes $(p_1,p_2)$}
				\For{each pixel $n$}
					\DIFdelbegin %DIFDELCMD < \Comment{$O(N_{sims} \cdot p_{max}^2 \cdot N_{pix})$}
%DIFDELCMD < 					%%%
\DIFdelend \DIFaddbegin \Comment{$O(N_\text{sims} \cdot p_\text{max}^2 \cdot N_\text{pix})$}
					\DIFaddend \State $C(p_1,p_2,n) \pluseq M(j,p_1,n) \cdot M(j,p_2,n)$
				\EndFor
			\EndFor
...
\DIFdelend \DIFaddbegin \Comment{$O(N_\text{sims} \cdot p_\text{max}^3 \cdot N_\text{pix})$}
					...
					}\DIFaddend \State \DIFdelbegin \DIFdel{$\beta^{lin}(i, p_1, p_2, p_3) \pluseq C(p_1, p_2, n) \cdot M(i, p_3, n)$
				}\DIFdelend \DIFaddbegin \DIFadd{$\beta^\text{lin}(i, p_1, p_2, p_3) \pluseq C(p_1, p_2, n) \cdot M(i, p_3, n)$
				}\DIFaddend \EndFor
			\EndFor
		\EndFor
	\end{algorithmic}
...
Note that the resulting sum \DIFdelbegin \DIFdel{fo $\beta^{lin}$ }\DIFdelend \DIFaddbegin \DIFadd{of $\beta^\text{lin}$ }\DIFaddend includes an unwanted contribution from the case where $j=i$, since $C(p_1,p_2,n)$ is obtained by summing over all maps. Thankfully, this extra contribution is exactly equal to the cubic term. We just have to subtract \DIFdelbegin \DIFdel{$\beta^{cub}$ }\DIFdelend \DIFaddbegin \DIFadd{$\beta^\text{cub}$ }\DIFaddend from the total sum to get the correct value of \DIFdelbegin \DIFdel{$\beta^{lin}$}\DIFdelend \DIFaddbegin \DIFadd{$\beta^\text{lin}$}\DIFaddend .

We shaved off a whole loop at the cost of extra memory usage. For our purposes \DIFdelbegin \DIFdel{$N_{sims} \sim 150 > p_{max} = 30$}\DIFdelend \DIFaddbegin \DIFadd{$N_\text{sims} \sim 150 > p_\text{max} = 30$}\DIFaddend , so the additional space required for $C(p_1,p_2,n)$ is relatively small compared to $M(i,p,n)$. The symmetry in $p_1$ and $p_2$ also means that we only need to store \DIFdelbegin \DIFdel{$p_{max}(p_{max}+1)/2$ }\DIFdelend \DIFaddbegin \DIFadd{$p_\text{max}(p_\text{max}+1)/2$ }\DIFaddend maps instead of \DIFdelbegin \DIFdel{$p_{max}^2$ }\DIFdelend \DIFaddbegin \DIFadd{$p_\text{max}^2$ }\DIFaddend for $C$.

Our next challenge is the amount of memory required to save $M(i,p,n)$. For the parameters mentioned above, this is $\approx 1.8$TB, which is quite significant. Although it is not impossible to find supercomputing systems which can accommodate such large arrays, reducing the memory usage would be highly beneficial. We seek to achieve this whilst keeping the overall computational complexity to be \DIFdelbegin \DIFdel{$O(N_{sims} \cdot p_{max}^3 \cdot N_{pix})$ }\DIFdelend \DIFaddbegin \DIFadd{$O(N_\text{sims} \cdot p_\text{max}^3 \cdot N_\text{pix})$ }\DIFaddend as before.

We observe that most summations are done for a single map $i$. The array $C(p_1,p_2,n)$ is required to be computed before the main loop for \DIFdelbegin \DIFdel{$\beta^{lin}$}\DIFdelend \DIFaddbegin \DIFadd{$\beta^\text{lin}$}\DIFaddend , but otherwise there are no `mixing' between the maps. We exploited this fact to develop Algorithm \ref{alg:beta_third_attempt}.

\begin{algorithm}[htbp]
	\caption{Computing $\beta$s: fast and memory efficient}
	\label{alg:beta_third_attempt}
	\begin{algorithmic}[1] \State Allocate $m(p,n)$ \DIFdelbegin %DIFDELCMD < \Comment{Memory $\sim p_{max} \cdot N_{pix}$}
%DIFDELCMD < 		%%%
\DIFdelend \DIFaddbegin \Comment{Memory $\sim p_\text{max} \cdot N_\text{pix}$}
		\DIFaddend \State Allocate $C(p_1,p_2,n)$ \DIFdelbegin %DIFDELCMD < \Comment{Memory $\sim p_{max} \cdot p_{max} \cdot N_{pix}$}
%DIFDELCMD < 		%%%
\DIFdelend \DIFaddbegin \Comment{Memory $\sim p_\text{max}^2 \cdot N_\text{pix}$}
		\DIFaddend \\
		\For{each map $i$}
...
%DIFDELCMD < 				%%%
\DIFdelend \DIFaddbegin \Comment{$O(N_\text{sims} \cdot p_\text{max} \cdot N_\text{pix}^{3/2})$}
				\DIFaddend \State \textbf{compute} $M(i,p,n)$ by SHT and store in $m(p,n)$
			\EndFor
			\\
			\For{each pair of modes $(p_1,p_2)$}
				\For{each pixel $n$}
...
%DIFDELCMD < 					%%%
\DIFdelend \DIFaddbegin \Comment{$O(N_\text{sims} \cdot p_\text{max}^2 \cdot N_\text{pix})$}
					\DIFaddend \State $C(p_1,p_2,n) \pluseq m(p_1,n) \cdot m(p_2,n)$
				\EndFor
			\EndFor
		\EndFor
...
%DIFDELCMD < 					%%%
\DIFdelend \DIFaddbegin \Comment{$O(N_\text{sims} \cdot p_\text{max}^3 \cdot N_\text{pix})$}	
					\DIFaddend \State \DIFdelbegin \DIFdel{$\beta^{cub}(i, p_1, p_2, p_3) \pluseq m(p_1,n) \cdot m(p_2,n) \cdot m(p_3,n)$
					}\DIFdelend \DIFaddbegin \DIFadd{$\beta^\text{cub}(i, p_1, p_2, p_3) \pluseq m(p_1,n) \cdot m(p_2,n) \cdot m(p_3,n)$
					}\DIFaddend \State \DIFdelbegin \DIFdel{$\beta^{lin}(i, p_1, p_2, p_3) \pluseq C(p_1, p_2, n) \cdot m( p_3, n)$
				}\DIFdelend \DIFaddbegin \DIFadd{$\beta^\text{lin}(i, p_1, p_2, p_3) \pluseq C(p_1, p_2, n) \cdot m( p_3, n)$
				}\DIFaddend \EndFor
			\EndFor
		\EndFor
	\end{algorithmic}
...
SHTs have \DIFaddbegin \DIFadd{a }\DIFaddend subdominant contribution to the total computation time even after becoming doubled in number. One of the main strengths of Algorithm \ref{alg:beta_third_attempt} is that both the memory and computation time scale linearly with the number of simulations used, \DIFdelbegin \DIFdel{$N_{sims}$}\DIFdelend \DIFaddbegin \DIFadd{$N_\text{sims}$}\DIFaddend . In the future when a larger number of Gaussian simulations are required to acquire a more accurate estimate of the linear term, it is straightforward to adapt our method accordingly.

We choose Algorithm \ref{alg:beta_third_attempt} to be our main method of computing $\beta$'s for \DIFdelbegin \DIFdel{CMB-BEst}\DIFdelend \DIFaddbegin \textsc{\DIFadd{CMB-BEst}}\DIFaddend .

\subsection{Parallel computing}



In order to fully benefit from modern computer architecture, parallel computing is \DIFdelbegin \DIFdel{a must}\DIFdelend \DIFaddbegin \DIFadd{essential}\DIFaddend . In this section, we outline how \DIFdelbegin \DIFdel{CMB-BEst }\DIFdelend \DIFaddbegin \textsc{\DIFadd{CMB-BEst}} \DIFaddend was parallelised for optimal performance on supercomputers.

Following \cite{Jeffers2016intel}, we discuss \DIFdelbegin \DIFdel{parallelism in }\DIFdelend \DIFaddbegin \DIFadd{parallelisation at }\DIFaddend three different levels: domain, thread, and data. They mostly correspond to nodes, cores, and registers in modern computer clusters. Each level has distinct characteristics which make them ideal for different parallelisation techniques. We make full use of each level in our methodology.

\textit{Domain} parallelism refers to dividing the task into many domains where each domain entails heavy computation while having limited data communication between them. Since the domains are largely independent, Message Passing Interface (MPI) \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Gropp1999MPI} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend is a suitable tool for parallelisation. MPI is a communication protocol where many instances (`ranks') of the same code may transfer data while running separately from each other \DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{\. }\DIFaddend A perfect example of domain parallelism in \DIFdelbegin \DIFdel{CMB-BEst }\DIFdelend \DIFaddbegin \textsc{\DIFadd{CMB-BEst}} \DIFaddend would be our line-of-sight integration over $r$. Almost no data is shared between different $r$s, despite the heavy computational cost from map operations within each domain. \DIFdelbegin \DIFdel{CMB-BEst }\DIFdelend \DIFaddbegin \textsc{\DIFadd{CMB-BEst}} \DIFaddend therefore scales well with the number of MPI ranks \DIFaddbegin \DIFadd{and requires minimal effort to parallelise using MPI}\DIFaddend .

\textit{Thread} parallelism opportunities arise when there are many independent computational tasks on a single set of data. Most modern supercomputers use multi-core processors. Each core, or processing \DIFdelbegin \DIFdel{units}\DIFdelend \DIFaddbegin \DIFadd{unit}\DIFaddend , can run one or more threads, executing instructions independently from each other while sharing memory space. Note that MPI would not be as effective here due to \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{a }\DIFaddend large amount of data sharing needed; ranks have to either continuously communicate with each other, or store individual copies of the data. This type of parallelism is extremely common in many applications, including SHTs and map integrations in \DIFdelbegin \DIFdel{CMB-BEst}\DIFdelend \DIFaddbegin \textsc{\DIFadd{CMB-BEst}}\DIFaddend . Open Multi-Processing (OpenMP) \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Dagum1998openmp} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend is one of the most popular implementations of multi-threading publicly available. We utilise the \textsc{C} OpenMP interface in our code. 

\textit{Data} parallelism applies when the same arithmetic operations are applied to multiple data items, ideally adjacent in memory space. This type of parallel processing is called SIMD: Single Instruction, Multiple Data. Many procedures involving large arrays fall into this category. We use Advanced Vector Extensions (AVX) supported by Intel processors to exploit data parallelism in \DIFdelbegin \DIFdel{CMB-BEst}\DIFdelend \DIFaddbegin \textsc{\DIFadd{CMB-BEst}}\DIFaddend , especially for \DIFaddbegin \DIFadd{the }\DIFaddend operations involving large (filtered) map arrays. In particular, Intel's Xeon Phi series implement AVX-512, where 512-bit registers hold up to 8 double-precision floating numbers \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Jeffers2016intel}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . A single instruction can be applied \DIFdelbegin \DIFdel{on }\DIFdelend \DIFaddbegin \DIFadd{to }\DIFaddend all of them at once in a single clock cycle, providing a major boost to \DIFaddbegin \DIFadd{the }\DIFaddend computation speed. We explicitly align large arrays in memory for optimal vectorisation performance.
...
\begin{algorithmic}
	\For{each set of modes $(p_1,p_2,p_3)$}
		\For{each pixel $n$}
			\State \DIFdelbegin \DIFdel{$\beta^{cub}(i, p_1, p_2, p_3) \pluseq m(p_1,n) \cdot m(p_2,n) \cdot m(p_3,n)$
			}\DIFdelend \DIFaddbegin \DIFadd{$\beta^\text{cub}(i, p_1, p_2, p_3) \pluseq m(p_1,n) \cdot m(p_2,n) \cdot m(p_3,n)$
			}\DIFaddend \State \DIFdelbegin \DIFdel{$\beta^{lin}(i, p_1, p_2, p_3) \pluseq C(p_1, p_2, n) \cdot m( p_3, n)$
		}\DIFdelend \DIFaddbegin \DIFadd{$\beta^\text{lin}(i, p_1, p_2, p_3) \pluseq C(p_1, p_2, n) \cdot m( p_3, n)$
		}\DIFaddend \EndFor
...
In every loop over the set of modes $(p_1, p_2, p_3)$, four large arrays are read from memory: $m(p_1,\cdot)$, $m(p_2,\cdot)$, $m(p_3,\cdot)$, and $C(p_1, p_2, \cdot)$. Each of them takes up around 400MB of memory. Since their size is greater than the cache storage capacity, data in front of them are gone from the cache by the time a loop over $n$ completes. All four arrays will then have to be loaded from \DIFaddbegin \DIFadd{the }\DIFaddend main memory again when the next iteration starts.

Suppose that the map arrays had smaller \DIFdelbegin \DIFdel{size }\DIFdelend \DIFaddbegin \DIFadd{sizes }\DIFaddend and all of them fit in one of the cache levels instead. After one loop iteration over $n$ for $(p_1, p_2, p_3)$, the arrays now remain in \DIFaddbegin \DIFadd{the }\DIFaddend cache. Then the next loop on $(p_1, p_2, p_3+1)$ begins, and the arrays $m(p_1,\cdot)$, $m(p_2,\cdot)$, and $C(p_1,p_2,\cdot)$ are required again for computation. They can now be accessed from \DIFaddbegin \DIFadd{the }\DIFaddend cache, which saves a significant amount of time. We see that each data element is reused multiple times before being discarded from the cache in this case.
...
New pixel numbers are calculated as $n' = B \cdot b + n$, where $B$ is the size of each block and $0 \le n < B$. We have not changed the total number of arithmetic operations required, so the computational complexity remains constant. Meanwhile, data locality within each block is greatly improved, as each of the blocked arrays \DIFdelbegin \DIFdel{are }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend now small enough to fit in \DIFaddbegin \DIFadd{the }\DIFaddend cache. Each data element is accessed in closer succession temporarily as well.

One caveat here is that having too many blocks may degrade \DIFaddbegin \DIFadd{the overall }\DIFaddend performance. There \DIFdelbegin \DIFdel{exist }\DIFdelend \DIFaddbegin \DIFadd{exists }\DIFaddend a non-negligible overhead coming from an extra \textit{for} loop and the OpenMP construct used over $n'$. The block size divided by the number of cores should not be smaller than the size of cache lines either. The optimal size of cache blocks depends on the memory architecture of the processor used. This often needs to be found empirically.

Figure (\ref{fig:cache_block_optimisation}) shows how \DIFaddbegin \DIFadd{the }\DIFaddend runtime of a small test code varies with the number of cache blocks used. The code was run on the Intel Gold 6154 processor, which is an 18-core \DIFdelbegin \DIFdel{high performance }\DIFdelend \DIFaddbegin \DIFadd{high-performance }\DIFaddend chip with total L1, L2, and L3 cache memory sizes of 1.125MiB, 18MiB, and 24.74MiB, respectively \cite{intel_data_sheet}. Even though some parameters such as \DIFdelbegin \DIFdel{$N_{sims}$ }\DIFdelend \DIFaddbegin \DIFadd{$N_\text{sims}$ }\DIFaddend are tuned down for faster testing, this is representative of the full computation.

\begin{figure}[htbp!] 
	\centering    
	\includegraphics[width=0.8\textwidth]{cache_block_optimisation_annotated.pdf}
	\caption[Cache block optimisation]{Cache block optimisation. \DIFdelbeginFL \DIFdelFL{Total }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{The total }\DIFaddendFL wall time of a test code for computing $\beta$s was measured while varying the number of cache blocks used. Having 128 blocks is optimal for the 18-core Intel Gold 6154 processor.}
	\label{fig:cache_block_optimisation}
\end{figure}

We find a significant improvement in performance moving from 32 to 48 blocks when the block size drops from 12MiB to 8MiB. This is precisely when three blocks are allowed to fit in \DIFaddbegin \DIFadd{the }\DIFaddend L3 cache (24.7MiB) at the same time. Improved data locality dramatically reduces time spent on memory access as we expected. The optimal number of blocks was found to be 128; each segment contains about 400,000 elements and takes up 3MiB of memory, which is 1/6 and 1/8 of L2 and L3 cache size, respectively. We gain roughly three times speed-up compared to the original code without cache blocking.
...
CMB bispectrum estimation is not only computationally challenging but also prone to numerical instabilities unless implemented carefully. We invested a considerable amount of time after the development of \DIFdelbegin \DIFdel{CMB-BEst in }\DIFdelend \DIFaddbegin \textsc{\DIFadd{CMB-BEst}} \DIFaddend validating various aspects of the code. We highlight some of our validation efforts in this section, checking consistency within the program itself (section \ref{section:internal_consistency}) and against existing code such as Modal \cite{Fergusson2012} (section \ref{section:consistency_with_Planck}).

\subsection{Internal consistency checks} \label{section:internal_consistency}

\DIFdelbegin \DIFdel{CMB-BEst }\DIFdelend \DIFaddbegin \textsc{\DIFadd{CMB-BEst}} \DIFaddend is a general code where one can freely choose a set of basis functions. Three of our main options are the `KSW' basis (\ref{eqn:KSW_basis}), `Legendre' basis (\ref{eqn:Legendre_basis_no_inv_k}, augmented with \DIFdelbegin \DIFdel{$q(k)=k^{n_s-2}$}\DIFdelend \DIFaddbegin \DIFadd{$q(k)=k^{n_\text{s}-2}$}\DIFaddend ) as discussed previously in Section \ref{section:basis_functions}, and Fourier basis (\ref{eqn:Fourier_basis}). Both KSW and Legendre basis sets can cover the standard templates: local, equilateral, and orthogonal (see e.g., \cite{PlanckCollaboration2013} for definitions). The KSW basis provides an exact form \DIFdelbegin \DIFdel{to }\DIFdelend \DIFaddbegin \DIFadd{for }\DIFaddend the three templates by choosing appropriate powers of $k$ as its basis elements. On the other hand, the templates are expanded in terms of separable Legendre polynomials up to some fixed degree \DIFdelbegin \DIFdel{$p_{max}$ }\DIFdelend \DIFaddbegin \DIFadd{$p_\text{max}$ }\DIFaddend for the Legendre basis. As long as \DIFdelbegin \DIFdel{$p_{max}$ }\DIFdelend \DIFaddbegin \DIFadd{$p_\text{max}$ }\DIFaddend is sufficiently large, most smooth bispectrum shapes can be represented accurately. 

Our first consistency check is shown in Figure \ref{fig:map_by_map_Legendre_KSW}, where we compare the \DIFdelbegin \DIFdel{$f_{NL}$ }\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}$ }\DIFaddend estimates from the Planck 2018 CMB map and 140 full focal plane (FFP10) realistic Gaussian simulations \cite{PlanckCollaboration2015simulations}. We use CMB maps obtained through the SMICA component separation method \cite{Cardoso2008component, PlanckCollaboration2013ComponentSeparation}. On the \DIFdelbegin \DIFdel{left hand }\DIFdelend \DIFaddbegin \DIFadd{left-hand }\DIFaddend side are scatter plots of \DIFdelbegin \DIFdel{$f_{NL}$ }\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}$ }\DIFaddend values obtained using each of the two basis sets. In an ideal case where the two estimates are identical for every test map, all the points would lie on a straight line given by $y=x$. Drawn in dashed red line is the best linear fit to the data. Its slope, intercept, and the $R$-squared value are annotated below. On the right is a more detailed plot of the computed \DIFdelbegin \DIFdel{$f_{NL}$ }\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}$ }\DIFaddend for each map.

\begin{figure}[htbp!] 
	\centering 
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{map_by_map_Legendre_KSW.pdf}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics{map_by_map_Legendre_KSW.pdf}
	\DIFaddendFL \caption{\DIFdelbeginFL \DIFdelFL{Map-by-map }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{A map-by-map }\DIFaddendFL comparison of \DIFdelbeginFL \DIFdelFL{$f_{NL}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{the $f_\text{NL}$ }\DIFaddendFL estimates \DIFdelbeginFL \DIFdelFL{for standard templates, }\DIFdelendFL evaluated using \DIFdelbeginFL \DIFdelFL{each of }\DIFdelendFL the KSW and Legendre basis sets \DIFaddbeginFL \DIFaddFL{for three standard templates}\DIFaddendFL . \DIFaddbeginFL \DIFaddFL{The }\DIFaddendFL Planck 2018 CMB map and 140 FFP10 simulations have been used, each representing a single point on the scatter plot \DIFdelbeginFL \DIFdelFL{shown }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{(}\DIFaddendFL left\DIFaddbeginFL \DIFaddFL{)}\DIFaddendFL . Details of the linear best-fit to data (red\DIFdelbeginFL \DIFdelFL{dashed}\DIFdelendFL ) are annotated below. \DIFdelbeginFL \DIFdelFL{On }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Shown on }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{right hand }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{right-hand }\DIFaddendFL side \DIFdelbeginFL \DIFdelFL{is shown a plot }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{are plots }\DIFaddendFL of \DIFdelbeginFL \DIFdelFL{$f_{NL}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{the differences in the $f_\text{NL}$ }\DIFaddendFL values for each map\DIFaddbeginFL \DIFaddFL{, together with the $1\sigma$ and $2\sigma$ intervals shaded in blue}\DIFaddendFL . In the ideal case where the two basis sets yield identical results, we should see all the points \DIFaddbeginFL \DIFaddFL{lie }\DIFaddendFL on \DIFaddbeginFL \DIFaddFL{the line }\DIFaddendFL $y=x$ for the left plot \DIFdelbeginFL \DIFdelFL{, }\DIFdelendFL and \DIFdelbeginFL \DIFdelFL{exactly overlapping graphs }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$y=0$ }\DIFaddendFL for the right \DIFaddbeginFL \DIFaddFL{plot}\DIFaddendFL . For more information on each of the three theoretical templates used, see e.g., \cite{PlanckCollaboration2013}.}
...
The fact that \DIFaddbegin \DIFadd{the }\DIFaddend results from the KSW and Legendre basis are consistent validates multiple aspects of our pipeline. First of all, we can deduce that the Legendre basis expansion (\ref{eqn:basis_expansion}) accurately represents the bispectrum shapes of interest, especially since the KSW basis is designed to be exact for the three standard shapes. Having \DIFdelbegin \DIFdel{$p_{max}=30$ }\DIFdelend \DIFaddbegin \DIFadd{$p_\text{max}=30$ }\DIFaddend modes is more than sufficient \DIFaddbegin \DIFadd{to achieve sub-percent accuracy}\DIFaddend . Figure \ref{fig:map_by_map_Legendre_30_29} explicitly compares results obtained from Legendre bases with \DIFdelbegin \DIFdel{$p_{max}=29$ }\DIFdelend \DIFaddbegin \DIFadd{$p_\text{max}=29$ }\DIFaddend and $30$ \DIFdelbegin \DIFdel{, }\DIFdelend while keeping everything else the same. The two results show \DIFdelbegin \DIFdel{absolute agreement, }\DIFdelend \DIFaddbegin \DIFadd{close agreement; }\DIFaddend the largest of \DIFdelbegin \DIFdel{errors }\DIFdelend \DIFaddbegin \DIFadd{the errors is }\DIFaddend less than $O(10^{-5})$. This further confirms that our expansion using Legendre polynomials has completely converged.

\begin{figure}[htbp!] 
	\centering    
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{map_by_map_Legendre_30_29.pdf}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics{map_by_map_Legendre_30_29.pdf}
	\DIFaddendFL \caption{\DIFdelbeginFL \DIFdelFL{Map-by-map }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{A map-by-map }\DIFaddendFL comparison of \DIFdelbeginFL \DIFdelFL{$f_{NL}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{the $f_\text{NL}$ }\DIFaddendFL estimates for \DIFaddbeginFL \DIFaddFL{three }\DIFaddendFL standard templates \DIFaddbeginFL \DIFaddFL{evaluated }\DIFaddendFL using the Legendre basis with different \DIFdelbeginFL \DIFdelFL{number }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{numbers }\DIFaddendFL of modes: \DIFdelbeginFL \DIFdelFL{$p_{max}=29$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$p_\text{max}=29$ }\DIFaddendFL and $30$. The two results agree with \DIFdelbeginFL \DIFdelFL{error }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{errors }\DIFaddendFL less than $O(10^{-5})$, as can be seen from the scatter plots (left) and \DIFaddbeginFL \DIFaddFL{the }\DIFaddendFL map-by-map \DIFaddbeginFL \DIFaddFL{residual }\DIFaddendFL plots (right). \DIFaddbeginFL \DIFaddFL{Shaded in blue are the $1\sigma$ and $2\sigma$ levels of the $f_\text{NL}$ estimator. }\DIFaddendFL This confirms that our expansion using Legendre polynomials has completely converged for these bispectrum shapes.}
	\label{fig:map_by_map_Legendre_30_29}
\end{figure}

Secondly, we validated the accuracy of the linear term and lensing-ISW bias calculations through the sample mean of \DIFdelbegin \DIFdel{$f_{NL}$ }\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}$ }\DIFaddend estimates from lensed simulations. When we exclude either of the two, we find the sample mean to be far from zero. The error is especially large for the local template. The linear term accounts for the anisotropy introduced by masking parts of the observed sky. The squeezed limit contribution to \DIFaddbegin \DIFadd{the }\DIFaddend bispectrum comes from couplings between a pair of small-scale modes and a large-scale one, making it more susceptible to bias from partial coverage. \DIFdelbegin \DIFdel{Presence }\DIFdelend \DIFaddbegin \DIFadd{The presence }\DIFaddend of sky masks therefore offsets the \DIFdelbegin \DIFdel{$f_{NL}$ }\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}$ }\DIFaddend estimates from the local shape which has a large squeezed limit. Lensing-ISW bias is also the largest in squeezed configurations and affects Local \DIFdelbegin \DIFdel{$f_{NL}$}\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}$}\DIFaddend . The fact that our lensed Gaussian simulations have \DIFdelbegin \DIFdel{$f_{NL}$s fluctuate }\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}$s fluctuating }\DIFaddend around $0$ validates our bias subtractions.

Lastly, we check that \DIFdelbegin \DIFdel{CMB-BEst accurately preserves }\DIFdelend \DIFaddbegin \textsc{\DIFadd{CMB-BEst}} \DIFadd{accurately preserves the }\DIFaddend optimality of the CMB bispectrum estimator. In the weak non-Gaussian limit, the estimator (\ref{eqn:bispectrum_estimator_standard}) saturates the Cramer-Rao bound. Its expected variance is the lowest amongst all possible unbiased bispectrum estimators of \DIFdelbegin \DIFdel{$f_{NL}$}\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}$}\DIFaddend . Heuristically speaking, the estimator extracts as much information about non-Gaussianity as possible from the CMB bispectrum. As we discussed in Chapter \ref{chapter:CMB_state-4_forecast}, this bound only depends on the Fisher information determined from normalisation; \DIFdelbegin \DIFdel{$Var[\hat{f}_{NL}]=F^{-1}=6/N$}\DIFdelend \DIFaddbegin \DIFadd{$Var[\hat{f}_\text{NL}]=F^{-1}=6/N$}\DIFaddend . We refer to this value as the \textit{theoretical} variance (the best possible from theory). Meanwhile, individual \DIFdelbegin \DIFdel{$f_{NL}$ }\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}$ }\DIFaddend estimates from simulated maps and independent and normally distributed. Sample variance obtained from \DIFdelbegin \DIFdel{$N_{sims}$ }\DIFdelend \DIFaddbegin \DIFadd{$N_\text{sims}$ }\DIFaddend simulations should therefore approach the theoretical variance as \DIFdelbegin \DIFdel{$N_{sims}\rightarrow\infty$}\DIFdelend \DIFaddbegin \DIFadd{$N_\text{sims}\rightarrow\infty$}\DIFaddend . Table \ref{table:trio_sample_and_theory_variances} summarises calculated values of the two types of variances discussed.

\begin{table}[h]
	\caption{Comparison of the sample and theoretical variances obtained from \DIFdelbeginFL \DIFdelFL{$f_{NL}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$f_\text{NL}$ }\DIFaddendFL estimates of standard shapes, computed using each of the KSW and Legendre basis sets. Sample variances are within $1\sigma$ of theoretical values assuming that the 140 individual \DIFdelbeginFL \DIFdelFL{$f_{NL}$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$f_\text{NL}$}\DIFaddendFL 's from simulated maps are normally distributed. This is statistically consistent with \DIFaddbeginFL \DIFaddFL{the }\DIFaddendFL optimality of our bispectrum estimator.}
	\centering
...
Note that the numbers do not match up exactly between sample and theoretical variances\DIFdelbegin \DIFdel{. The estimator for sample variance $\hat{S}^2 = (\sum_i (f^{(i)}_{NL})^2 )/N_{sims}$ }\DIFdelend \DIFaddbegin \DIFadd{, and the results for equilateral template appear to be more optimal than the theory allows. This is because the sample variance $\hat{S}^2 = (\sum_i (f^{(i)}_\text{NL})^2 )/N_\text{sims}$ }\DIFaddend follows a chi-squared distribution with \DIFdelbegin \DIFdel{$N_{sims}-1$ }\DIFdelend \DIFaddbegin \DIFadd{$N_\text{sims}-1$ }\DIFaddend degrees of freedom under our assumptions. The standard deviation corresponding to the (normalised) distribution equals \DIFdelbegin \DIFdel{$\sqrt{2/(N_{sims}-1)}$}\DIFdelend \DIFaddbegin \DIFadd{$\sqrt{2/(N_\text{sims}-1)}$}\DIFaddend , which is $\approx 0.12$ for \DIFdelbegin \DIFdel{$N_{sims}=140$}\DIFdelend \DIFaddbegin \DIFadd{$N_\text{sims}=140$}\DIFaddend . It is therefore not surprising to see our sample variances differ by up to $12\%$ from the theoretical ones. 

We performed further checks to ensure that this discrepancy is due to statistical fluctuations rather than systematic errors. The ratio between \DIFaddbegin \DIFadd{the }\DIFaddend sample and theoretical variances remained nearly constant across the KSW and Legendre basis from \DIFdelbegin \DIFdel{CMB-BEst}\DIFdelend \DIFaddbegin \textsc{\DIFadd{CMB-BEst}}\DIFaddend , as well as Planck's Modal pipeline on the same set of simulated maps. Meanwhile, sample variances evaluated from independent sets of simulations do fluctuate around the theoretical value. A closer check has been done for each of the mode sets $(p_1,p_2,p_3)$ in the Legendre basis. The decomposition coefficient $\alpha$ is set to be 1 at $(p_1,p_2,p_3)$ and its permutations, and then vanishing everywhere else. Substituting into \eqref{eqn:fNL_from_betas} and \eqref{eqn:normlisation_from_gamma}, we get
\begin{align}
	f\DIFdelbegin \DIFdel{_{NL}}\DIFdelend \DIFaddbegin \DIFadd{_\text{NL}}\DIFaddend ^{(i)} &= \frac{1}{N} \left[ \left( \beta\DIFdelbegin \DIFdel{^{cub,(i)}}\DIFdelend \DIFaddbegin \DIFadd{^{\textrm{cub},(i)}}\DIFaddend _{p_1 p_2 p_3} - 3 \beta\DIFdelbegin \DIFdel{^{lin,(i)}}\DIFdelend \DIFaddbegin \DIFadd{^{\textrm{lin},(i)}}\DIFaddend _{p_1 p_2 p_3} \right) + \text{5 cyc.} \right], \\
...
While Figure \ref{fig:map_by_map_Legendre_KSW} shows excellent agreement between results from the KSW and Legendre basis sets overall, there are small but noticeable scatters in the plot for the local shape. The slope of linear best fit is also slightly above 1, meaning that \DIFdelbegin \DIFdel{$f_{NL}$ }\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}$ }\DIFaddend estimates from the Legendre routine tend to vary 5\% more than the KSW ones. \DIFdelbegin \DIFdel{Larger variance means that }\DIFdelend \DIFaddbegin \DIFadd{A larger variance means }\DIFaddend less information extracted. Our first hypothesis was that \DIFaddbegin \DIFadd{the }\DIFaddend Legendre polynomials are losing a small fraction of information \DIFaddbegin \DIFadd{from the CMB }\DIFaddend due to their fixed $k$ range in their definition ($\ref{eqn:Legendre_basis_no_inv_k}$). We test this by increasing the range by changing \DIFdelbegin \DIFdel{$k_{max}/k_{min} = 1000$ }\DIFdelend \DIFaddbegin \DIFadd{$k_\text{max}/k_\text{min} = 1000$ }\DIFaddend to $2000$. The results are shown in Figure \ref{fig:map_by_map_Legendre_KSW_k_ratio_2000}.

\begin{figure}[htbp!] 
	\centering    
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{map_by_map_Legendre_KSW_k_ratio_2000.pdf}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics{map_by_map_Legendre_KSW_k_ratio_2000.pdf}
	\DIFaddendFL \caption{\DIFdelbeginFL \DIFdelFL{Map-by-map }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{A map-by-map }\DIFaddendFL comparison of \DIFdelbeginFL \DIFdelFL{$f_{NL}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{the $f_\text{NL}$ }\DIFaddendFL estimates for \DIFaddbeginFL \DIFaddFL{three }\DIFaddendFL standard templates from the KSW and Legendre basis sets, similar to Figure \ref{fig:map_by_map_Legendre_KSW}. Here, the Legendre basis has a wider $k$ domain: \DIFdelbeginFL \DIFdelFL{$k_{max}/k_{min} = 2000$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$k_\text{max}/k_\text{min} = 2000$ }\DIFaddendFL instead of the usual $1000$. \DIFdelbeginFL \DIFdelFL{Number }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{The number }\DIFaddendFL of modes (\DIFdelbeginFL \DIFdelFL{$p_{max}$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$p_\text{max}$}\DIFaddendFL ) has been reduced to $10$ instead of $30$ \DIFdelbeginFL \DIFdelFL{for simplicity}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{due to limited computational resources}\DIFaddendFL . We see that \DIFaddbeginFL \DIFaddFL{the }\DIFaddendFL additional information \DIFaddbeginFL \DIFaddFL{gained }\DIFaddendFL from large scales (low $k$) fixes the small \DIFdelbeginFL \DIFdelFL{scatter }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{scatters }\DIFaddendFL present in \DIFdelbeginFL \DIFdelFL{$f_{NL}^{local}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$f_\text{NL}^\textrm{local}$ }\DIFaddendFL of the previous plot\DIFaddbeginFL \DIFaddFL{, however at the cost of increased spread in $f_\text{NL}^\textrm{equil}$ and $f_\text{NL}^\textrm{ortho}$}\DIFaddendFL .}
	\label{fig:map_by_map_Legendre_KSW_k_ratio_2000}
\end{figure}

Our main focus of this figure is on the local template results. Estimates from the KSW and Legendre basis sets match almost perfectly now. When altering the ratio \DIFdelbegin \DIFdel{$k_{max}/k_{min}$}\DIFdelend \DIFaddbegin \DIFadd{$k_\text{max}/k_\text{min}$}\DIFaddend , we fixed the \DIFdelbegin \DIFdel{$k_{max}$ and lowered $k_{min}$}\DIFdelend \DIFaddbegin \DIFadd{$k_\text{max}$ and lowered $k_\text{min}$}\DIFaddend . Including more small-$k$, or \DIFdelbegin \DIFdel{large scale }\DIFdelend \DIFaddbegin \DIFadd{large-scale }\DIFaddend modes provides extra information in the bispectrum. \DIFdelbegin \DIFdel{Local }\DIFdelend \DIFaddbegin \DIFadd{The local }\DIFaddend shape is especially affected by this \DIFdelbegin \DIFdel{, }\DIFdelend since squeezed configurations including one of these extra large scale modes have a more significant contribution to the total estimate. Also\DIFdelbegin \DIFdel{note that $k_{max} / k_{min} = 2000$ }\DIFdelend \DIFaddbegin \DIFadd{, note that $k_\text{max} / k_\text{min} = 2000$ }\DIFaddend is comfortably larger than the equivalent ratio in harmonic space \DIFdelbegin \DIFdel{$l_{max} / l_{min} = 2500 / 2$}\DIFdelend \DIFaddbegin \DIFadd{$l_\text{max} / l_\text{min} = 2500 / 2$}\DIFaddend .

Despite the improvements in the squeezed limit and local template, we may not simply set \DIFdelbegin \DIFdel{$k_{max} / k_{min} = 2000$ }\DIFdelend \DIFaddbegin \DIFadd{$k_\text{max} / k_\text{min} = 2000$ }\DIFaddend as default because it hurts convergence in other shapes of interest. As can be seen from the Equilateral and Orthogonal plots in Figure \ref{fig:map_by_map_Legendre_KSW_k_ratio_2000}, newer estimates of \DIFdelbegin \DIFdel{$f_{NL}$ }\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}$ }\DIFaddend are less accurate for shapes other than Local. The fact that \DIFdelbegin \DIFdel{$p_{max}=10$ }\DIFdelend \DIFaddbegin \DIFadd{$p_\text{max}=10$ }\DIFaddend here rather than $30$ is one of the main causes of the drop in precision, but having smaller $k$ values within Legendre polynomials' domain also has a negative impact. Shapes with \DIFdelbegin \DIFdel{inverse $k$ scaling }\DIFdelend \DIFaddbegin \DIFadd{a $1/k$ scaling in their expressions }\DIFaddend vary more dramatically when $k$ is small and tend to be harder to expand in terms of Legendre polynomials.

For the final check of internal consistency, we inspect how each point in the line of sight integral ($r$) contributes to \DIFdelbegin \DIFdel{$f_{NL}$ estimates. CMB-BEst}\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}$ estimates. }\textsc{\DIFadd{CMB-BEst}}\DIFaddend 's formalism makes it straightforward to plot the $r$ integrand since the integral is done at the very end. Figure \ref{fig:trio_r_dependence} shows plots of \DIFdelbegin \DIFdel{$f_{NL}$ }\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}$ }\DIFaddend contributions from different sources and for the three standard bispectrum templates. Shaded in light blue are the $1\sigma$ regions obtained from 140 simulations for each point in $r$.

\begin{figure}[htbp!] 
	\centering    
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{trio_r_dependence.pdf}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics{trio_r_dependence.pdf}
	\DIFaddendFL \caption{Contributions to the total \DIFdelbeginFL \DIFdelFL{$f_{NL}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$f_\text{NL}$ }\DIFaddendFL from each point in the line-of-sight integral over $r$ for standard templates. On the \DIFdelbeginFL \DIFdelFL{left hand }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{left-hand }\DIFaddendFL side, we plot contributions from the cubic, linear, and lensing-ISW bias, as well as the total \DIFdelbeginFL \DIFdelFL{$f_{NL}$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$f_\text{NL}$}\DIFaddendFL . Shown in blue is the $1\sigma$ interval obtained from \DIFaddbeginFL \DIFaddFL{the }\DIFaddendFL corresponding terms in 140 FFP10 Gaussian simulations. We focus on the $r$ interval around recombination where most of the signal comes from. Plots on the \DIFdelbeginFL \DIFdelFL{right hand }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{right-hand }\DIFaddendFL side show \DIFdelbeginFL \DIFdelFL{$f_{NL}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{the $f_\text{NL}$ }\DIFaddendFL contributions over the whole $r$ range in log scale. \DIFaddbeginFL \DIFaddFL{The }\DIFaddendFL ISW effect, reionisation, and recombination are responsible for the three most noticeable peaks in all three plots.}
	\label{fig:trio_r_dependence}
\end{figure}

As illustrated in plots on the \DIFdelbegin \DIFdel{right hand }\DIFdelend \DIFaddbegin \DIFadd{right-hand }\DIFaddend side of Figure \ref{fig:trio_r_dependence}, the vast majority of signal comes from recombination around $r = 14,000$Mpc. In fact, its contribution is dominant enough \DIFdelbegin \DIFdel{so that neglecting signal }\DIFdelend \DIFaddbegin \DIFadd{that neglecting signals }\DIFaddend from everywhere else would still be a good approximation. \DIFdelbegin \DIFdel{CMB-BEst }\DIFdelend \DIFaddbegin \textsc{\DIFadd{CMB-BEst}} \DIFaddend uses an adaptive $r$ grid which is denser around recombination, following the works of \cite{Smith2011}. Other small but notable contributions to the total estimate \DIFdelbegin \DIFdel{comes }\DIFdelend \DIFaddbegin \DIFadd{come }\DIFaddend from reionisation ($r \sim 10,000$Mpc) and the ISW effect ($r < 5,000$Mpc).

Zooming in on an interval around recombination, significant contributions from the cubic term to \DIFdelbegin \DIFdel{$f_{NL}$ of }\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}$ of the }\DIFaddend Local template is rather prominent (upper left of Figure \ref{fig:trio_r_dependence}). The signal is sufficiently larger than the expected random fluctuations evaluated from Gaussian simulations, which could be hinting at non-zero primordial non-Gaussianity. However, contributions from the linear term (shown orange) completely \DIFdelbegin \DIFdel{counterbalances }\DIFdelend \DIFaddbegin \DIFadd{counterbalance }\DIFaddend it, bringing the total down to values consistent with zero. This again validates \DIFaddbegin \DIFadd{the }\DIFaddend accuracy of our methodology; bias to \DIFdelbegin \DIFdel{$f_{NL}$ }\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}$ }\DIFaddend generated from anisotropic sky masks are precisely subtracted off using the linear terms.

We do not find any statistically significant signal across the whole $r$ range otherwise, especially when taking \DIFaddbegin \DIFadd{the }\DIFaddend look-elsewhere effect into account. It is not meaningful to find a couple of 3-4$\sigma$ values when the other 500 points are simply disregarded. If we detect primordial non-Gaussianity in the future, however, then these plots of \DIFdelbegin \DIFdel{$f_{NL}$ }\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}$ }\DIFaddend contributions for each $r$ will provide valuable insights \DIFdelbegin \DIFdel{on }\DIFdelend \DIFaddbegin \DIFadd{into }\DIFaddend where the signal comes from.
...
\begin{table}[h]
	...
	\begin{tabular}{lcccc}
		\toprule
		& \DIFdelbeginFL %DIFDELCMD < \multicolumn{2}{c}{CMB-BEst} %%%
...
Note that while the constraints from \DIFdelbegin \DIFdel{CMB-BEst }\DIFdelend \DIFaddbegin \textsc{\DIFadd{CMB-BEst}} \DIFaddend are largely consistent with Planck 2018 results, there \DIFdelbegin \DIFdel{discrepancies }\DIFdelend \DIFaddbegin \DIFadd{are discrepancies of }\DIFaddend up to $0.3\sigma$. However, there are variations around this level within the estimates from different pipelines of Planck as well. Equilateral constraints from Planck's own KSW and Modal estimators shown here, for example, differ by $\approx 0.4\sigma$. A similar amount of fluctuation can be found in the full result shown in \cite{PlanckCollaboration2018}. In an ideal world, they should match exactly across different approaches as long as the same dataset is used. However, \DIFaddbegin \DIFadd{the }\DIFaddend statistical significance of the individual \DIFdelbegin \DIFdel{$f_{NL}$}\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}$}\DIFaddend 's is not largely affected by such variations.

We invested a significant proportion of our time investigating this error. Here we discuss three potential areas which might account for the discrepancies. First of all, human error during the implementation and estimation process should not be neglected. Bispectrum analysis is complex and computationally expensive. Implementing it often involves writing a long and heavily optimised code. During the development and testing stages, we found and fixed many mistakes in our $10,000+$ lines of \textsc{C} code. Various unit tests were performed in the process to test individual sections of the code: basis expansion, projection to the $l$ space, SHTs, parallelisation, and more. Internal consistency checks were then used to verify \DIFaddbegin \DIFadd{the }\DIFaddend integrity of the combined pipeline. The Planck team also went \DIFdelbegin \DIFdel{on }\DIFdelend \DIFaddbegin \DIFadd{to great }\DIFaddend lengths to validate and cross-check different methods \cite{PlanckCollaboration2013}. It therefore seems unlikely that trivial mistakes are causing the gap in results.

The next source of error we studied is the parameter set. The cosmological parameters we used for constraints in Table \ref{table:trio_fNL_comparison_with_planck} are identical to those of the Planck Modal pipeline. Small changes in cosmological parameters are also shown to have little effect on \DIFdelbegin \DIFdel{$f_{NL}$ }\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}$ }\DIFaddend \cite{PlanckCollaboration2015,PlanckCollaboration2018}. One major difference, however, is the number of Gaussian simulations used. We included 140 simulated maps while Modal has about 300. This number is mainly restricted by computational resources required for the large Legendre basis in \DIFdelbegin \DIFdel{CMB-BEst}\DIFdelend \DIFaddbegin \textsc{\DIFadd{CMB-BEst}}\DIFaddend . While we found 140 to be sufficient for most cases, sample variance may cause some fluctuation in the linear term and error bar. A rough estimate for sampling error is \DIFdelbegin \DIFdel{$\sim \sqrt{2/N_{sims}} \approx 0.12$}\DIFdelend \DIFaddbegin \DIFadd{$\sim \sqrt{2/N_\text{sims}} \approx 0.12$}\DIFaddend , as calculated in the previous section. Other than \DIFdelbegin \DIFdel{$N_{sims}$}\DIFdelend \DIFaddbegin \DIFadd{$N_\text{sims}$}\DIFaddend , more internal parameters such as the grid density of discretised arrays have also been checked to yield consistent results when varied. 

We are left with systematic errors as potential causes for discrepancies. The most significant bias to \DIFdelbegin \DIFdel{$f_{NL}$ }\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}$ }\DIFaddend constraints is the lensing-ISW bias discussed in Section \ref{section:other_sources_of_non_gaussianity}. Table \ref{table:trio_lensing_bias_comparison_with_planck} shows biases from the lensing bispectrum \cite{Lewis2011lensing} for both \DIFdelbegin \DIFdel{CMB-BEst }\DIFdelend \DIFaddbegin \textsc{\DIFadd{CMB-BEst}} \DIFaddend and Planck 2018 analysis. The numbers vary but are consistent overall. Map-by-map \DIFdelbegin \DIFdel{comparison }\DIFdelend \DIFaddbegin \DIFadd{comparisons }\DIFaddend of constraints from the Legendre basis and Modal are shown in Figure \ref{fig:map_by_map_Legendre_Modal}. 

\begin{table}[h]
	\caption{Bias to \DIFdelbeginFL \DIFdelFL{$f_{NL}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$f_\text{NL}$ }\DIFaddendFL of standard shapes originating from the lensing bispectrum. We compare \DIFdelbeginFL \DIFdelFL{CMB-BEst}\DIFdelendFL \DIFaddbeginFL \textsc{\DIFaddFL{CMB-BEst}}\DIFaddendFL 's two different basis sets and the Planck 2018 analysis \cite{PlanckCollaboration2018}, using SMICA map and FFP10 simulations. }
	\centering
...
\begin{figure}[htbp!] 
	\centering    
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{map_by_map_Legendre_Modal.pdf}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics{map_by_map_Legendre_Modal.pdf}
	\DIFaddendFL \caption{\DIFdelbeginFL \DIFdelFL{Map-by-map }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{A map-by-map }\DIFaddendFL comparison of \DIFdelbeginFL \DIFdelFL{$f_{NL}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{the $f_\text{NL}$ }\DIFaddendFL estimates obtained from \DIFdelbeginFL \DIFdelFL{CMB-BEst}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{the }\textsc{\DIFaddFL{CMB-BEst}}\DIFaddendFL 's Legendre basis set against the Modal estimator results \DIFdelbeginFL \DIFdelFL{for }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{of the }\DIFaddendFL Planck 2018 analysis \cite{PlanckCollaboration2018}. The first 140 FFP10 simulations are used \DIFaddbeginFL \DIFaddFL{here}\DIFaddendFL . On the \DIFdelbeginFL \DIFdelFL{left hand }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{left-hand }\DIFaddendFL side are scatter plots where each simulation is represented by a point according to \DIFdelbeginFL \DIFdelFL{$f_{NL}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$f_\text{NL}$ }\DIFaddendFL estimates of standard templates. Their linear best fit lines are shown in \DIFdelbeginFL \DIFdelFL{dashed }\DIFdelendFL red. \DIFdelbeginFL \DIFdelFL{Results }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Differences in the estimates }\DIFaddendFL from the two routines are shown map-by-map on the \DIFdelbeginFL \DIFdelFL{right hand }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{right-hand }\DIFaddendFL side\DIFaddbeginFL \DIFaddFL{, together with the $1\sigma$ and $2\sigma$ levels shaded in blue}\DIFaddendFL . Overall, \DIFdelbeginFL \DIFdelFL{CMB-BEst }\DIFdelendFL \DIFaddbeginFL \textsc{\DIFaddFL{CMB-BEst}} \DIFaddendFL and Modal are in good agreement without any significant systematic errors.}
	\label{fig:map_by_map_Legendre_Modal}
\end{figure}

Out of the three templates, \DIFaddbegin \DIFadd{the }\DIFaddend local shape shows the largest scatter between constraints. Comparing \DIFdelbegin \DIFdel{$f_{NL}$}\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}$}\DIFaddend 's of simulated maps from \DIFdelbegin \DIFdel{CMB-BEst }\DIFdelend \DIFaddbegin \textsc{\DIFadd{CMB-BEst}} \DIFaddend and Modal, we find a correlation of 0.916. The intercept value of $0.78$ in the linear fit is mainly due to the sample mean present in Modal. Legendre's sample mean is $0.069$ for Local, while Modal's is $-0.698$. Otherwise, the difference between the two pipelines \DIFdelbegin \DIFdel{are }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend distributed such that its sample standard deviation equals $2.5$, skewness is $-0.14$, and kurtosis is $0.19$. Having a low level of small skewness and kurtosis \DIFaddbegin \DIFadd{of }\DIFaddend less than $1$ implies that the distribution is close to a univariate normal, consistent with random fluctuation. Similarly, we do not find any significant systematic error from the equilateral and orthogonal shapes either.

Having not found a clear source of error, we conclude that the small gaps between the \DIFdelbegin \DIFdel{$f_{NL}$ }\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}$ }\DIFaddend estimates in Table \ref{table:trio_fNL_comparison_with_planck} mainly come from differences in methodology and are consistent with random fluctuations.

We now move on to constraining models with oscillations. The simplest template for oscillatory models is the feature model studied in Chapter \ref{chapter:CMB_state-4_forecast}. We use a template shape function of the form $S^\text{feat}(k_1,k_2,k_3) = \sin(\omega (k_1 + k_2 + k_3) + \phi)$. Figure \ref{fig:sine_template_frequency_Legendre_Modal} shows \DIFdelbegin \DIFdel{$f_{NL}$ }\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}$ }\DIFaddend constraints obtained from the Legendre basis and compares them with Modal results from the Planck 2018 analysis. The `phase' $\phi$ is set to zero, while \DIFaddbegin \DIFadd{the }\DIFaddend oscillation `frequency' $\omega$ was varied from $10$ Mpc to $350$ Mpc. We follow \cite{Fergusson2015a} and increase $\omega$ in steps of $10$ so that \DIFdelbegin \DIFdel{correlation }\DIFdelend \DIFaddbegin \DIFadd{correlations }\DIFaddend between shapes with different $\omega$ \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{are }\DIFaddend kept low.

\begin{figure}[htbp!] 
	\centering    
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{sine_template_frequency_Legendre_Modal.pdf}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics{sine_template_frequency_Legendre_Modal.pdf}
	\DIFaddendFL \caption{\DIFdelbeginFL \DIFdelFL{Estimated $f_{NL}$'s }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{The $f_\text{NL}$ estimates }\DIFaddendFL for \DIFaddbeginFL \DIFaddFL{the }\DIFaddendFL feature models with \DIFdelbeginFL \DIFdelFL{shape }\DIFdelendFL $S(k_1,k_2,k_3) = \sin(\omega (k_1 + k_2 + k_3)$, obtained using the Legendre basis \DIFdelbeginFL \DIFdelFL{with }\DIFdelendFL (blue) and Planck's Modal (black). \DIFdelbeginFL \DIFdelFL{Left}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Top}\DIFaddendFL : \DIFaddbeginFL \DIFaddFL{a }\DIFaddendFL direct comparison of \DIFdelbeginFL \DIFdelFL{$f_{NL}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$f_\text{NL}$ }\DIFaddendFL for different values of $\omega$. Error bars indicate the expected standard \DIFdelbeginFL \DIFdelFL{deviation }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{deviations }\DIFaddendFL in the estimator \DIFdelbeginFL \DIFdelFL{, }\DIFdelendFL calculated using 140 Gaussian simulations. \DIFdelbeginFL \DIFdelFL{Right}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Bottom}\DIFaddendFL : \DIFdelbeginFL \DIFdelFL{measured }\DIFdelendFL signal-to-noise \DIFdelbeginFL \DIFdelFL{$f_{NL}/\sigma$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$f_\text{NL}/\sigma\;(f_\text{NL})$}\DIFaddendFL , again for a range of $\omega$ values. Shaded in blue are the $1\sigma$ and $2\sigma$ levels. We see that the two approaches yield coherent estimates overall.}
	\label{fig:sine_template_frequency_Legendre_Modal}
\end{figure}

The Legendre basis accurately expands the feature model template using Legendre polynomials via basis expansion outlined in (\ref{eqn:basis_expansion}). Estimates from the two methods, \DIFdelbegin \DIFdel{CMB-BEst}\DIFdelend \DIFaddbegin \textsc{\DIFadd{CMB-BEst}}\DIFaddend 's Legendre and Planck's Modal, are mostly compatible. The most notable difference is at $\omega=230$Mpc where \DIFdelbegin \DIFdel{$f_{NL}$ }\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}$ }\DIFaddend from Legendre is more than $1\sigma$ larger compared to Modal. Even though it is interesting that the new estimate now passes the $2\sigma$ threshold, having one such point out of 35 shown here has less statistical significance.

Both the Legendre and Modal approaches involve expanding the shape function with respect to a polynomial basis. Polynomials are versatile but have limited resolution for oscillatory signals; it cannot resolve shapes with a number of oscillations greater than the maximum degree of polynomials (\DIFdelbegin \DIFdel{$p_{max}$ }\DIFdelend \DIFaddbegin \DIFadd{$p_\text{max}$ }\DIFaddend here). For the Legendre basis with $k$ range $[2.09\times 10^{-4}, 2.09\times 10^{-1}]\text{Mpc}^{-1}$ and \DIFdelbegin \DIFdel{$p_{max}=30$}\DIFdelend \DIFaddbegin \DIFadd{$p_\text{max}=30$}\DIFaddend , frequencies greater than \DIFdelbegin \DIFdel{$\pi p_{max} / (k_{max} - k_{min}) \approx 436$}\DIFdelend \DIFaddbegin \DIFadd{$\pi p_\text{max} / (k_\text{max} - k_\text{min}) \approx 436$}\DIFaddend Mpc are unresolvable. In reality, numerics start to break before this value. We do not have a reference analytic value for true \DIFdelbegin \DIFdel{$f_{NL}$}\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}$}\DIFaddend s, but evaluating correlations between shapes is effective for checking our numerics.
...
Figure \ref{fig:feature_template_correlations} shows correlations between \DIFdelbegin \DIFdel{$f_{NL}$}\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}$}\DIFaddend s from feature models with different frequency $\omega$s. The plots show \DIFdelbegin \DIFdel{correlation between $f_{NL}$ }\DIFdelend \DIFaddbegin \DIFadd{the correlation between $f_\text{NL}$ }\DIFaddend estimates from 140 FFP10 Gaussian simulations (`sample'), together with the one evaluated using our `late-time' inner product ('theoretical') given by
\begin{align}
...
\end{align}
where $\Gamma$ is defined in (\ref{def:gamma}). In the limit \DIFdelbegin \DIFdel{$N_{sims} \rightarrow \infty$}\DIFdelend \DIFaddbegin \DIFadd{$N_\text{sims} \rightarrow \infty$}\DIFaddend , the sample correlation can be shown to approach the theoretical value in the weakly non-Gaussian limit. We see from Figure \ref{fig:feature_template_correlations} that they indeed display the same qualitative behaviour for \DIFdelbegin \DIFdel{$N_{sims}=140$}\DIFdelend \DIFaddbegin \DIFadd{$N_\text{sims}=140$}\DIFaddend .
...
We verify our claim that numerical inaccuracies at high $\omega$s are caused by the lack of resolution due to a limited number of polynomial modes. Figure \ref{fig:feature_template_correlations_compare_lmax} illustrates how having a smaller $k$ range allows us to constrain faster oscillations more accurately. Note that we also change \DIFdelbegin \DIFdel{$l_{max}$ }\DIFdelend \DIFaddbegin \DIFadd{$l_\text{max}$ }\DIFaddend correspondingly since smaller scales are neglected. The plot on the \DIFdelbegin \DIFdel{right hand }\DIFdelend \DIFaddbegin \DIFadd{right-hand }\DIFaddend side uses a Legendre basis with the \DIFdelbegin \DIFdel{$(k_{min}, k_{max})$ }\DIFdelend \DIFaddbegin \DIFadd{$(k_\text{min}, k_\text{max})$ }\DIFaddend range rescaled by a factor of $3/4$. Fewer oscillations appear within the $k$ interval and are therefore easier to expand using fewer polynomials. As we expected, the scale at which non-diagonal correlations blow up is now shifted to a higher frequency, $\omega \approx (4/3) \cdot 300$. 

\begin{figure}[htbp!] 
	\centering    
	\includegraphics[width=\textwidth]{sine_template_correlations_compare_lmax_new.pdf}
	\caption{Theoretical correlations between feature model templates with different frequency $\omega$s, as described in Figure \ref{fig:feature_template_correlations}. The phase $\phi$ is set to zero in both cases, while the right plot is obtained from the Legendre basis with a different $k$ range. The maximum $l$ value is reduced from $2500$ to $2000$, shifting \DIFdelbeginFL \DIFdelFL{$(k_{min},k_{max})$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$(k_\text{min},k_\text{max})$ }\DIFaddendFL by a factor of $2000/2500 = 0.75$. Having a smaller $k$ range means fewer oscillations within the $k$ interval for same $\omega$ which provides better effective resolution. The right plot does indeed show smaller non-diagonal correlations at high frequencies.}
	\label{fig:feature_template_correlations_compare_lmax}
\end{figure}

There is a subtlety around \DIFdelbegin \DIFdel{$f_{NL}$ }\DIFdelend \DIFaddbegin \DIFadd{the $f_\text{NL}$ }\DIFaddend estimates obtained from inaccurate primordial basis expansions. \DIFdelbegin \DIFdel{CMB-BEst }\DIFdelend \DIFaddbegin \textsc{\DIFadd{CMB-BEst}} \DIFaddend computes $\beta$s and $\Gamma$s with respect to \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{a }\DIFaddend fixed Legendre basis, which has been shown to be accurate \DIFdelbegin \DIFdel{even for the highest degree of polynomials. When }\DIFdelend \DIFaddbegin \DIFadd{for each and every polynomial mode. Therefore, even when }\DIFaddend the primordial basis is unable to resolve rapid \DIFdelbegin \DIFdel{modulations as we have seen, the $f_{NL}$ }\DIFdelend \DIFaddbegin \DIFadd{oscillations, the $f_\text{NL}$ }\DIFaddend estimates we obtain are nevertheless meaningful; they are simply probing \DIFdelbegin \DIFdel{the wrong }\DIFdelend \DIFaddbegin \DIFadd{a different }\DIFaddend model. The constraints are not for the given shape function, but rather its projection to the vector subspace spanned by the basis functions as shown in (\ref{eqn:primordial_basis_projection}). Detecting \DIFaddbegin \DIFadd{a }\DIFaddend non-zero \DIFdelbegin \DIFdel{$f_{NL}$ }\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}$ }\DIFaddend here can still have significant implications.

Another popular template for models with oscillations is the `resonance' shape parametrised as $S(k_1, k_2, k_3) = \sin(\omega \log(k_1 + k_2 + k_3 ) + \phi)$. Log-spaced \DIFdelbegin \DIFdel{modulations }\DIFdelend \DIFaddbegin \DIFadd{oscillations }\DIFaddend are numerically harder to deal with since the oscillation frequency diverges as $k \rightarrow 0$. Note also that any scaling factors to the $k$'s can be absorbed into the phase via $\log(c(k_1 + k_2 + k_3)) = \log(k_1 + k_2 + k_3) + \log(c)$. For our Legendre basis with \DIFdelbegin \DIFdel{$k_{max}/k_{min}$ }\DIFdelend \DIFaddbegin \DIFadd{$k_\text{max}/k_\text{min}$ }\DIFaddend fixed to $1000$, the full $k$ range includes $\approx 1.1\omega$ oscillations. Any frequency larger than $\approx 27$ therefore cannot be expanded using \DIFdelbegin \DIFdel{$p_{max}=30$ }\DIFdelend \DIFaddbegin \DIFadd{$p_\text{max}=30$ }\DIFaddend polynomials. We still explore shapes with higher $\omega$s\DIFaddbegin \DIFadd{, }\DIFaddend however, since the basis can pick up slower oscillations in higher $k$ values. Corresponding constraints should be taken with a grain of salt; they probe bispectrum shapes \textit{similar} to the resonance template. Low-$k$ oscillations are especially likely to be wiped out from these shapes.
...
\begin{figure}[htbp!] 
	\centering    
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{sinlog_template_frequency_Legendre_Modal.pdf}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics{sinlog_template_frequency_Legendre_Modal.pdf}
	\DIFaddendFL \caption{Constraints for the resonance shape $S(k_1,k_2,k_3) = \sin(\omega \log(k_1 + k_2 + k_3 ) + \phi)$ with varying \DIFaddbeginFL \DIFaddFL{frequency }\DIFaddendFL $\omega$s\DIFdelbeginFL \DIFdelFL{and }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{, while the phase }\DIFaddendFL $\phi$ \DIFaddbeginFL \DIFaddFL{is }\DIFaddendFL set to $0$ (\DIFdelbeginFL \DIFdelFL{left}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{`SinLog'}\DIFaddendFL ) and $\pi/2$ (\DIFdelbeginFL \DIFdelFL{right}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{`CosLog'}\DIFaddendFL ). Results are obtained using \DIFdelbeginFL \DIFdelFL{CMB-BEst}\DIFdelendFL \DIFaddbeginFL \textsc{\DIFaddFL{CMB-BEst}}\DIFaddendFL 's Legendre basis (blue) and Planck's Modal estimator (black). Top \DIFaddbeginFL \DIFaddFL{two}\DIFaddendFL : signal-to-noise significance of the estimated \DIFdelbeginFL \DIFdelFL{$f_{NL}$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$f_\text{NL}$}\DIFaddendFL s with \DIFaddbeginFL \DIFaddFL{their }\DIFaddendFL $1\sigma$ and $2\sigma$ levels shaded in blue. Bottom \DIFaddbeginFL \DIFaddFL{two}\DIFaddendFL : standard deviations of the \DIFdelbeginFL \DIFdelFL{$f_{NL}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$f_\text{NL}$ }\DIFaddendFL estimates against \DIFdelbeginFL \DIFdelFL{frequency }\DIFdelendFL $\omega$ \DIFdelbeginFL \DIFdelFL{, }\DIFdelendFL calculated from \DIFdelbeginFL \DIFdelFL{the $f_{NL}$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$f_\text{NL}$}\DIFaddendFL s of 140 \DIFaddbeginFL \DIFaddFL{Gaussian }\DIFaddendFL simulations (sample) and the $\Gamma$ matrix (theory).}
	\label{fig:sinlog_template_frequency_Legendre_Modal}
\end{figure}

Similarly to the feature models studied in Chapter \ref{chapter:CMB_state-4_forecast}, uncertainty in the estimated \DIFdelbegin \DIFdel{$f_{NL}$}\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}$}\DIFaddend s increases as we raise $\omega$, exploring more rapid oscillations in \DIFaddbegin \DIFadd{the }\DIFaddend bispectrum. Both the Modal and Legendre methods lose their ability to resolve shapes with $\omega$s larger than $35$. The constraints are then no longer for the precise resonance shape but rather an approximation to it. A notable difference between Modal and \DIFdelbegin \DIFdel{CMB-BEst}\DIFdelend \DIFaddbegin \textsc{\DIFadd{CMB-BEst}}\DIFaddend 's Legendre basis is their behaviour at high $\omega$. Modal's numerical accuracy is completely lost after $\omega\approx 50$, leading to disproportionately large bispectra and small $\sigma$. On the other hand, Legendre retains stability in its basis expansion, yielding constraints for approximate bispectrum shapes closest to the true, high-frequency ones.

\begin{figure}[htbp!] 
	\centering    
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{sinlog_template_frequency_variances_Legendre.pdf}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics{sinlog_template_frequency_variances_Legendre.pdf}
	\DIFaddendFL \caption{\DIFdelbeginFL \DIFdelFL{Ratio }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Ratios }\DIFaddendFL between \DIFaddbeginFL \DIFaddFL{the }\DIFaddendFL sample and theoretical variances obtained from \DIFaddbeginFL \DIFaddFL{the }\DIFaddendFL Legendre basis for resonance shapes. \DIFdelbeginFL \DIFdelFL{Sampled }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{The sample }\DIFaddendFL variance is calculated from \DIFdelbeginFL \DIFdelFL{$f_{NL}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{the $f_\text{NL}$ }\DIFaddendFL estimates of $140$ Gaussian simulations. \DIFaddbeginFL \DIFaddFL{The }\DIFaddendFL $1\sigma$ and $2\sigma$ intervals \DIFaddbeginFL \DIFaddFL{(shaded in blue) }\DIFaddendFL are chosen based on a $\chi^2$-distribution, which accurately describes \DIFaddbeginFL \DIFaddFL{the statistics of the }\DIFaddendFL sample \DIFdelbeginFL \DIFdelFL{variances }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{variance }\DIFaddendFL as long as the \DIFdelbeginFL \DIFdelFL{$f_{NL}$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$f_\text{NL}$}\DIFaddendFL s are \DIFdelbeginFL \DIFdelFL{normally }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Gaussian }\DIFaddendFL distributed. \DIFdelbeginFL \DIFdelFL{Results }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{The results }\DIFaddendFL are statistically consistent with random fluctuations except potentially at $\omega=20$, as discussed in the main text.}
...
\begin{figure}[htbp!] 
	...
	\caption{Correlations between \DIFaddbeginFL \DIFaddFL{the }\DIFaddendFL resonance model templates \DIFdelbeginFL \DIFdelFL{$S(k_1,k_2,k_3) = \sin(\omega\log(k_1+k_2+k_3)+\phi)$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{of shape given by ${S(k_1,k_2,k_3) = \sin(\omega\log(k_1+k_2+k_3)+\phi)}$ }\DIFaddendFL with different $\omega$ values, for (a) $\phi = 0$ and (b) $\phi=\pi/2$. `Sample' correlations are obtained from the Legendre basis \DIFdelbeginFL \DIFdelFL{$f_{NL}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$f_\text{NL}$ }\DIFaddendFL estimates from 140 Gaussian simulations. `Theory' correlations come from the inner product induced by $\Gamma$ \DIFdelbeginFL \DIFdelFL{matrix }\DIFdelendFL as shown in (\ref{eqn:late_time_inner_product}).}
...
DBI inflation generates non-Gaussianity with its shape similar to the equilateral template \eqref{def:equilateral_template}. The latest constraints on the model \DIFdelbegin \DIFdel{comes }\DIFdelend \DIFaddbegin \DIFadd{come }\DIFaddend from the Planck CMB bispectrum analysis using an approximate template 
\begin{align}
	B_\Phi\DIFdelbegin \DIFdel{^{DBI}}\DIFdelend \DIFaddbegin \DIFadd{^\text{DBI}}\DIFaddend (k_1,k_2,k_3) = \frac{6A_\Phi^2}{(k_1 k_2 k_3)^3} \frac{-3/7}{(k_1 + k_2 + k_3)^2} \bigg[ \sum_i k_i^5 &+ \sum_{i\neq j} (2k_i^4 k_j - 3k_i^3 k_j^2)  \nonumber \\
	&+ \sum_{i\neq j\neq l} (k_i^3 k_j k_l - 4k_i^2 k_j^2 k_l) \bigg]. \label{eqn:dbi_bispectrum_template}
\end{align}
We switched to a convention where the potential $\Phi$ is used instead of $\zeta$. On superhorizon scales at the end of inflation\DIFaddbegin \DIFadd{, }\DIFaddend they differ by a constant factor: $\Phi = (3/5)\zeta$. With respect to this template, the theoretical bispectrum from DBI inflation has \DIFdelbegin \DIFdel{amplitude
}\DIFdelend \DIFaddbegin \DIFadd{an amplitude equal to
}\DIFaddend \begin{align}
	f\DIFdelbegin \DIFdel{_{NL}^{DBI} }\DIFdelend \DIFaddbegin \DIFadd{_\text{NL}^\text{DBI} }\DIFaddend \approx - \frac{35}{108} \left[ \DIFdelbegin \DIFdel{\frac{1}{\left( c^{DBI}_s \right)^2} }\DIFdelend \DIFaddbegin \DIFadd{\frac{1}{\left( c^\text{DBI}_\text{s} \right)^2} }\DIFaddend - 1 \right], \label{eqn:DBI_fNL_and_sound_speed}
\end{align}
an approximation which is accurate as long as the DBI sound speed \DIFdelbegin \DIFdel{$c^{DBI}_s \ll 1$}\DIFdelend \DIFaddbegin \DIFadd{$c^\text{DBI}_\text{s} \ll 1$}\DIFaddend . Constraints on \DIFdelbegin \DIFdel{$f_{NL}^{DBI}$ }\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}^\text{DBI}$ }\DIFaddend therefore place limits on \DIFdelbegin \DIFdel{$c_s$}\DIFdelend \DIFaddbegin \DIFadd{$c_\text{s}$}\DIFaddend . Planck 2018 analysis \cite{PlanckCollaboration2018} found \DIFdelbegin \DIFdel{$f_{NL}^{DBI} = 46 \;\pm\; 58$ }\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}^\text{DBI} = 46 \;\pm\; 58$ }\DIFaddend with a 68\% confidence level from CMB temperature data. The corresponding bound on the speed of sound is \DIFdelbegin \DIFdel{$c^{DBI}_s \ge 0.079$}\DIFdelend \DIFaddbegin \DIFadd{$c^\text{DBI}_\text{s} \ge 0.079$}\DIFaddend .

We reproduce the DBI sound speed constraint from Planck 2018 analysis using the \textsc{Primodal}$+$\textsc{CMB-BEst} routine. Unlike the Planck analysis, our method does not require templates to connect inflationary predictions to CMB data. Instead of using a single estimate from an approximate template, we scan over DBI theory parameters and constrain the individual primordial bispectrum directly. A given parameter set can be ruled out if the corresponding constraint on \DIFdelbegin \DIFdel{$f_{NL}$ excludes $f_{NL}=1$ }\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}$ excludes $f_\text{NL}=1$ }\DIFaddend with high confidence. 

One notable limitation in the current implementation of \textsc{Primodal} is that the details of how inflation ends are not prescribed. This means that the scales during inflation and what we observe cannot be mapped directly, which restricts our ability to translate the results into a constraint on more fundamental parameters such as \DIFdelbegin \DIFdel{$\beta_{IR}$}\DIFdelend \DIFaddbegin \DIFadd{$\beta_\text{IR}$}\DIFaddend . Our work here still serves as a useful validation of the combined pipeline. For the DBI sound speed scan, the approximate relation \eqref{eqn:DBI_fNL_and_sound_speed} is used to relate the output $c^*_s$\textemdash sound speed at some pivot scale during horizon crossing\textemdash from \textsc{Primodal} with the physical DBI sound speed \DIFdelbegin \DIFdel{$c^{DBI}_s$}\DIFdelend \DIFaddbegin \DIFadd{$c^\text{DBI}_\text{s}$}\DIFaddend . Numerous other validation tests we performed such as convergence tests with respect to the basis size \DIFdelbegin \DIFdel{$p_{max}$ }\DIFdelend \DIFaddbegin \DIFadd{$p_\text{max}$ }\DIFaddend are unaffected by this caveat and still \DIFdelbegin \DIFdel{serves }\DIFdelend \DIFaddbegin \DIFadd{serve }\DIFaddend as a proof of concept for our combined pipeline.

The parameter \DIFdelbegin \DIFdel{$\beta_{IR}$ }\DIFdelend \DIFaddbegin \DIFadd{$\beta_\text{IR}$ }\DIFaddend is varied within the interval $[0.1885,0.58]$ while fixing other parameters as \DIFdelbegin \DIFdel{$\lambda_{DBI} = 2.00475 \times 10^15$, $V_0 = 5.2 \times 10^{-12} M^4_{pl}$, and $\phi_0 = 0.46042 M_{pl}$, where $M_{pl}$ }\DIFdelend \DIFaddbegin \DIFadd{$\lambda_\text{DBI} = 2.00475 \times 10^15$, $V_0 = 5.2 \times 10^{-12} M^4_\text{pl}$, and $\phi_0 = 0.46042 M_\text{pl}$, where $M_\text{pl}$ }\DIFaddend is the Planck mass. Further details of the scan will be included in \cite{Sohn2021inprep}. The results are depicted in Figure \ref{fig:dbi_sound_speed_scan}.

\begin{figure}[htbp!] 
	\centering    
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{dbi_sound_speed_scan_annotated_new.pdf}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics{dbi_sound_speed_scan_annotated.pdf}
	\DIFaddendFL \caption{Constraints on the DBI model with varying sound speed \DIFdelbeginFL \DIFdelFL{$c_s$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$c_\text{s}$}\DIFaddendFL . Primordial bispectrum computed from \textsc{Primodal} is fed directly into \DIFdelbeginFL \DIFdelFL{CMB-BEst }\DIFdelendFL \DIFaddbeginFL \textsc{\DIFaddFL{CMB-BEst}} \DIFaddendFL in the form of coefficients $\alpha_{p_1 p_2 p_3}$ appearing in Legendre basis expansion. Constraints from the Legendre basis with \DIFdelbeginFL \DIFdelFL{$p_{max}=30$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$p_\text{max}=30$ }\DIFaddendFL (black $+$) are shown in comparison with \DIFdelbeginFL \DIFdelFL{$p_{max}=29$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$p_\text{max}=29$ }\DIFaddendFL (black $\times$). $1\sigma$ and $2\sigma$ confidence intervals around $0$ are shaded in blue and represent the uncertainty in the estimated \DIFdelbeginFL \DIFdelFL{$f_{NL}$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$f_\text{NL}$}\DIFaddendFL s. Bispectrum shapes are normalised so that \DIFdelbeginFL \DIFdelFL{$f_{NL}=1$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$f_\text{NL}=1$ }\DIFaddendFL corresponds to the DBI model under consideration. Our constraint on DBI sound speed is \DIFdelbeginFL \DIFdelFL{$c^{DBI}_s > 0.056$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$c^\text{DBI}_\text{s} > 0.056$ }\DIFaddendFL with $95\%$ confidence level. Equivalent results from an approximate DBI bispectrum template \eqref{eqn:dbi_bispectrum_template} (blue dashed) give the \DIFdelbeginFL \DIFdelFL{$c^{DBI}_s$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$c^\text{DBI}_\text{s}$ }\DIFaddendFL bound \DIFaddbeginFL \DIFaddFL{which is }\DIFaddendFL identical up to two significant figures.}
	\label{fig:dbi_sound_speed_scan}
\end{figure}

First of all, we check that our Legendre basis with \DIFdelbegin \DIFdel{$p_{max}=30$ }\DIFdelend \DIFaddbegin \DIFadd{$p_\text{max}=30$ }\DIFaddend has converged and accurately represents the primordial bispectrum from DBI inflation. Values of the \DIFdelbegin \DIFdel{$f_{NL}$ }\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}$ }\DIFaddend estimates obtained from \DIFdelbegin \DIFdel{$p_{max}=30$ }\DIFdelend \DIFaddbegin \DIFadd{$p_\text{max}=30$ }\DIFaddend were tested against the equivalent result with \DIFdelbegin \DIFdel{$p_{max}=29$ }\DIFdelend \DIFaddbegin \DIFadd{$p_\text{max}=29$ }\DIFaddend and shown to differ by less than $0.01\%$, as can be seen in Figure \ref{fig:dbi_sound_speed_scan}. In order to thoroughly investigate the degree of convergence, we studied a late-time equivalent of $\epsilon$ in  \eqref{def:primordial_shape_epsilon} used to compare primordial bispectrum shapes;
\begin{align}
	\epsilon^2 (\vv{f}_1, \vv{f}_2) = \DIFdelbegin \DIFdel{\frac{1}{N_{sim}}  }\DIFdelend \DIFaddbegin \DIFadd{\frac{1}{N_\text{sim}}  }\DIFaddend \sum_{i=1}\DIFdelbegin \DIFdel{^{N_{sim}} }\DIFdelend \DIFaddbegin \DIFadd{^{N_\text{sim}} }\DIFaddend \left( \frac{f^{(i)}_1}{\sigma_1} - \frac{f^{(i)}_2}{\sigma_2} \right)^2,
\end{align}
where $f^{(i)}_1$ and $f^{(i)}_2$ denote \DIFdelbegin \DIFdel{$f_{NL}$ estimate }\DIFdelend \DIFaddbegin \DIFadd{the $f_\text{NL}$ estimates }\DIFaddend for the $i$th Gaussian simulated map from the two routines. $\epsilon^2$ measures the mean squared error in the two sets of \DIFdelbegin \DIFdel{$f_{NL}$ }\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}$ }\DIFaddend signal-to-noises for \DIFdelbegin \DIFdel{$N_{sim}$ }\DIFdelend \DIFaddbegin \DIFadd{$N_\text{sim}$ }\DIFaddend simulations. Comparing results from the Legendre basis with \DIFdelbegin \DIFdel{$p_{max} = 30$ }\DIFdelend \DIFaddbegin \DIFadd{$p_\text{max} = 30$ }\DIFaddend and $25$ across the scan, we confirmed that $\epsilon < 0.01$ in the full range and $\epsilon \approx 0.002$ for the majority. A plot with the full result will be provided in \cite{Sohn2021inprep}.

Secondly, we constrain the DBI sound speed as \DIFdelbegin \DIFdel{$c_s \gtrapprox 0.056$ }\DIFdelend \DIFaddbegin \DIFadd{$c_\text{s} \gtrapprox 0.056$ }\DIFaddend with $95\%$ confidence using CMB temperature data from Planck. Each of the points in Figure \ref{fig:dbi_sound_speed_scan} \DIFdelbegin \DIFdel{are }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend normalised to some \DIFdelbegin \DIFdel{$\tilde{f}_{NL}$ such that $\tilde{f}_{NL}=1$ }\DIFdelend \DIFaddbegin \DIFadd{$\tilde{f}_\text{NL}$ such that $\tilde{f}_\text{NL}=1$ }\DIFaddend corresponds to the bispectrum predicted by the model. Estimates from the Planck 2018 map take negative values with error bars small enough to exclude \DIFdelbegin \DIFdel{$f_{NL}=1$ }\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}=1$ }\DIFaddend with $2\sigma$ confidence level when \DIFdelbegin \DIFdel{$c_s < 0.056$}\DIFdelend \DIFaddbegin \DIFadd{$c_\text{s} < 0.056$}\DIFaddend . Our constraint is similar but not identical to the result from Planck 2018 analysis which reads \DIFdelbegin \DIFdel{$c_s \ge 0.079$}\DIFdelend \DIFaddbegin \DIFadd{$c_\text{s} \ge 0.079$}\DIFaddend . The discrepancy stems from differences in the constraints for \DIFaddbegin \DIFadd{the }\DIFaddend DBI template \eqref{eqn:dbi_bispectrum_template}: \DIFdelbegin \DIFdel{$f^{DBI}_{NL}=27 \pm 58$ }\DIFdelend \DIFaddbegin \DIFadd{$f^\text{DBI}_\text{NL}=27 \pm 58$ }\DIFaddend for \textsc{Primodal}+\textsc{CMB-BEst} whereas \DIFdelbegin \DIFdel{$f^{DBI}_{NL}=46 \pm 58$ }\DIFdelend \DIFaddbegin \DIFadd{$f^\text{DBI}_\text{NL}=46 \pm 58$ }\DIFaddend for Planck. This $\approx0.32\sigma$ gap in signal to noise is \DIFdelbegin \DIFdel{largely }\DIFdelend \DIFaddbegin \DIFadd{mainly }\DIFaddend because the Modal expansion is incomplete \DIFdelbegin \DIFdel{so that }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend the correlation between the expanded and full DBI template equals $r\approx 0.95$. As discussed in Appendix B of \cite{PlanckCollaboration2013}, this level of correlation causes an expected scatter in \DIFdelbegin \DIFdel{$f_{NL}$}\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}$}\DIFaddend s approximately equal to $0.3\sigma$.

Lastly, we verify that the DBI template \eqref{eqn:dbi_bispectrum_template} accurately approximates the full numerical result obtained from \textsc{Primodal}. \DIFdelbegin \DIFdel{Shown in blue dashed line }\DIFdelend \DIFaddbegin \DIFadd{Drawn in blue-dashed }\DIFaddend on Figure \ref{fig:dbi_sound_speed_scan}, the difference between \DIFdelbegin \DIFdel{$\tilde{f}_{NL}$ }\DIFdelend \DIFaddbegin \DIFadd{the $\tilde{f}_\text{NL}$ }\DIFaddend constraints obtained using \textsc{Primodal} and the DBI template differ by \DIFdelbegin \DIFdel{order $O(0.01)\sigma$}\DIFdelend \DIFaddbegin \DIFadd{$O(0.01\sigma)$}\DIFaddend .

\bigskip

We extend the \textsc{Primodal}+\textsc{CMB-BEst} pipeline to study DBI models with resonance. The primordial bispectrum appears similar to the DBI template but has extra log-spaced oscillations superimposed. The Planck analysis \DIFdelbegin \DIFdel{covers both }\DIFdelend \DIFaddbegin \DIFadd{covered the }\DIFaddend DBI shape and \DIFaddbegin \DIFadd{the }\DIFaddend resonance templates independently \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{PlanckCollaboration2018} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend but has not constrained the combination of the two. Our method studies the bispectrum obtained directly from inflation without using approximate templates. We vary the effective frequency $\omega$ of the oscillations while keeping other DBI theory parameters fixed. Figure \ref{fig:dbi_resonance_scan} summarises the results of the scan.

\begin{figure}[htbp!] 
	\centering    
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.8\textwidth]{dbi_reso_scan_fNLs_new.pdf}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics{dbi_reso_scan_fNLs_new.pdf}
	\DIFaddendFL \caption{Constraints on the DBI model with oscillations imposed on the inflationary potential, causing resonance-type bispectra approximately parametrised as $\sin(\omega \log(k_1+k_2+k_3) + \phi)$. The primordial bispectrum computed from \textsc{Primodal} for various oscillation frequency $\omega$ has been fed directly to \DIFdelbeginFL \DIFdelFL{CMB-BEst}\DIFdelendFL \DIFaddbeginFL \textsc{\DIFaddFL{CMB-BEst}}\DIFaddendFL . The results (black line) are shown together with the ones obtained from a smaller basis where \DIFdelbeginFL \DIFdelFL{$p_{max}=29$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$p_\text{max}=29$ }\DIFaddendFL (blue $\times$). The blue regions in the plot represent $1\sigma$ and $2\sigma$ confidence intervals.}
	\label{fig:dbi_resonance_scan}
\end{figure}

We find no significant detection of non-Gaussianity in the parameter range we study. Note also that our \DIFdelbegin \DIFdel{$f_{NL}$ }\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}$ }\DIFaddend estimates become unreliable after $\omega\gtrsim2.0$ as the Legendre basis fails to represent rapid oscillations accurately. This can be seen by comparing the estimates from \DIFaddbegin \DIFadd{the }\DIFaddend bases of size \DIFdelbegin \DIFdel{$p_{max}=30$ }\DIFdelend \DIFaddbegin \DIFadd{$p_\text{max}=30$ }\DIFaddend and $29$ in the figure. Augmenting the Legendre basis with specific functions or choosing an alternative set of basis \cite{Clarke2021} can improve the convergence up to $\omega\approx5$ (to be included in \cite{Sohn2021inprep}), which is a significant improvement but still rather restrictive compared to the viable parameter range of $\omega\lesssim 35$ for the `vanilla' resonance models shown in Figure \ref{fig:sinlog_template_frequency_Legendre_Modal}.

Investigating the lack of convergence has revealed a more fundamental issue with the primordial basis expansion. Earlier in this chapter, we discussed how the tetrapyd domain of \DIFaddbegin \DIFadd{the }\DIFaddend primordial bispectrum can be extended to the cube given by \DIFdelbegin \DIFdel{$[k_{min},k_{max}]^3$ }\DIFdelend \DIFaddbegin \DIFadd{$[k_\text{min},k_\text{max}]^3$ }\DIFaddend such that the decomposition coefficients can be computed accurately and efficiently via \eqref{eqn:basis_expansion}. By doing so, we are expanding the given bispectrum in a cube which contains the tetrapyd while analytically continuing the function in between the two. Having good convergence in the cube then implies good convergence in the tetrapyd inside it. However, in cases where the target function behaves badly outside the tetrapyd, trying to fit the function in the whole cube instead of the tetrapyd may be suboptimal.

We demonstrate this issue by computing the \DIFdelbegin \DIFdel{$f_{NL}$ }\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}$ }\DIFaddend constraints using a bispectrum template \DIFdelbegin \DIFdel{$S(k_1,k_2,k_3) = S^{equil}(k_1,k_2,k_3) \sin(\omega \log(k_1+k_2+k_3))$}\DIFdelend \DIFaddbegin \DIFadd{$S(k_1,k_2,k_3) = S^\text{equil}(k_1,k_2,k_3) \sin(\omega \log(k_1+k_2+k_3))$}\DIFaddend : the equilateral shape with sine-log oscillations imposed. Figure \ref{fig:sinlog_equil_template_decomp_comparison} shows two plots analogous to Figure \ref{fig:sinlog_template_correlations}. The correlations between \DIFdelbegin \DIFdel{$f_{NL}$}\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}$}\DIFaddend s estimated from different values of $\omega$ are computed using two different approaches for the primordial basis expansion. Shown on the \DIFdelbegin \DIFdel{left hand }\DIFdelend \DIFaddbegin \DIFadd{left-hand }\DIFaddend side is the result for when we evaluate the given shape function $S(k_1,k_2,k_3)$ within the whole \textit{cube}, as we have so far. We see that the basis fails to express the given bispectrum template accurately after $\omega\gtrsim4$. Large correlations shown in the plot are simply artefacts of inaccuracy in the expansion.

\begin{figure}[htbp!] 
	\centering    
	\includegraphics[width=\textwidth]{sinlog_equil_template_correlations_compare_decomp.pdf}
	\caption{Correlations between \DIFdelbeginFL \DIFdelFL{$f_{NL}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$f_\text{NL}$ }\DIFaddendFL constraints for the enveloped resonance shape \DIFdelbeginFL \DIFdelFL{$S(k_1,k_2,k_3) = S^{equil}(k_1,k_2,k_3) \sin(\omega \log(k_1+k_2+k_3))$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$S(k_1,k_2,k_3) = S^\text{equil}(k_1,k_2,k_3) \sin(\omega \log(k_1+k_2+k_3))$ }\DIFaddendFL with different $\omega$s. Since the given shape behaves badly in the region outside the tetrapyd but within the cubic domain of (\ref{eqn:basis_expansion}), we observe large off-diagonal correlations in the original Legendre basis expansion (Left). Manually setting the shape function to zero at these unphysical configurations therefore dramatically improves numerical performance (Right). }
	\label{fig:sinlog_equil_template_decomp_comparison}
\end{figure}

On the \DIFdelbegin \DIFdel{right hand }\DIFdelend \DIFaddbegin \DIFadd{right-hand }\DIFaddend side is the result for which the shape function values outside the tetrapyd were manually set to zero during the primordial basis expansion. The modified shape function is still continuous within the cubic domain\DIFaddbegin \DIFadd{, }\DIFaddend since the equilateral shape \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{vanishes on boundaries of the tetrapyd }\DIFaddend despite its divergence outside \DIFdelbegin \DIFdel{the tetrapyd, vanishes on the boundaries }\DIFdelend of it. We see that this simple trick has dramatically improved the basis' ability to expand highly oscillatory bispectrum templates. The cross-correlations between templates remain small up until $\omega\approx75$, quite similar to what we saw for the sine-log templates. We confirm that it is indeed the bad behaviour of the templates outside the tetrapyd domain during the primordial basis expansion that causes the lack of convergence. We plan to explore this topic further in the future.
...
The standard model of cosmology has \DIFdelbegin \DIFdel{had a remarkable success}\DIFdelend \DIFaddbegin \DIFadd{been remarkably successful}\DIFaddend ; the CMB power spectrum from Planck, for example, shows an exquisite fit to the $\Lambda$CDM model with only six free parameters. \DIFdelbegin \DIFdel{There are many unanswered questions remaining }\DIFdelend \DIFaddbegin \DIFadd{Many questions remain unanswered, }\DIFaddend however, especially regarding \DIFdelbegin \DIFdel{the }\DIFdelend early universe physics. The most widely accepted theory for the early universe is inflation, whose prediction of the scale-invariant primordial spectra has been tested to be consistent with the observed data. Inflation also resolves the horizon and flatness problems present in the vanilla Big Bang cosmology. The simplest scenario of the single-field slow-roll inflation with a canonical kinetic term can explain the current observations, but \DIFdelbegin \DIFdel{there are also }\DIFdelend numerous other physically well-motivated scenarios \DIFdelbegin \DIFdel{that }\DIFdelend are yet to be ruled out.

The key to distinguishing different models of inflation is primordial non-Gaussianity. Deviations from the most canonical inflation model leave weak non-Gaussian signatures in the primordial perturbations. These imprints are best captured in the bispectrum of the primordial perturbations. Studying the shape and amplitude of the primordial bispectrum\DIFdelbegin \DIFdel{therefore }\DIFdelend \DIFaddbegin \DIFadd{, therefore, }\DIFaddend allows us to constrain various inflation models directly. The CMB is the \DIFdelbegin \DIFdel{most }\DIFdelend ideal probe for this job since its anisotropy depends linearly on the initial density perturbations. The CMB bispectrum hence directly relates to the primordial bispectrum and is used to construct the optimal estimator for the primordial non-Gaussianity parameter \DIFdelbegin \DIFdel{$f_{NL}$}\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}$}\DIFaddend . 

The most recent Planck analysis has constrained \DIFdelbegin \DIFdel{$f_{NL}$ }\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}$ }\DIFaddend to great precision using the CMB bispectrum estimator. No primordial source of non-Gaussianity has been found, and \DIFdelbegin \DIFdel{$f_{NL}$}\DIFdelend \DIFaddbegin \DIFadd{$f_\text{NL}$}\DIFaddend s for the standard shapes are currently consistent with $0$. More interesting `hints' of non-Gaussianity have been found in models with oscillations in the bispectrum. Signals of $3$-$4\sigma$ significance have been found in these models, but scanning over a large parameter space meant that these are not yet conclusive detections. A further investigation using independent and more precise measurements would be extremely beneficial.

The next generation of CMB experiments is anticipated to measure the polarisation of the CMB with greatly enhanced sensitivity. Given that the Planck constraints on oscillatory models benefited much more from the inclusion of polarisation data than other shapes, we predict that the constraining power would benefit immensely from the future CMB data. Our work presented in Chapter 4 \DIFdelbegin \DIFdel{forecast that with }\DIFdelend \DIFaddbegin \DIFadd{forecasts that }\DIFaddend the most sensitive CMB Stage-4 experiment specification \DIFdelbegin \DIFdel{translates, we expect }\DIFdelend \DIFaddbegin \DIFadd{is expected to yield }\DIFaddend a factor of 1.7-2.2 times more stringent constraints compared to Planck.

Despite the bright prospects and growing \DIFdelbegin \DIFdel{interests for }\DIFdelend \DIFaddbegin \DIFadd{interest in }\DIFaddend constraining oscillatory models, a large part of the model and parameter space are currently unconstrained due to numerical and computational difficulties. The CMB bispectrum estimation is a challenging task where the na\"ive computation is practically impossible. There are two main approaches to this: KSW and Modal.
\DIFaddbegin 

\DIFaddend The KSW estimator exploits the inherent separability of the primordial bispectrum template \DIFdelbegin \DIFdel{to efficiently. This method }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend can accurately constrain even highly oscillatory shapes \DIFdelbegin \DIFdel{, but is severely }\DIFdelend \DIFaddbegin \DIFadd{but is }\DIFaddend restrictive in the type of bispectrum shapes it can handle. On the other hand, the Modal estimator \DIFdelbegin \DIFdel{expands the primordial and late-time bispectra in terms of }\DIFdelend \DIFaddbegin \DIFadd{uses }\DIFaddend separable mode functions to \DIFdelbegin \DIFdel{tackle the problem. The Modal code is heavily optimised and can }\DIFdelend \DIFaddbegin \DIFadd{expand the primordial and late-time bispectra and hence is able to }\DIFaddend constrain a wide range of non-separable shapes. However, \DIFdelbegin \DIFdel{the Modal estimator has troubles dealing with high frequency oscillations, since the basis expansion struggles to converge. Choosing a specialised basis set to handle oscillations is also complicated by the fact that }\DIFdelend \DIFaddbegin \DIFadd{dealing with high-frequency oscillations is challenging for Modal because }\DIFaddend there are two separate basis sets\textemdash primordial and late-time\textemdash to consider\DIFaddbegin \DIFadd{; high-frequency primordial basis functions lose a lot of their features during the projection, making it hard to expand them using the late-time basis functions accurately}\DIFaddend .

Our novel approach to the CMB bispectrum estimation\DIFaddbegin \DIFadd{, }\DIFaddend \textsc{CMB-BEst}\DIFaddbegin \DIFadd{, }\DIFaddend is designed to combine the strengths of the KSW and Modal estimators, and hence is suitable for constraining general oscillatory models using future CMB data. We use the primordial basis expansion to decompose the given bispectrum shape into separable terms \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{like Modal but }\DIFaddend then apply a KSW-like method to obtain constraints on each of them. \DIFdelbegin \DIFdel{The }\DIFdelend \textsc{CMB-BEst} works for general basis sets \DIFdelbegin \DIFdel{, and is in fact equivalent }\DIFdelend \DIFaddbegin \DIFadd{and contains }\DIFaddend the KSW estimator \DIFdelbegin \DIFdel{for a simple choice of basis}\DIFdelend \DIFaddbegin \DIFadd{as a sub-case}\DIFaddend .
\DIFdelbegin \DIFdel{It can handle high frequency oscillations as accurately as the KSW estimator, and applies to general non-separable shapes like the Modal estimator}\DIFdelend \DIFaddbegin 

\DIFadd{The main strength of }\textsc{\DIFadd{CMB-BEst}} \DIFadd{lies in its flexibility and accuracy; the code can take general basis functions which get projected using an exact analytic form. This would be especially useful for studying general bispectrum shapes with high-frequency oscillations that are currently unconstrained due to practical difficulties in numerical calculations}\DIFaddend .

\DIFdelbegin \DIFdel{There is no free lunch, however. }\DIFdelend The \textsc{CMB-BEst} pipeline is\DIFaddbegin \DIFadd{, however, }\DIFaddend computationally more costly than both \DIFdelbegin \DIFdel{of }\DIFdelend the KSW and Modal ones. We therefore invested a significant amount of time optimising the code \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{at }\DIFaddend both algorithmic and implementation levels. The code was then parallelised \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{on }\DIFaddend multiple levels to fully benefit from the modern computing architecture. The total runtime has improved many orders of magnitude \DIFdelbegin \DIFdel{over the course of }\DIFdelend \DIFaddbegin \DIFadd{throughout }\DIFaddend this process. The completed code has then thoroughly tested against the Modal pipeline used in the Planck 2018 analysis. We found that the results are consistent with Planck map-by-map \DIFaddbegin \DIFadd{using 140 simulated maps, }\DIFaddend for the standard \DIFdelbegin \DIFdel{bispectrum shapes , }\DIFdelend \DIFaddbegin \DIFadd{shapes }\DIFaddend as well as the feature and resonance templates.

\textsc{CMB-BEst} has been validated and is now in the exploitation phase. Working in collaboration with the authors of \textsc{Primodal}, we developed a fluid pipeline where a given inflationary Lagrangian can be directly constrained without the use of approximate templates. This allows a fast and accurate constraint to the specific model under consideration. As a proof-of-concept example, we presented \DIFdelbegin \DIFdel{the }\DIFdelend work on constraining the DBI sound speed in Chapter 5, where we obtained \DIFdelbegin \DIFdel{$c_s^{DBI} \ge 0.056$ }\DIFdelend \DIFaddbegin \DIFadd{$c_\text{s}^\text{DBI} \ge 0.056$ }\DIFaddend with 95\% confidence. The combined \textsc{Primodal}+\textsc{CMB-BEst} pipeline can perform similar scans on other theoretical \DIFdelbegin \DIFdel{paramters}\DIFdelend \DIFaddbegin \DIFadd{parameters}\DIFaddend , which we plan to do in the near future.

\DIFaddbegin \DIFadd{We further demonstrated that }\textsc{\DIFadd{CMB-BEst}} \DIFadd{can indeed handle high-frequency oscillations of the resonance-type bispectra more stably compared to Modal. Its resolving power is limited only by the accuracy in its primordial basis expansion, unlike Modal which also depends on the late-time counterpart. Our next goal is to develop and utilise specialised basis sets tailored to highly oscillatory bispectrum shapes of interest. 
}

\DIFaddend There are currently two main \DIFdelbegin \DIFdel{rooms }\DIFdelend \DIFaddbegin \DIFadd{routes }\DIFaddend for improvement for \textsc{CMB-BEst}. First, the code can be generalised to incorporate the E-mode polarisation data. Implementing this should be straightforward using an orthonormalisation defined in Chapter 4. The additional computational complexity involved in the extra set of spherical harmonic transforms for polarisation is expected to be subdominant and have \DIFaddbegin \DIFadd{a }\DIFaddend small impact overall. Second, we plan on improving the current method used for the decomposition of a given primordial shape function. We will investigate further \DIFdelbegin \DIFdel{on }\DIFdelend the effect of the integration domain, as we saw from the example of the DBI resonance models in Chapter 5. \DIFaddbegin \DIFadd{We note that this concerns only the last step of the pipeline and the main parts of the bispectrum estimation remain unaffected.
}\DIFaddend 
...
\DIFaddend \begin{spacing}{0.9}



\DIFdelbegin %DIFDELCMD < \bibliographystyle{apalike}
%DIFDELCMD < \cleardoublepage
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \bibliographystyle{JHEP}
\DIFaddend \bibliography{References/references} 
