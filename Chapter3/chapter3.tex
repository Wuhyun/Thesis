...
In a single field slow-roll inflation with standard kinetic term and vacuum, the primordial bispectrum is suppressed by slow-roll parameters \cite{Maldacena2013}. Numerous other inflationary scenarios that are physically well-motivated predict non-Gaussian signatures. They are expected to leave imprints on CMB bispectrum with characteristic shape and amplitude, parametrised by $f_{NL}$.

We discuss bispectrum in relation to primordial non-Gaussianity in this section, from both theoretical and observational side. Section \ref{section:bispectrum} covers basic formalism of bispectrum analysis. Section \ref{section:primordial_non_Gaussianity} then introduces theoretical tools for deriving primordial bispectrum from a given inflation model Lagrangian. On the CMB side, we formulate the optimal bispectrum estimator and existing implementations of it in Section \ref{section:CMB_bispectrum_estimation}.
...
\end{align}
Note that the bispectrum $B(k_1,k_2,k_3)$ is a three-dimensional function, as opposed to the one-dimensional power spectrum $P(k)$. The domain of $B$ is further restricted by the constraint that $k_1,k_2,k_3$ must form a triangle.

The functional form of bispectrum comprises two parts: its dependence on the overall scaling $K=k_1+k_2+k_3$, called \textit{running}, and the design of triangle formed by $k_1$,$k_2$ and $k_3$, the \textit{shape}. Some notable shapes are depicted in Figure \ref{fig:triangle_configurations}.
...
We write $P_zeta(k) = A_\zeta k^{n_s-4}$ using power spectrum amplitude $A_\zeta$ and scalar spectral index $n_s$. The local template is then given by
\begin{align}
	B_\zeta^{local} (k_1, k_2, k_3) := 2A_\zeta^2 \left[ \frac{1}{k_1^{4-n_s} k_2^{4-n_s}} + \text{\;2 cyc.} \right].
\end{align}
Note that we do not include $f_{NL}^{local}$ in our definition of the template either; it is set to $1$.

The bispectrum for multi-field models typically fall into this category (reviewed in e.g., \cite{Byrnes2010review}). The local shape peaks in the squeezed limit where $k_1 \approx k_2 \gg k_3$, diverging as $k_3\rightarrow0$. General single field models, on the other hand, is better described by the equilateral and orthogonal templates \cite{Creminelli2006limits,Senatore2010orthogonal} .
\begin{align}
	...
	&\hspace{0.2\textwidth} + \left. \left( \frac{1}{k_1^{(4-n_s)/3} k_2^{2(4-n_s)/3} k_3^{(4-n_s)}} + \text{5 cyc.} \right) \right], \\
	...
	&\hspace{0.2\textwidth} + \left. 3\left( \frac{1}{k_1^{(4-n_s)/3} k_2^{2(4-n_s)/3} k_3^{(4-n_s)}} + \text{5 cyc.} \right) \right].
\end{align}
The equilateral and orthogonal shapes are peaked at the equilateral and flattened limit shown in Figure \ref{fig:triangle_configurations}, respectively. The latter was constructed explicitly to probe bispectrum shapes perpendicular to the local and equilateral ones.

\section{Primordial non-Gaussianity} \label{section:primordial_non_Gaussianity}

Previously in Section \ref{section:quantum_fluctuations}, we derived the power spectrum of perturbations in the inflation field assuming homogeneous background. This section outlines how to compute general cosmological correlation functions from single field inflation, now in the perturbed metric.

We introduce the necessary tools in Section \ref{section:in_in_formalism} and \ref{section:ADM_formalism}. Bispectrum from a general type of single field inflation is derived in Section \ref{section:bispectrum_from_single_field_inflation}. We focus on illustrating the framework without delving much into the technical details. A simple example is used to demonstrate how explicit calculations are done, while we refer to, e.g., \cite{Maldacena2013,Chen2010,Burrage2011large} for the full (and laborious) calculations.
...
\end{align}
where the lapse and shift functions $N$ and $N^i$, respectively, are non-dynamical variables in the action. They act as Lagrange multipliers to give constraint equations. We focus on scalar perturbations here and ignore vector and tensor modes, owing to the SVT theorem. The spatial metric $h_{ij}$ contains two of them. When inflation driven by a single scalar field, there is one extra mode from perturbations of the field. Out of the three dynamical scalar degrees of freedom, two can be fixed by a gauge choice. (TODO: add reference to section 2 here) We choose a gauge so that the inflation field is uniform, and the spatial metric takes the form
\begin{align}
	h_{ij} = a(t)^2 \;e^{2\zeta} \; \delta_{ij},  \label{eqn:spatial_metric_curvature_perturbation}
\end{align}
where $\zeta(\vv{x},t)$ is equivalent to the curvature perturbation we defined earlier (TODO: add reference to section 2 here). Note that, if we define $N(t) = \int^{t}_{t_0} H dt' = \ln a(t) - \ln a(t_0)$ (not to be confused with the lapse function $N$) to represent the number of $e$-folds of expansion between time $t$ and $t_0$, then $\zeta = \delta N$. 
	
We now write down the action. Compared to \eqref{eqn:real_scalar_field_action}, the Einstein-Hilbert term is added since the metric is no longer fixed. We consider a generalised form of the field Lagrangian;
\begin{align}
	S = \int dt d^3 \vv{x} \; \sqrt{-g} \left[ \frac{M_p}{2} R + P(X,\phi) \right],  \label{eqn:general_single_field_action}
\end{align}
where $M_p := (8\pi G)^{-1/2}$ is the reduced Planck mass, and $X:=-\frac{1}{2} g^{\mu\nu} \partial_\mu \phi \partial_\nu \phi$ denote the canonical kinetic term. We set $M_p=1$ for convenience. $P(X,\phi)$ is an arbitrary function which generalises $X-V(\phi)$. (TODO: add references to P(X,phi) models).

One notable consequence of non-trivial kinetic term $P$ is that fluctuations in the inflation field no longer necessarily propagate with speed of light. The sound speed is defined as
\begin{align}
	c_s^2 = \frac{P_{,X}}{P_{,X} + 2 X P_{,XX}},
\end{align}
...
In order to obtain action for $\zeta$, we substitute \eqref{eqn:spatial_metric_curvature_perturbation} into the expression and expand perturbatively in $\zeta$. Terms of order up to three are relevant for three-point correlation functions. It is sufficient to evaluate $N$ and $N_i$ to first order in $zeta$, since they multiply with constraint equations which vanish up to first order.
...
The third order contains many terms with varying contributions to bispectrum. We write only one particular term here for a later example, and refer to \cite{Chen2007b} for the full result.
\begin{align}
...
For our simplified calculation of power spectrum from inflation derived in Section \ref{section:quantum_fluctuations}, Hamiltonian \eqref{eqn:single_field_inflation_simple_hammiltonian} of the perturbative field $v(\vv{x},\tau) := a(t) \delta\phi(\vv{x},t)$ consisted only of quadratic terms. This is no longer the case. Third-order and higher terms act as couplings that contribute to correlation functions.

A standard practise is to work in the interaction picture where the Hamiltonian is split into a quadratic part and the interacting part;
\begin{align}
	H = H_0 + H_{int}.
\end{align}
Operators and states both depend on time in the interaction picture. Main advantage of the interaction picture is that we can use results from the free ($H=H_0$) theory to evolve operators in time. In particular, we may canonically quantise the field $\hat{\zeta}(\vv{x},t)$ in terms of its Fourier transform.
\begin{align}
...
The in-in formalism (TODO: add references here) lets us calculate correlation functions at given time $t$ using two `in' states: vacua at infinite past in our case. For some local operator $Q(t)$ written as a product of $\hat{\zeta}(\vv{x},t)$ and $\hat{\pi}(\vv{x},t)$, we define correlation as
\begin{align}
...
\end{align}
The interaction picture vacuum $|\vv{0}\rangle$ lie in the infinite past $t=t_0$, where we assume that it can be identified as the vacuum of non-interacting theory; $a_\vv{k}|\vv{0}\rangle = 0$.

Note that the operators within \eqref{eqn:in_in_formalism_expectation} does not time ordered, thanks to the anti-time-ordering operator $\bar{T}$. To complicate things further, we do not have an equivalent of the Feynmann propagator in Minkowski space that handles time ordering for us. We instead treat the time ordering manually. Define \textit{contraction} between two terms $\zeta(\vv{k}_1,t_1)$ on the left and $\zeta(\vv{k}_2,t_2)$ on the right as the commutator
\begin{align}
...
\end{align}
where the positive and negative frequency solutions $\zeta^+$ and $\zeta^-$ refer to terms with $\hat{a}$ and $\hat{a}^\dagger$, respectively. Note that swapping the order of $\zeta(\vv{k}_1,t_1)$ and $\zeta(\vv{k}_2,t_2)$ does alter the value of this commutator. We make equivalent definitions to quantised conjugate momenta $\hat{\pi}_\vv{k}$.

We can now have a result comparable to Wick's theorem.
\begin{align}
	\langle\vv{0}| \hat{O}(\vv{k}_1,t_2) \cdots \hat{O}(\vv{k}_n,t_n) |\vv{0}\rangle = \;:\; \hat{O}(\vv{k}_1,t_2)  \cdots \hat{O}(\vv{k}_n,t_n) \;:\; + \;:\;\text{all possible constractions}\;:\;. \label{eqn:in_in_Wicks_theorem}
\end{align}
...
Before proceeding to computing correlation functions using the in-in formalism, we first need to obtain the Hamiltonian of the system. The conjugate momentum is defined as $\pi = \partial \mathcal{L} / \partial \dot{\zeta}$, where $\mathcal{L}$ is the Lagrangian density. The Hamiltonian density takes the form $\mathcal{H} = \pi \dot{\zeta} - \mathcal{L} = \mathcal{H}_0 + \mathcal{H}_{int}$. In the order of perturbations we are studying, $\mathcal{H}_{int}$ is simply equal to $-\mathcal{L}_3$ from the cubic action $S_3$. It is also sufficient to consider leading contribution of $\zeta$, so that $\pi=\dot{\zeta}$. We will abuse notation a little and use $\dot{\zeta}$ in place of $\pi$ for the rest of this section. (TODO: add reference to Lim's notes?)
...
\end{align}
where we used the definition of Fourier transform and omitted the time dependence for brevity.
...
\end{align}
Note that contractions within the same time causes one of the $\vv{p}_j$ to be $\vv{0}$ because of the delta function in \eqref{def:interaction_hamiltonian_zeta_dot_cube}. $\zeta_\vv{0}$ corresponds to a constant scaling of the background $a(t)$ and therefore vanishes.

Combining results so far, we obtain an expression for bispectrum as follows.
\begin{align}
...
\subsection{Other sources of non-Gaussianity.}
...
Another significant late-time contribution to CMB anisotropy is from the Integrated Sachs-Wolfe (ISW) effect. This is the last term in \eqref{eqn:line_of_sight_solution_approximated} with a simple change of variable:
\begin{align}
	\Theta_{ISW}(\eta_0,\vv{x}_0, \hat{\vv{n}}) = 2\int_0^{\chi_*} d\chi \; \Psi'_W (\chi\hat{\vv{n}}; \eta_0-\chi).
\end{align}



Component separation, foreground.
SMICA-type cleaning.
Point sources.
