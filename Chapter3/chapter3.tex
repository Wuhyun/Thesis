...
\end{align}
The normal ordering $::$ keeps $\hat{a}$s to the right and $\hat{a}^\dagger$ to the left, so that its vacuum expectation vanishes. The fields $\hat{O}$ are either $\hat{\zeta}$ or $\hat{\pi}$. 
