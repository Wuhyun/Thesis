...
One notable consequence of non-trivial kinetic term $P$ is that fluctuations in the inflation field no longer necessarily propagate with speed of light. The sound speed, defined as the ratio $\delta P / \delta \rho$, is given by
\begin{align}
...
\end{align}
The integrand is small except during the accelerated expansion in low redshift, again $z\lessapprox2$. The two integrals \eqref{def:lensing_potential} and \eqref{eqn:ISW_effect} are highly correlated through $\Psi_W$ and thus generate non-trivial bispectrum mainly in the squeezed limit. The resulting lensing-ISW (LISW) bispectrum is well approximated by the template given by \cite{Lewis2011lensing}
\begin{align}
	b^{LISW}_{l_1 l_2 l_3} = \frac{1}{2}\left[ l_1 (l_1 + 1) - l_2 (l_2 + 1) + l_3 (l_3 + 1) \right] \tilde{C}_{l_1}^{TT} C_{l_3}^{T\phi} + \text{5 perms.},
\end{align}
where $\tilde{C}^{TT}_l$ is the \textit{lensed} angular power spectrum and $C^{T\psi}_l$ is the cross power spectrum between temperature and $\psi$. Assuming standard $\Lambda$CDM cosmology we expect to find the bispectrum above in the CMB without any additional scale factors. We can measure its amplitude through the usual bispectrum estimator; $\hat{f}^{LISW}_{NL} = \langle \vv{b}^{LISW}, \; \vv{b}^{obs} \rangle / \langle \vv{b}^{LISW}, \; \vv{b}^{LISW} \rangle$ using the inner product from \eqref{def:bispectrum_estimation_fisher_inner_product}. This value was first detected using Planck 2013 CMB maps to a $2.5\sigma$ level \cite{PlanckCollaboration2013ISW}. The most recent Planck analysis estimates $f^{LISW}_{NL} = 0.73 \pm 0.27$ using SMICA foreground-cleaned CMB temperature maps \cite{PlanckCollaboration2018}.

The lensing-ISW bispectrum is a physically meaningful secondary observable from 
