...
\end{align}
Wigner 3-j symbols, written here as a 2-by-3 matrix, arise from formal addition of angular momenta. They are real coefficients appearing in the expansion of zero total angular momentum state $|0 \; 0\rangle$;
\begin{align}
...
\end{align}
The normalisation condition (\ref{eqn:wigner_3j_normalisation}) can be easily derived by computing norm of the state $|0 \; 0 \rangle$ in definition. The second identity (\ref{eqn:wigner_3j_legendre_integral}) allows us to rewrite the given 3-j symbol to a separable integral.
...
\begin{align}
	...
	=& \frac{(2l_1+1)(2l_2+1)(2l_3+1)}{8\pi} \int_{-1}^{1} d\mu \; P_{l_1}(\mu) P_{l_2}(\mu) P_{l_3}(\mu).
\end{align}
...
\begin{align}
	B(k_1,k_2,k_3) = \sum_i f_{NL}^{(i)} B^{(i)}(k_1,k_2,k_3)
\end{align}
