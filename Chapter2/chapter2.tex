...
Gaussianity of primordial perturbations for slow-roll inflation means that the $a_{lm}$s, evolved linearly from the initial fluctuations, are also Gaussian distributed. CMB measurements show that the $a_{lm}$s indeed follow multivariate Gaussian distribution. The form of \eqref{eqn:alm_from_phi} shows that the multipoles $a_{lm}$ are uncorrelated with each other, and since they are Gaussian distributed, vanishing correlation implies independence. In particular, $a_{lm}$s with same $l$ but distinct $m$s are independent and identically distributed. We can therefore combine them to get a more precise estimate for $C_l$;
\begin{align}
