...
The linearised Einstein field equations \eqref{eqn:perturbed_einstein_equations_1}-\eqref{eqn:perturbed_einstein_equations_3} dictates the time evolution of gravitational perturbations given the total energy, momentum, and pressure of the universe's components. Contributions to these characteristics from cold dark matter can be solved using the perturbed continuity equation \eqref{eqn:perturbed_continuity_equation} and Euler equations \eqref{eqn:perturbed_Euler_equation}. This section covers how perturbations in the photon temperature evolve and translates to the measured CMB anisotropy, the main observable of this thesis.

Studying photons has two major complications compared to cold dark matter. First, photons are relativistic particles. While the perturbations in cold dark matter are completely characterised by the two parameters $\delta\rho_m$ and $\vv{v}_m$,

 Our next target is to study perturbations in photon distributions and outline 

We now draw our attention to photons. In particular, we would like to study how perturbations in the photon temperature evolve in time during 
...
\subsubsection*{Collisionless equation} \label{section:collisionless_equation}
\subsubsection*{Thomson scattering} \label{section:thompson_scattering}

\subsection{Line of sight solution} \label{section:line_of_sight_solution}
